...
void CXMLBase::operator=( const CXMLBase& markup )
{
...
bool CXMLBase::SetDoc( LPCTSTR szDoc )
{
	// Set document text
	if ( szDoc )
		m_strDoc = szDoc;
...
bool CXMLBase::Load( LPCTSTR szFileName )
{
...
bool CXMLBase::ReadTextFile( LPCTSTR szFileName, CString& strDoc, CString* pstrError, int* pnFlags )
{
...
bool CXMLBase::Save( LPCTSTR szFileName )
{
	return WriteTextFile( szFileName, m_strDoc, &m_strError, &m_nFlags );
}

bool CXMLBase::WriteTextFile( LPCTSTR szFileName, CString& strDoc, CString* pstrError, int* pnFlags )
{
...
bool CXMLBase::FindElem( LPCTSTR szName )
{
...
bool CXMLBase::FindChildElem( LPCTSTR szName )
{
...
CString CXMLBase::EscapeText( LPCTSTR szText, int nFlags )
{
...
CString CXMLBase::UnescapeText( LPCTSTR szText, int nTextLength /*=-1*/ )
{
...
int CXMLBase::FindNode( int nType )
{
...
CString CXMLBase::GetTagName() const
{
...
bool CXMLBase::GetOffsets( int& nStart, int& nEnd ) const
{
...
CString CXMLBase::GetAttribName( int n ) const
{
...
bool CXMLBase::SavePos( LPCTSTR szPosName )
{
...
bool CXMLBase::RestorePos( LPCTSTR szPosName )
{
...
bool CXMLBase::x_AllocPosArray( int nNewSize /*=0*/ )
{
...
int CXMLBase::x_ParseElem( int iPosParent, TokenPos& token )
{
...
bool CXMLBase::x_FindAny( LPCTSTR szDoc, int& nChar )
{
...
bool CXMLBase::x_FindName( CXMLBase::TokenPos& token )
{
...
CString CXMLBase::x_GetToken( const CXMLBase::TokenPos& token )
{
...
int CXMLBase::x_FindElem( int iPosParent, int iPos, LPCTSTR szPath ) const
{
...
int CXMLBase::x_ParseNode( CXMLBase::TokenPos& token, CXMLBase::NodePos& node )
{
...
CString CXMLBase::x_GetPath( int iPos ) const
{
	CString strPath;
	while ( iPos )
	{
...
CString CXMLBase::x_GetTagName( int iPos ) const
{
...
bool CXMLBase::x_FindAttrib( CXMLBase::TokenPos& token, LPCTSTR szAttrib, int n/*=0*/ )
{
...
CString CXMLBase::x_GetAttrib( int iPos, LPCTSTR szAttrib ) const
{
...
bool CXMLBase::x_SetAttrib( int iPos, LPCTSTR szAttrib, int nValue )
{
...
bool CXMLBase::x_SetAttrib( int iPos, LPCTSTR szAttrib, LPCTSTR szValue )
{
...
bool CXMLBase::x_CreateNode( CString& strNode, int nNodeType, LPCTSTR szText )
{
...
CString CXMLBase::x_EncodeCDATASection( LPCTSTR szData )
{
...
bool CXMLBase::x_SetData( int iPos, int nValue )
{
	// Convert integer to string
	_TCHAR szVal[25];
	_stprintf( szVal, _T("%d"), nValue );
...
bool CXMLBase::x_SetData( int iPos, LPCTSTR szData, int nFlags )
{
...
CString CXMLBase::x_GetData( int iPos ) const
{
	if ( iPos == m_iPos && m_nNodeLength )
	{
...
CString CXMLBase::x_GetElemContent( int iPos ) const
{
...
bool CXMLBase::x_SetElemContent( LPCTSTR szContent )
{
...
void CXMLBase::x_DocChange( int nLeft, int nReplace, const CString& strInsert )
{
...
void CXMLBase::x_Adjust( int iPos, int nShift, bool bAfterPos /*=false*/ )
{
...
int CXMLBase::x_InsertNew( int iPosParent, int& iPosRel, CXMLBase::NodePos& node )
{
...
bool CXMLBase::x_AddElem( LPCTSTR szName, int nValue, int nFlags )
{
	// Convert integer to string
	_TCHAR szVal[25];
	_stprintf( szVal, _T("%d"), nValue );
...
bool CXMLBase::x_AddElem( LPCTSTR szName, LPCTSTR szValue, int nFlags )
{
	if ( nFlags & MNF_CHILD )
	{
...
CString CXMLBase::x_GetSubDoc( int iPos ) const
{
	if ( iPos )
	{
...
bool CXMLBase::x_AddSubDoc( LPCTSTR szSubDoc, int nFlags )
{
...
int CXMLBase::x_RemoveElem( int iPos )
{
...
void CXMLBase::x_LinkElem( int iPosParent, int iPosBefore, int iPos )
{
...
int CXMLBase::x_UnlinkElem( int iPos )
{
...
int CXMLBase::x_ReleasePos( int iPos )
{
...
int CXMLBase::x_ReleaseSubDoc( int iPos )
{
...
void CXMLBase::x_AdjustForNode( int iPosParent, int iPos, int nShift )
{
...
bool CXMLBase::x_AddNode( int nNodeType, LPCTSTR szText, int nFlags )
{
...
void CXMLBase::x_RemoveNode( int iPosParent, int& iPos, int& nNodeType, int& nNodeOffset, int& nNodeLength )
{
...
bool CXMLMakeUp::Load( LPCTSTR szFileName )
{
	if ( ! ReadTextFile(szFileName, &m_strError, &m_nFlags) )
		return false;

	DecryptionString();
	//return x_ParseDoc();
	return Load( m_byDoc, m_ilength );
}

bool CXMLMakeUp::Load(byte *byData, int ilength)
{
	bool bSuccess = false;
	m_strDoc.Format("%s", byData );
	m_strError.Format( _T("%d bytes"), ilength );
	m_nFlags = 0;
	bSuccess = x_ParseDoc();
	return bSuccess;
}

bool CXMLMakeUp::EncryptionString()
{
	bool bSuccess = false;
	return bSuccess;
}

bool CXMLMakeUp::DecryptionString()
{
	bool bSuccess = false;
	const int KEY = 239;
	byte * byTemp = m_byDoc;
	...
	{
		m_byDoc[i] = m_byDoc[i] ^ KEY;
	}

//	__asm
//	{
//		pushad
//			mov	ecx, m_ilength		; loop counter
//			mov	esi,0				; index 0 in buffer
//L1:
//		mov eax, DWORD ptr [byTemp]
//		xor	eax, 0x23			; translate a byte
//			mov byTemp, eax
...
bool CXMLMakeUp::ReadTextFile( LPCTSTR szFileName,  CString* pstrError, int* pnFlags )
{
	// Static utility method to load text file into strDoc
	//
	// Open file to read binary
	...
	m_byDoc = new byte[nFileByteLen + 1];
	m_ilength = nFileByteLen;
	fseek( fp, 0, SEEK_SET );

	if ( nFileByteLen )
	{
		fread( m_byDoc, nFileByteLen, 1, fp );
	}
	if ( pstrError )
		(*pstrError).Format( _T("%s%d bytes"), szDescBOM, nFileByteLen );
