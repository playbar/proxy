  ...
  cx_try
  {
	...
	*szfmt = '\0';
	...
	if (m_stImgInfo.nEscape == -1){
		m_bmpHeadInfo.biWidth = w;
		m_bmpHeadInfo.biHeight= h;
		m_stImgInfo.dwType = fmt<0 ? 0 : fmt;
	...
	if (m_stImgInfo.nEscape == -1 && fmt>0){
		error = 0;
	} else {
		error = 1;
	}
...
bool CxImageJAS::Encode(CxFile * hFile, DWORD imagetype)
{
	...
	if (m_bmpHeadInfo.biClrUsed!=0 && !IsGrayScale()){
		strcpy(m_stImgInfo.szLastError,"JasPer can save only RGB or GrayScale images");
		return false;
	}
  ...
  cx_try {
	...
	numcmpts = m_bmpHeadInfo.biClrUsed==0 ? 3 : 1;
	...
	for (cmptno = 0, cmptparm = cmptparms; cmptno < numcmpts; ++cmptno, ++cmptparm) {
		...
		cmptparm->hstep = 1;
		cmptparm->vstep = 1;
		cmptparm->width = m_bmpHeadInfo.biWidth;
		cmptparm->height = m_bmpHeadInfo.biHeight;
		cmptparm->prec = 8;
		cmptparm->sgnd = false;
	}
	...
	for (x = 0; x < numcmpts; ++x) {
		cmpts[x] = jas_matrix_create(1, m_bmpHeadInfo.biWidth);
		if (!cmpts[x]) {
	...
	for (y = 0; y < m_bmpHeadInfo.biHeight; ++y) {
		...
		yflip = m_bmpHeadInfo.biHeight - 1 - y;
		for (cmptno = 0; cmptno < numcmpts; ++cmptno) {
			if (jas_image_writecmpt(image, cmptno, 0, yflip, m_bmpHeadInfo.biWidth, 1, cmpts[cmptno])) {
	...
	*szfmt = '\0';
	...
	if (imagetype == CXIMAGE_FORMAT_PGX){
		strcpy(szfmt,"pgx");
		if (m_bmpHeadInfo.biClrUsed==0) cx_throw("PGX can save only GrayScale images");
	}
#endif
	int outfmt = jas_image_strtofmt(szfmt);

	char szoutopts[32];
	sprintf(szoutopts,"rate=%.3f", m_stImgInfo.fQuality/100.0f);
