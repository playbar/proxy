...
bool CxImageJBG::Decode(CxFile *hFile)
{
  ...
  cx_try
  {
	...
	if (m_stImgInfo.nEscape == -1){
		...
		m_stImgInfo.dwType = CXIMAGE_FORMAT_JBG;
	...
	if (m_stImgInfo.nEscape == -1 && m_stImgInfo.dwType == CXIMAGE_FORMAT_JBG) return true;
	return false;
  }
	return true;
}
...
bool CxImageJBG::Encode(CxFile * hFile)
{
	...
	if (m_bmpHeadInfo.biBitCount != 1){
		strcpy(m_stImgInfo.szLastError,"JBG can save only 1-bpp images");
		return false;
	}

	int w, h, bpp, planes, ew, i, j, x, y;

	w = m_bmpHeadInfo.biWidth;
	h = m_bmpHeadInfo.biHeight;
	planes = 1;
	bpp = (planes+7)>>3;
	ew = (w + 7)>>3;
	...
	if (!buffer) {
		strcpy(m_stImgInfo.szLastError,"Sorry, not enough memory available!");
		return false;
	}

	for (y=0; y<h; y++){
		i= y*ew;
		j= (h-y-1)*m_stImgInfo.dwEffWidth;
		for (x=0; x<ew; x++){
			buffer[i + x]=m_stImgInfo.pImage[j + x]^mask;
		}
	}
	...
	if (hFile->Error()){
		strcpy(m_stImgInfo.szLastError,"Problem while writing JBG file");
		return false;
	}
