...
bool CxImageJPG::Decode(CxFile * hFile)
{
	...
	*/
	...
	jerr.buffer=m_stImgInfo.szLastError;
	/* More stuff */
 ...
 */ //</DP>
	...
	if (m_stImgInfo.nEscape == -1) {
		...
		m_bmpHeadInfo.biWidth = cinfo.output_width;
		m_bmpHeadInfo.biHeight = cinfo.output_height;
		m_stImgInfo.dwType = CXIMAGE_FORMAT_JPG;
		jpeg_destroy_decompress(&cinfo);
		return true;
	}
	...
	*/
	...
	if (!m_pDib) longjmp(jerr.setjmp_buffer, 1);  //<DP> check if the image has been created
	...
	if (cinfo.out_color_space==JCS_GRAYSCALE){
		...
		if (cinfo.quantize_colors){
			...
			m_bmpHeadInfo.biClrUsed=cinfo.actual_number_of_colors;
		} else {
	...
	*/
	...
	while (cinfo.output_scanline < cinfo.output_height) {
		...
		if (m_stImgInfo.nEscape) longjmp(jerr.setjmp_buffer, 1); // <vho> - cancel decoding
		...
		if ((cinfo.num_components==4)&&(cinfo.quantize_colors==FALSE)){
			...
			dst=iter.GetRow();
			src=buffer[0];
			for(long x3=0,x4=0; x3<(long)m_stImgInfo.dwEffWidth && x4<row_stride; x3+=3, x4+=4){
				k=src[x4+3];
	...
	*/
	...
	if ((cinfo.num_components==3)&&(cinfo.quantize_colors==FALSE)){
		...
		for(long y=0;y<m_bmpHeadInfo.biHeight;y++){
			if (m_stImgInfo.nEscape) longjmp(jerr.setjmp_buffer, 1); // <vho> - cancel decoding
			RGBtoBGR(r0,3*m_bmpHeadInfo.biWidth);
			r0+=m_stImgInfo.dwEffWidth;
		}
	}
...
bool CxImageJPG::Encode(CxFile * hFile)
{
	...
	if (m_bmpHeadInfo.biClrUsed!=0 && !IsGrayScale()){
		strcpy(m_stImgInfo.szLastError,"JPEG can save only RGB or GreyScale images");
		return false;
	}	
	...
	*/
	...
	if (setjmp(jerr.setjmp_buffer)) {
		...
		* We need to clean up the JPEG object, close the input file, and return.
		*/
		strcpy(m_stImgInfo.szLastError, jerr.buffer); //<CSC>
...
//#endif
	...
	*/
	row_stride = m_stImgInfo.dwEffWidth;	/* JSAMPLEs per row in image_buffer */
	...
	while (cinfo.next_scanline < cinfo.image_height) {
		...
		// not necessary if swapped red and blue definition in jmorecfg.h;ln322 <W. Morrison>
		if (m_bmpHeadInfo.biClrUsed==0){				 // swap R & B for RGB images
