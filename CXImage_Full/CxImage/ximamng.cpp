...
bool CxImageMNG::Decode(CxFile *hFile)
{
	...
	cx_try
	{
		...
		if (m_stImgInfo.nEscape == -1) {
			...
			m_bmpHeadInfo.biWidth = hmng->iWidth;
			m_bmpHeadInfo.biHeight = hmng->iHeight;
			m_stImgInfo.dwType = CXIMAGE_FORMAT_MNG;
			return true;
		}
		...
		if (m_stImgInfo.nNumFrames<=0) m_stImgInfo.nNumFrames=1;

		if (mnginfo.animation_enabled==0){
			// select the frame
			if (m_stImgInfo.nFrame>=0 && m_stImgInfo.nFrame<m_stImgInfo.nNumFrames){
				for (int n=0;n<m_stImgInfo.nFrame;n++) mng_display_resume(hmng);
			} else cx_throw("Error: frame not present in MNG file");
		}

		if (mnginfo.nBkgndIndex >= 0){
			m_stImgInfo.nBkgndIndex = mnginfo.nBkgndIndex;
			m_stImgInfo.nBkgndColor.rgbRed = mnginfo.nBkgndColor.rgbRed;
			m_stImgInfo.nBkgndColor.rgbGreen = mnginfo.nBkgndColor.rgbGreen;
			m_stImgInfo.nBkgndColor.rgbBlue = mnginfo.nBkgndColor.rgbBlue;
		}

		//store the newly created image
		if (Create(mnginfo.width,mnginfo.height,mnginfo.bpp, CXIMAGE_FORMAT_MNG)){
			memcpy(GetBits(), mnginfo.image, m_stImgInfo.dwEffWidth * m_bmpHeadInfo.biHeight);
#if CXIMAGE_SUPPORT_ALPHA
			SwapRGB2BGR();
			AlphaCreate();
...
bool CxImageMNG::Encode(CxFile *hFile)
{
	...
	cx_try
	{
		if (m_bmpHeadInfo.biClrUsed != 0) cx_throw("MNG encoder can save only RGB images");
		// set the file we want to play
		mnginfo.file = hFile;
		mnginfo.bpp = m_bmpHeadInfo.biBitCount;
		mnginfo.effwdt = m_stImgInfo.dwEffWidth;
		mnginfo.height = m_bmpHeadInfo.biHeight;
		mnginfo.width =  m_bmpHeadInfo.biWidth;

		mnginfo.image = (BYTE*)malloc(m_bmpHeadInfo.biSizeImage);
		if (mnginfo.image == NULL) cx_throw("could not allocate memory for MNG");
		memcpy(mnginfo.image,m_stImgInfo.pImage, m_bmpHeadInfo.biSizeImage);
