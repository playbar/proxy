...
bool CxImageSKA::Decode(CxFile *hFile)
{
	...
	if (m_stImgInfo.nEscape == -1){
		m_bmpHeadInfo.biWidth = ska_header.Width ;
		m_bmpHeadInfo.biHeight= ska_header.Height;
		m_stImgInfo.dwType = CXIMAGE_FORMAT_SKA;
		return true;
	}
...
bool CxImageSKA::Encode(CxFile * hFile)
{
	...
	if(m_bmpHeadInfo.biBitCount > 8)	{
		strcpy(m_stImgInfo.szLastError,"SKA Images must be 8 bit or less");
		return false;
	}
	...
	if (m_bmpHeadInfo.biBitCount<8) IncreaseBpp(8);
