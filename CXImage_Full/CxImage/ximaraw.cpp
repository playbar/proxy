...
bool CxImageRAW::Decode(CxFile *hFile)
{
  ...
  cx_try
  {
	...
	dcr.ifname = szClass;
	dcr.sz_error = m_stImgInfo.szLastError;
	...
	if (m_stImgInfo.nEscape == -1){
		m_bmpHeadInfo.biWidth = dcr.width;
		m_bmpHeadInfo.biHeight= dcr.height;
		m_stImgInfo.dwType = CXIMAGE_FORMAT_RAW;
	...
	if (m_stImgInfo.nEscape == -1 && m_stImgInfo.dwType == CXIMAGE_FORMAT_RAW) return true;
	return false;
  }
	/* that's it */
	return true;
...
bool CxImageRAW::Encode(CxFile * hFile)
{
	...
	strcpy(m_stImgInfo.szLastError, "Save RAW not supported");
	return false;
}
////////////////////////////////////////////////////////////////////////////////
