...
bool CxImageTIF::Decode(CxFile * hFile)
{
  ...
  cx_try
  {
	...
	m_stImgInfo.nNumFrames = TIFFNumberOfDirectories(m_tif);

	if (!TIFFSetDirectory(m_tif, (uint16)m_stImgInfo.nFrame))
	...
	if (m_stImgInfo.nEscape == -1) {
		...
		m_bmpHeadInfo.biWidth = width;
		m_bmpHeadInfo.biHeight = height;
		m_stImgInfo.dwType = CXIMAGE_FORMAT_TIF;
	...
	if (TIFFGetField(m_tif, TIFFTAG_XPOSITION, &offset))	m_stImgInfo.xOffset = (long)offset;
	if (TIFFGetField(m_tif, TIFFTAG_YPOSITION, &offset))	m_stImgInfo.yOffset = (long)offset;
	...
	if (isRGB){
		m_bmpHeadInfo.biBitCount=24;
	}else{
		if ((photometric==PHOTOMETRIC_MINISBLACK)||(photometric==PHOTOMETRIC_MINISWHITE)||(photometric==PHOTOMETRIC_PALETTE)){
			if	(bitspersample == 1){
				m_bmpHeadInfo.biBitCount=1;		//B&W image
				m_bmpHeadInfo.biClrUsed =2;
			} else if (bitspersample == 4) {
				m_bmpHeadInfo.biBitCount=4;		//16 colors gray scale
				m_bmpHeadInfo.biClrUsed =16;
			} else {
				m_bmpHeadInfo.biBitCount=8;		//gray scale
				m_bmpHeadInfo.biClrUsed =256;
			}
		} else if (bitspersample == 4) {
			m_bmpHeadInfo.biBitCount=4;			// 16 colors
			m_bmpHeadInfo.biClrUsed=16;
		} else {
			m_bmpHeadInfo.biBitCount=8;			//256 colors
			m_bmpHeadInfo.biClrUsed=256;
		}
	...
	if (m_stImgInfo.nEscape) cx_throw("Cancelled"); // <vho> - cancel decoding

	Create(width,height,m_bmpHeadInfo.biBitCount,CXIMAGE_FORMAT_TIF);	//image creation
	if (!m_pDib) cx_throw("CxImageTIF can't create image");
	...
	if (isRGB) {
		...
		row = &raster[0];
		bits2 = m_stImgInfo.pImage;
		for (y = 0; y < height; y++) {

			if (m_stImgInfo.nEscape){ // <vho> - cancel decoding
				_TIFFfree(raster);
				cx_throw("Cancelled");
			}
		...
		if (bitsize<(long)(m_bmpHeadInfo.biSizeImage*samplesperpixel))
			bitsize = m_bmpHeadInfo.biSizeImage*samplesperpixel;
			...
			for (y = 0; y < nrow; y++) {
				...
				if (samplesperpixel==1) { 
					if (BIG_palette)
						if (bits16) {
							long offset16 = (nrow-y-1)*line16;		// + VK
							MoveBitsPal( m_stImgInfo.pImage + m_stImgInfo.dwEffWidth * (height-ys-nrow+y),
									 bits16 + offset16, width, bitspersample, pal );
						} else
							MoveBitsPal( m_stImgInfo.pImage + m_stImgInfo.dwEffWidth * (height-ys-nrow+y),
									 bits + offset, width, bitspersample, pal );
					else if ((bitspersample == m_bmpHeadInfo.biBitCount) || 
						(bitspersample == 16))	//simple 8bpp, 4bpp image or 16bpp
						memcpy(m_stImgInfo.pImage+m_stImgInfo.dwEffWidth*(height-ys-nrow+y),bits+offset,m_stImgInfo.dwEffWidth);
					else
						MoveBits( m_stImgInfo.pImage + m_stImgInfo.dwEffWidth * (height-ys-nrow+y),
					...
					if (m_bmpHeadInfo.biBitCount!=24){ //fix image
		...
		if (row_shifts && (samplesperpixel == 1) && (bitspersample==16) && !BIG_palette) {
			...
			for( y=0; y<height; y++ ) {
				if (min_row_shift < row_shifts[y]) {
					...
					BYTE* data = m_stImgInfo.pImage + m_stImgInfo.dwEffWidth * y;
	  ...
	  if (m_stImgInfo.nEscape == -1 && m_stImgInfo.dwType == CXIMAGE_FORMAT_TIF) return true;
	  return false;
  }
	TIFFClose(m_tif);
	return true;
...
bool CxImageTIF::Encode(CxFile * hFile, bool bAppend)
{
  cx_try
  {
	...
	if (m_pDib==NULL) cx_throw(CXIMAGE_ERR_NOIMAGE);
...
{
	uint32 height=m_bmpHeadInfo.biHeight;
	uint32 width=m_bmpHeadInfo.biWidth;
	uint16 bitcount=m_bmpHeadInfo.biBitCount;
...
//	int line;
	...
	switch (bitcount) {
		...
		case 8:
			...
			for (x = 0; x < m_bmpHeadInfo.biClrUsed; x++) {
...
//	pitch = (uint16)CalculatePitch(line);
	...
	if (GetPalette()){
		...
		for(WORD a=0;a<m_bmpHeadInfo.biClrUsed;a++){	//swap blue and red components
	...
	TIFFSetField(m_tif, TIFFTAG_XRESOLUTION, (float)m_stImgInfo.xDPI);
	TIFFSetField(m_tif, TIFFTAG_YRESOLUTION, (float)m_stImgInfo.yDPI);
...
//	TIFFSetField(m_tif, TIFFTAG_YPOSITION, (float)info.yOffset);
	...
	switch(bitcount) {				
		...
		{
			if (samplesperpixel==1){
				bits = (BYTE*)malloc(m_stImgInfo.dwEffWidth);
				if (!bits) return false;
				for (y = 0; y < height; y++) {
					memcpy(bits,m_stImgInfo.pImage + (height - y - 1)*m_stImgInfo.dwEffWidth,m_stImgInfo.dwEffWidth);
		...
		case 24:
		{
			BYTE *buffer = (BYTE *)malloc(m_stImgInfo.dwEffWidth);
			if (!buffer) return false;
			for (y = 0; y < height; y++) {
				// get a pointer to the scanline
				memcpy(buffer, m_stImgInfo.pImage + (height - y - 1)*m_stImgInfo.dwEffWidth, m_stImgInfo.dwEffWidth);
		...
		{
#if CXIMAGE_SUPPORT_ALPHA
			BYTE *buffer = (BYTE *)malloc((m_stImgInfo.dwEffWidth*4)/3);
			if (!buffer) return false;
			for (y = 0; y < height; y++) {
				// get a pointer to the scanline
				memcpy(buffer, m_stImgInfo.pImage + (height - y - 1)*m_stImgInfo.dwEffWidth, m_stImgInfo.dwEffWidth);
