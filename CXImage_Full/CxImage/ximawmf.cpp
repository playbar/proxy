...
/////////////////////////////////////////////////////////////////////
		...
		if (!hMeta){
			strcpy(m_stImgInfo.szLastError,"corrupted WMF");
	...
	if (m_stImgInfo.nEscape == -1) {	// Check if cancelled
		m_bmpHeadInfo.biWidth = cx;
		m_bmpHeadInfo.biHeight= cy;
		m_stImgInfo.dwType = CXIMAGE_FORMAT_WMF;
		DeleteEnhMetaFile(hMeta);
		strcpy(m_stImgInfo.szLastError,"output dimensions returned");
		return true;
	}

	if (!cx || !cy)	{
		DeleteEnhMetaFile(hMeta);
		strcpy(m_stImgInfo.szLastError,"empty WMF");
		return false;
	}
		...
		if (hBitmap){
			...
			if (iEntries) { 
				if ((plogPal = (PLOGPALETTE)GlobalAlloc(GMEM_FIXED | GMEM_ZEROINIT, 
					...
					DeleteDC(hDC);
					DeleteEnhMetaFile(hMeta);
					strcpy(m_stImgInfo.szLastError,"Cancelled");
					return false;
				} 
			...
			if (m_stImgInfo.nEscape) {	// Check if cancelled
				DeleteObject(hBitmap);
				DeleteDC(hDC);
				strcpy(m_stImgInfo.szLastError,"Cancelled");
				return false;
			...
			bRet = GetDIBits(hDC, hBitmap, 0,
			        (UINT)cy, GetBits(), (LPBITMAPINFO)m_pDib, DIB_RGB_COLORS);
...
{
	if (hFile == NULL) return false;
	strcpy(m_stImgInfo.szLastError, "Save WMF not supported");
	return false;
}
