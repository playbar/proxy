...
void CxImage::OverflowCoordinates(long &x, long &y, OverflowMethod const ofMethod)
{
  ...
  switch (ofMethod) {
    case OM_REPEAT:
      //clip coordinates
      x=max(x,0); x=min(x, m_bmpHeadInfo.biWidth-1);
      y=max(y,0); y=min(y, m_bmpHeadInfo.biHeight-1);
      break;
    case OM_WRAP:
      //wrap coordinates
      x = x % m_bmpHeadInfo.biWidth;
      y = y % m_bmpHeadInfo.biHeight;
      if (x<0) x = m_bmpHeadInfo.biWidth + x;
      if (y<0) y = m_bmpHeadInfo.biHeight + y;
      break;
    case OM_MIRROR:
      //mirror pixels near border
      if (x<0) x=((-x) % m_bmpHeadInfo.biWidth);
      else if (x>=m_bmpHeadInfo.biWidth) x=m_bmpHeadInfo.biWidth-(x % m_bmpHeadInfo.biWidth + 1);
      if (y<0) y=((-y) % m_bmpHeadInfo.biHeight);
      else if (y>=m_bmpHeadInfo.biHeight) y=m_bmpHeadInfo.biHeight-(y % m_bmpHeadInfo.biHeight + 1);
      break;
    default:
      return;
  }//switch
...
{
  if (x>=0 && x<m_bmpHeadInfo.biWidth && y>=0 && y<m_bmpHeadInfo.biHeight) return;  //if pixel is within bounds, no change
  switch (ofMethod) {
    case OM_REPEAT:
      //clip coordinates
      x=max(x,0); x=min(x, m_bmpHeadInfo.biWidth-1);
    ...
    case OM_WRAP:
      //wrap coordinates
      x = (float)fmod(x, (float) m_bmpHeadInfo.biWidth);
      y = (float)fmod(y, (float) m_bmpHeadInfo.biHeight);
      if (x<0) x = m_bmpHeadInfo.biWidth + x;
      if (y<0) y = m_bmpHeadInfo.biHeight + y;
      break;
    case OM_MIRROR:
      //mirror pixels near border
      if (x<0) x=(float)fmod(-x, (float) m_bmpHeadInfo.biWidth);
      else if (x>=m_bmpHeadInfo.biWidth) x=m_bmpHeadInfo.biWidth-((float)fmod(x, (float) m_bmpHeadInfo.biWidth) + 1);
      if (y<0) y=(float)fmod(-y, (float) m_bmpHeadInfo.biHeight);
      else if (y>=m_bmpHeadInfo.biHeight) y=m_bmpHeadInfo.biHeight-((float)fmod(y, (float) m_bmpHeadInfo.biHeight) + 1);
      break;
    default:
      return;
  }//switch
...
RGBQUAD CxImage::GetPixelColorWithOverflow(long x, long y, OverflowMethod const ofMethod, RGBQUAD* const rplColor)
{
  ...
  if ((!IsInside(x,y)) || m_pDib==NULL) {     //is pixel within bouns?:
    ...
    switch (ofMethod) {
      ...
      case OM_BACKGROUND:
		  ...
		  if (m_stImgInfo.nBkgndIndex >= 0) {
			  if (m_bmpHeadInfo.biBitCount<24) color = GetPaletteColor((BYTE)m_stImgInfo.nBkgndIndex);
			  else color = m_stImgInfo.nBkgndColor;
		  }//if
		  return color;
      case OM_REPEAT:
      case OM_WRAP:
  ...
  RGBQUAD* const rplColor)
{
  ...
  switch (inMethod) {
    ...
    default: {
      ...
      if (xi<-1 || xi>=m_bmpHeadInfo.biWidth || yi<-1 || yi>=m_bmpHeadInfo.biHeight) {  //all 4 points are outside bounds?:
        switch (ofMethod) {
      ...
      if ((xi+1)<m_bmpHeadInfo.biWidth && xi>=0 && (yi+1)<m_bmpHeadInfo.biHeight && yi>=0 && m_bmpHeadInfo.biClrUsed==0) {
        ...
        WORD wa=256-wt1-wc;
        WORD wrr,wgg,wbb;
        BYTE *pxptr=(BYTE*)m_stImgInfo.pImage+yi*m_stImgInfo.dwEffWidth+xi*3;
        wbb=wa*(*pxptr++); wgg=wa*(*pxptr++); wrr=wa*(*pxptr++);
        wbb+=wb*(*pxptr++); wgg+=wb*(*pxptr++); wrr+=wb*(*pxptr);
        pxptr+=(m_stImgInfo.dwEffWidth-5); //move to next row
        ...
        if (m_pAlpha) {
          ...
          pxptr+=(m_bmpHeadInfo.biWidth-1);                                //move to next row
	...
	case IM_POWER:
      ...
      if (((xi+2)<0) || ((xi-1)>=m_bmpHeadInfo.biWidth) || ((yi+2)<0) || ((yi-1)>=m_bmpHeadInfo.biHeight)) { //all points are outside bounds?:
        switch (ofMethod) {
          case OM_COLOR: case OM_TRANSPARENT: case OM_BACKGROUND:
            //we don't need to interpolate anything with all points outside in this case
            return GetPixelColorWithOverflow(-999, -999, ofMethod, rplColor);
      ...
      rr=gg=bb=aa=0;
      if (((xi+2)<m_bmpHeadInfo.biWidth) && xi>=1 && ((yi+2)<m_bmpHeadInfo.biHeight) && (yi>=1) && !IsIndexed()) {
    ...
    case IM_LANCZOS:
      ...
      if (((xi+6)<0) || ((xi-5)>=m_bmpHeadInfo.biWidth) || ((yi+6)<0) || ((yi-5)>=m_bmpHeadInfo.biHeight)) {
      ...
      if (((xi+6)<m_bmpHeadInfo.biWidth) && ((xi-5)>=0) && ((yi+6)<m_bmpHeadInfo.biHeight) && ((yi-5)>=0) && !IsIndexed()) {
        //optimized interpolation (faster pixel reads) for RGB24 images with all pixels inside bounds
        BYTE *pxptr, *pxptra;
