...
bool CxImageGIF::Decode(CxFile *fp)
{
	...
	if (m_stImgInfo.nEscape == -1) {
		// Return output dimensions only
		m_bmpHeadInfo.biWidth = dscgif.scrwidth;
		m_bmpHeadInfo.biHeight = dscgif.scrheight;
		m_stImgInfo.dwType = CXIMAGE_FORMAT_GIF;
		return true;
	}
	...
	int iImage = 0;
	m_stImgInfo.nNumFrames=get_num_frames(fp,&TabCol,&dscgif);

	if ((m_stImgInfo.nFrame<0)||(m_stImgInfo.nFrame>=m_stImgInfo.nNumFrames)) return false;

	//it cannot be a true color GIF with only one frame
	if (m_stImgInfo.nNumFrames == 1)
		bTrueColor=0;
	...
	for (BOOL bContinue = TRUE; bContinue; )
	{
		...
		if (m_stImgInfo.nEscape > 0) return false; // <vho> - cancel decoding
		if (bPreviousWasNull || ch==0)
		{
			...
			case ',': // image
				...
				if (iImage==0){
					...
					first_transparent_index = m_stImgInfo.nBkgndIndex;
				...
				*/
				...
				if ((image.pf & 0x80) || (dscgif.pflds & 0x80)) {
					...
					SetPalette((m_stImgInfo.nBkgndIndex >= 0 ? 256 : palcount), r, g, b);
				}
				...
				if (m_stImgInfo.nEscape) return false; // <vho> - cancel decoding
				...
				if (bTrueColor<2 ){ //standard GIF: mix frame with background
					...
					m_stImgInfo.nFrame=m_stImgInfo.nNumFrames-1;
					//build the RGB image
				...
				if (m_stImgInfo.bGetAllFrames && imaRGB == NULL) {
					if (iImage == 0) {
						DestroyFrames();
						m_ppFrames = new CxImage*[m_stImgInfo.nNumFrames];
						for(int frameIdx = 0; frameIdx < m_stImgInfo.nNumFrames; frameIdx++){
							m_ppFrames[frameIdx] = NULL;
						}
					}
					m_ppFrames[iImage] = new CxImage(*this);
					m_ppFrames[iImage]->SetRetreiveAllFrames(false);
				}
				if (prevdispmeth <= 1) {
				...
				if (m_stImgInfo.nFrame==iImage) bContinue=false; else iImage++;
	...
	if (bTrueColor>=2 && imaRGB){
		if (gifgce.flags & 0x1){
			imaRGB->SetTransColor(GetPaletteColor((BYTE)m_stImgInfo.nBkgndIndex));
	...
	if (bContinue) {
		...
		if (fc == 0xF9)	{
			...
			if (bContinue) {
				...
				if (bContinue) {
					m_stImgInfo.nBkgndIndex  = (gifgce.flags & 0x1) ? gifgce.transpcolindex : -1;
					m_stImgInfo.dwFrameDelay = gifgce.delaytime;
...
int CxImageGIF::out_line(CImageIterator* iter, unsigned char *pixels, int linelen)
{
	...
	if (m_bmpHeadInfo.biBitCount < 8){
		for(long x=0;x<m_bmpHeadInfo.biWidth;x++){
			BYTE pos;
			BYTE* iDst= pixels + (x*m_bmpHeadInfo.biBitCount >> 3);
...
bool CxImageGIF::Encode(CxFile * fp)
{
	...
	if ( GetNumFrames()>1 && m_ppFrames ) {
		return Encode(fp, m_ppFrames, GetNumFrames() );
	}
...
bool CxImageGIF::Encode(CxFile * fp, CxImage ** pImages, int pagecount, bool bLocalColorMap, bool bLocalDispMeth)
{
  cx_try {
	...
	fp->PutC(';'); // Write the GIF file terminator
	  ...
	  if (strcmp(message,"")) strncpy(m_stImgInfo.szLastError,message,255);
	  return false;
  }
	return true;
}
...
void CxImageGIF::EncodeHeader(CxFile *fp)
{
	...
	Putword(m_bmpHeadInfo.biWidth,fp);			   //Logical screen descriptor
	Putword(m_bmpHeadInfo.biHeight,fp);
	...
	if (m_bmpHeadInfo.biClrUsed==0){
		...
		Flags = 0x80;
		Flags |=(m_bmpHeadInfo.biBitCount - 1) << 5;
		Flags |=(m_bmpHeadInfo.biBitCount - 1);
	}
...
void CxImageGIF::EncodeExtension(CxFile *fp)
{
	...
	gifgce.flags = 0;
	gifgce.flags |= ((m_stImgInfo.nBkgndIndex != -1) ? 1 : 0);
	gifgce.flags |= ((GetDisposalMethod() & 0x7) << 2);
	gifgce.delaytime = (WORD)m_stImgInfo.dwFrameDelay;
	gifgce.transpcolindex = (BYTE)m_stImgInfo.nBkgndIndex;	   
...
{
	curx = 0;
	cury = m_bmpHeadInfo.biHeight - 1;	//because we read the image bottom to top
	CountDown = (long)m_bmpHeadInfo.biWidth * (long)m_bmpHeadInfo.biHeight;

	fp->PutC(',');

	Putword(m_stImgInfo.xOffset,fp);
	Putword(m_stImgInfo.yOffset,fp);
	Putword(m_bmpHeadInfo.biWidth,fp);
	Putword(m_bmpHeadInfo.biHeight,fp);

	BYTE Flags=0x00; //non-interlaced (0x40 = interlaced) (0x80 = LocalColorMap)
	if (bLocalColorMap)	{ Flags|=0x80; Flags|=m_bmpHeadInfo.biBitCount-1; }
	fp->PutC(Flags);
	...
	int InitCodeSize = m_bmpHeadInfo.biBitCount <=1 ? 2 : m_bmpHeadInfo.biBitCount;
...
//	EncodeLoopExtension(fp);
	...
	const long cellh = 15;
	CxImageGIF tmp;
	for (long y=0;y<m_bmpHeadInfo.biHeight;y+=cellh){
		for (long x=0;x<m_bmpHeadInfo.biWidth;x+=cellw){
			if ((m_bmpHeadInfo.biWidth -x)<cellw) w=m_bmpHeadInfo.biWidth -x; else w=cellw;
			if ((m_bmpHeadInfo.biHeight-y)<cellh) h=m_bmpHeadInfo.biHeight-y; else h=cellh;
			...
			for (unsigned long j=0;j<h;j++){
				for (unsigned long k=0;k<w;k++){
					i=(BYTE)(1+k+cellw*j);
					tmp.SetPaletteColor(i,GetPixelColor(x+k,m_bmpHeadInfo.biHeight-y-h+j));
...
void CxImageGIF::output( code_int  code)
{
	...
	if( code == EOFCode ) {
		...
		if(g_outfile->Error()) strcpy(m_stImgInfo.szLastError,"Write Error in GIF file");
	}
}
////////////////////////////////////////////////////////////////////////////////
