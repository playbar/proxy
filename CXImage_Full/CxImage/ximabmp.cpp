...
bool CxImageBMP::Encode(CxFile * hFile)
{
	...
	hdr.bfOffBits = 14 /*sizeof(BITMAPFILEHEADER)*/ + m_bmpHeadInfo.biSize + GetPaletteSize();
	...
	if (GetNumColors()==0 && AlphaIsValid())
	{
		...
		memcpy(&infohdr,&m_bmpHeadInfo,sizeof(BITMAPINFOHEADER));
...
#endif //CXIMAGE_SUPPORT_ALPHA
	{
		...
		hFile->Write(&hdr,min(14,sizeof(BITMAPFILEHEADER)),1);
		//copy attributes
		memcpy(m_pDib,&m_bmpHeadInfo,sizeof(BITMAPINFOHEADER));
		bihtoh((BITMAPINFOHEADER*)m_pDib);
		// Write the DIB header and the pixels
		hFile->Write(m_pDib,GetSize(),1);
		bihtoh((BITMAPINFOHEADER*)m_pDib);
	}
	return true;
}
...
bool CxImageBMP::Decode(CxFile * hFile)
{
  ...
  cx_try {
	...
	if (m_stImgInfo.nEscape == -1) {
		...
		m_bmpHeadInfo.biWidth = bmpHeader.biWidth;
		m_bmpHeadInfo.biHeight = bmpHeader.biHeight;
		m_stImgInfo.dwType = CXIMAGE_FORMAT_BMP;
    ...
    if (pRgb){
        if (bIsOldBmp){
            ...
            for (int i=DibNumColors(&m_bmpHeadInfo)-1; i>=0; i--){
			...
			for (unsigned int i=0; i<m_bmpHeadInfo.biClrUsed; i++) pRgb[i].rgbReserved=0;
        }
    }
	...
	switch (dwBitCount) {
		case 32 :
			...
			if (dwCompression == BI_BITFIELDS || dwCompression == BI_RGB){
				long imagesize=4*m_bmpHeadInfo.biHeight*m_bmpHeadInfo.biWidth;
					...
					if (dwCompression == BI_RGB){
						...
						if (AlphaIsValid()){
							...
							for (long y=0; y<m_bmpHeadInfo.biHeight; y++){
								p = buff32 + 3 + m_bmpHeadInfo.biWidth * 4 * y;
		...
		case 24 :
			if (bf.bfOffBits != 0L) hFile->Seek(off + bf.bfOffBits,SEEK_SET);
			if (dwCompression == BI_RGB){
				hFile->Read(m_stImgInfo.pImage, m_bmpHeadInfo.biSizeImage,1); // read in the pixels
			} else cx_throw("unknown compression");
			break;
		case 16 :
		{
			...
			if (bf.bfOffBits != 0L) hFile->Seek(off + bf.bfOffBits,SEEK_SET);
			// read in the pixels
			hFile->Read(m_stImgInfo.pImage, m_bmpHeadInfo.biHeight*((m_bmpHeadInfo.biWidth+1)/2)*4,1);
			// transform into RGB
			Bitfield2RGB(m_stImgInfo.pImage,bfmask[0],bfmask[1],bfmask[2],16);
			break;
		}
		case 8 :
		case 4 :
	...
	if (m_stImgInfo.nEscape == -1 && m_stImgInfo.dwType == CXIMAGE_FORMAT_BMP) return true;
	return false;
  }
    return true;
}
