...
{
	strcpy(m_stImgInfo.szLastError,message);
...
bool CxImagePNG::Decode(CxFile *hFile)
{
  ...
  cx_try
  {
    ...
    * set up your own error handlers in the png_create_read_struct() earlier. */
	...
	png_set_error_fn(png_ptr,m_stImgInfo.szLastError,/*(png_error_ptr)*/user_error_fn,NULL);

	/* read the file information */
	png_read_info(png_ptr, info_ptr);

	if (m_stImgInfo.nEscape == -1){
		m_bmpHeadInfo.biWidth = info_ptr->width;
		m_bmpHeadInfo.biHeight= info_ptr->height;
		m_stImgInfo.dwType = CXIMAGE_FORMAT_PNG;
		longjmp(png_ptr->jmpbuf, 1);
	}
	...
	case PNG_COLOR_TYPE_RGB_ALPHA:
		...
		break;
	default:
		strcpy(m_stImgInfo.szLastError,"unknown PNG color type");
		longjmp(png_ptr->jmpbuf, 1);
	}
	...
	if (info_ptr->num_trans!=0){ //palette transparency
		if (info_ptr->num_trans==1){
			if (info_ptr->color_type == PNG_COLOR_TYPE_PALETTE){
				m_stImgInfo.nBkgndIndex = info_ptr->trans_values.index;
			} else{
				m_stImgInfo.nBkgndIndex = info_ptr->trans_values.gray>>nshift;
			}
		}
		if (info_ptr->num_trans>1){
			...
			if (pal){
				DWORD ip;
				for (ip=0;ip<min(m_bmpHeadInfo.biClrUsed,(unsigned long)info_ptr->num_trans);ip++)
					pal[ip].rgbReserved=info_ptr->trans[ip];
				for (ip=info_ptr->num_trans;ip<m_bmpHeadInfo.biClrUsed;ip++){
					pal[ip].rgbReserved=255;
				}
				m_stImgInfo.bAlphaPaletteEnabled=true;
			}
		}
	}
	...
	if (channels == 3){ //check RGB binary transparency
		...
		if (png_get_tRNS(png_ptr, info_ptr, &trans, &num_trans, &image_background)){
			m_stImgInfo.nBkgndColor.rgbRed   = (BYTE)(info_ptr->trans_values.red>>nshift);
			m_stImgInfo.nBkgndColor.rgbGreen = (BYTE)(info_ptr->trans_values.green>>nshift);
			m_stImgInfo.nBkgndColor.rgbBlue  = (BYTE)(info_ptr->trans_values.blue>>nshift);
			m_stImgInfo.nBkgndColor.rgbReserved = 0;
	...
	// <vho> - handle cancel
	if (m_stImgInfo.nEscape) longjmp(png_ptr->jmpbuf, 1);
	...
	for (int pass=0; pass < number_passes; pass++) {
		...
		do	{
...
#endif // CXIMAGE_SUPPORT_ALPHA		// vho
			{
				...
				if (info_ptr->interlace_type && pass>0){
					...
					if (info_ptr->bit_depth > 8){
						for(long ax=(m_bmpHeadInfo.biWidth*channels-1);ax>=0;ax--)
				...
				if (info_ptr->bit_depth > 8){
					for(long ax=0;ax<(m_bmpHeadInfo.biWidth*channels);ax++)
			...
			y++;
		} while(y<m_bmpHeadInfo.biHeight);
	}
	...
	if (m_stImgInfo.nEscape == -1 && m_stImgInfo.dwType == CXIMAGE_FORMAT_PNG) return true;
	return false;
  }
	/* that's it */
	return true;
...
bool CxImagePNG::Encode(CxFile *hFile)
{
  ...
  cx_try
  {
    ...
    */
	...
	/* set metrics */
	png_set_pHYs(png_ptr, info_ptr, m_bmpHeadInfo.biXPelsPerMeter, m_bmpHeadInfo.biYPelsPerMeter, PNG_RESOLUTION_METER);
	...
	if (m_stImgInfo.nBkgndIndex >= 0){
		...
		info_ptr->trans_values.index = (BYTE)m_stImgInfo.nBkgndIndex;
		...
		if (!bGrayScale && m_bmpHeadInfo.biClrUsed && m_stImgInfo.nBkgndIndex)
			SwapIndex(0,(BYTE)m_stImgInfo.nBkgndIndex);
	}
	...
	if (AlphaIsValid() && m_bmpHeadInfo.biBitCount==24 && m_stImgInfo.nBkgndIndex>=0){
	...
	int row_size = max(m_stImgInfo.dwEffWidth, info_ptr->width*info_ptr->channels*(info_ptr->bit_depth/8));
	...
	for (int pass = 0; pass < num_pass; pass++){
		//write image
		iter.Upset();
		long ay=m_bmpHeadInfo.biHeight-1;
		RGBQUAD c;
		do	{
#if CXIMAGE_SUPPORT_ALPHA	// <vho>
			if (AlphaIsValid()){
				for (long ax=m_bmpHeadInfo.biWidth-1; ax>=0;ax--){
	...
	if (!bGrayScale && m_bmpHeadInfo.biClrUsed && m_stImgInfo.nBkgndIndex>0)
		SwapIndex((BYTE)m_stImgInfo.nBkgndIndex,0);
