...
bool CxImage::Threshold(BYTE level)
{
	...
	CxImage tmp(m_bmpHeadInfo.biWidth,m_bmpHeadInfo.biHeight,1);
	if (!tmp.IsValid()){
...
bool CxImage::Threshold(CxImage* pThresholdMask)
{
	...
	if (!pThresholdMask->IsValid() ||
		...
		strcpy(m_stImgInfo.szLastError,"invalid ThresholdMask");
		return false;
	}
...
bool CxImage::SplitRGB(CxImage* r,CxImage* g,CxImage* b)
{
	...
	CxImage tmpr(m_bmpHeadInfo.biWidth,m_bmpHeadInfo.biHeight,8);
	CxImage tmpg(m_bmpHeadInfo.biWidth,m_bmpHeadInfo.biHeight,8);
	CxImage tmpb(m_bmpHeadInfo.biWidth,m_bmpHeadInfo.biHeight,8);
...
bool CxImage::SplitCMYK(CxImage* c,CxImage* m,CxImage* y,CxImage* k)
{
	...
	CxImage tmpc(m_bmpHeadInfo.biWidth,m_bmpHeadInfo.biHeight,8);
	CxImage tmpm(m_bmpHeadInfo.biWidth,m_bmpHeadInfo.biHeight,8);
	CxImage tmpy(m_bmpHeadInfo.biWidth,m_bmpHeadInfo.biHeight,8);
	CxImage tmpk(m_bmpHeadInfo.biWidth,m_bmpHeadInfo.biHeight,8);

	RGBQUAD color;
	for(long yy=0; yy<m_bmpHeadInfo.biHeight; yy++){
		for(long xx=0; xx<m_bmpHeadInfo.biWidth; xx++){
...
bool CxImage::SplitYUV(CxImage* y,CxImage* u,CxImage* v)
{
	...
	CxImage tmpy(m_bmpHeadInfo.biWidth,m_bmpHeadInfo.biHeight,8);
	CxImage tmpu(m_bmpHeadInfo.biWidth,m_bmpHeadInfo.biHeight,8);
	CxImage tmpv(m_bmpHeadInfo.biWidth,m_bmpHeadInfo.biHeight,8);
...
bool CxImage::SplitYIQ(CxImage* y,CxImage* i,CxImage* q)
{
	...
	CxImage tmpy(m_bmpHeadInfo.biWidth,m_bmpHeadInfo.biHeight,8);
	CxImage tmpi(m_bmpHeadInfo.biWidth,m_bmpHeadInfo.biHeight,8);
	CxImage tmpq(m_bmpHeadInfo.biWidth,m_bmpHeadInfo.biHeight,8);
...
bool CxImage::SplitXYZ(CxImage* x,CxImage* y,CxImage* z)
{
	...
	CxImage tmpx(m_bmpHeadInfo.biWidth,m_bmpHeadInfo.biHeight,8);
	CxImage tmpy(m_bmpHeadInfo.biWidth,m_bmpHeadInfo.biHeight,8);
	CxImage tmpz(m_bmpHeadInfo.biWidth,m_bmpHeadInfo.biHeight,8);
...
bool CxImage::SplitHSL(CxImage* h,CxImage* s,CxImage* l)
{
	...
	CxImage tmph(m_bmpHeadInfo.biWidth,m_bmpHeadInfo.biHeight,8);
	CxImage tmps(m_bmpHeadInfo.biWidth,m_bmpHeadInfo.biHeight,8);
	CxImage tmpl(m_bmpHeadInfo.biWidth,m_bmpHeadInfo.biHeight,8);
...
{
	if (m_bmpHeadInfo.biClrUsed==0) return;

	for(DWORD j=0; j<m_bmpHeadInfo.biClrUsed; j++){
		BYTE i=(BYTE)(j*correction*(255/(m_bmpHeadInfo.biClrUsed-1)));
		RGBQUAD hsl={120,240,i,0};
...
float CxImage::Mean()
{
	...
	BYTE *iSrc=tmp.m_stImgInfo.pImage;
	iSrc += tmp.m_stImgInfo.dwEffWidth*ymin; // necessary for selections <Admir Hodzic>

	for(long y=ymin; y<ymax; y++){
		m_stImgInfo.nProgress = (long)(100*(y-ymin)/(ymax-ymin)); //<zhanghk><Anatoly Ivasyuk>
		for(long x=xmin; x<xmax; x++){
			sum+=iSrc[x];
		}
		iSrc+=tmp.m_stImgInfo.dwEffWidth;
	}
...
bool CxImage::Filter(long* kernel, long Ksize, long Kfactor, long Koffset)
{
	...
	if ((m_bmpHeadInfo.biBitCount==8) && IsGrayScale())
	{
		unsigned char* cPtr;
		unsigned char* cPtr2;      
		int iCount;
		int iY, iY2, iY1;
		cPtr = m_stImgInfo.pImage;
		cPtr2 = (unsigned char *)tmp.m_stImgInfo.pImage;
		for(long y=ymin; y<ymax; y++){
			m_stImgInfo.nProgress = (long)(100*(y-ymin)/(ymax-ymin));
			if (m_stImgInfo.nEscape) break;
			iY1 = y*m_stImgInfo.dwEffWidth+xmin;
			for(long x=xmin; x<xmax; x++, iY1++){
...
#endif //CXIMAGE_SUPPORT_SELECTION
				{
					b=ksumcur=0;
					iCount = 0;
					iY2 = ((y-k2)*m_stImgInfo.dwEffWidth);
					for(long j=-k2;j<kmax;j++, iY2+=m_stImgInfo.dwEffWidth)
					{
						if (0>(y+j) || (y+j)>=m_bmpHeadInfo.biHeight) continue;
						iY = iY2+x;
						for(long k=-k2;k<kmax;k++, iCount++)
						{
							if (0>(x+k) || (x+k)>=m_bmpHeadInfo.biWidth) continue;
							i=kernel[iCount];
							b += cPtr[iY+k] * i;
							ksumcur += i;
						}
...
void CxImage::Mix(CxImage & imgsrc2, ImageOpType op, long lXOffset, long lYOffset, bool bMixAlpha)
{
    ...
    {
		m_stImgInfo.nProgress = (long)(100*lY/m_bmpHeadInfo.biHeight);
		if (m_stImgInfo.nEscape) break;
					...
					case OpScreen:
						{ 
							...
							if (imgsrc2.IsTransparent(lX+lXOffset,lY+lYOffset)){
								...
								a =(BYTE)((a*imgsrc2.m_stImgInfo.nAlphaMax)/255);
							} else {
								a=255;
							}
...
bool CxImage::Combine(CxImage* r,CxImage* g,CxImage* b,CxImage* a, long colorspace)
{
	...
	RGBQUAD c;
	for (long y=0;y<h;y++){
		m_stImgInfo.nProgress = (long)(100*y/h); //<Anatoly Ivasyuk>
		for (long x=0;x<w;x++){
...
void CxImage::blur_text (BYTE threshold, BYTE decay, BYTE max_depth, CxImage* iSrc, CxImage* iDst, BYTE bytes)
{
	...
	xmin = ymin = 0;
	xmax = iSrc->m_bmpHeadInfo.biWidth;
	ymax = iSrc->m_bmpHeadInfo.biHeight;
	...
	for (n=0; n<bytes; n++){
		...
		{
			if (m_stImgInfo.nEscape) break;
			m_stImgInfo.nProgress = (long)((y-ymin)*dbScaler*(1+n));
...
bool CxImage::TextBlur(BYTE threshold, BYTE decay, BYTE max_depth, bool bBlurHorizontal, bool bBlurVertical, CxImage* iDst)
{
	...
	if (!(m_bmpHeadInfo.biBitCount == 24 || IsGrayScale())){
		pPalette = new RGBQUAD[m_bmpHeadInfo.biClrUsed];
	...
	if (bBlurHorizontal)
		blur_text(threshold, decay, max_depth, this, &tmp, m_bmpHeadInfo.biBitCount>>3);

	if (bBlurVertical){
		CxImage src2(*this);
		src2.RotateLeft();
		tmp.RotateLeft();
		blur_text(threshold, decay, max_depth, &src2, &tmp, m_bmpHeadInfo.biBitCount>>3);
		tmp.RotateRight();
	}
...
bool CxImage::GaussianBlur(float radius /*= 1.0f*/, CxImage* iDst /*= 0*/)
{
	...
	if (!tmp_x.IsValid()){
		strcpy(m_stImgInfo.szLastError,tmp_x.GetLastError());
		return false;
	}
	...
	long x,y;
	int bypp = m_bmpHeadInfo.biBitCount>>3;

	CImageIterator itSrc(this);
	CImageIterator itTmp(&tmp_x);

	double dbScaler = 50.0f/m_bmpHeadInfo.biHeight;

	// blur the rows
    for (y=0;y<m_bmpHeadInfo.biHeight;y++)
	{
		if (m_stImgInfo.nEscape) break;
		m_stImgInfo.nProgress = (long)(y*dbScaler);

		blur_line(ctable, cmatrix, cmatrix_length, itSrc.GetRow(y), itTmp.GetRow(y), m_bmpHeadInfo.biWidth, bypp);
	}

	CxImage tmp_y(tmp_x, false, true, true);
	if (!tmp_y.IsValid()){
		strcpy(m_stImgInfo.szLastError,tmp_y.GetLastError());
		return false;
	}
	...
	// blur the cols
    BYTE* cur_col = (BYTE*)malloc(bypp*m_bmpHeadInfo.biHeight);
    BYTE* dest_col = (BYTE*)malloc(bypp*m_bmpHeadInfo.biHeight);

	dbScaler = 50.0f/m_bmpHeadInfo.biWidth;

	for (x=0;x<m_bmpHeadInfo.biWidth;x++)
	{
		if (m_stImgInfo.nEscape) break;
		m_stImgInfo.nProgress = (long)(50.0f+x*dbScaler);

		itTmp.GetCol(cur_col, x);
		itDst.GetCol(dest_col, x);
		blur_line(ctable, cmatrix, cmatrix_length, cur_col, dest_col, m_bmpHeadInfo.biHeight, bypp);
...
bool CxImage::SelectiveBlur(float radius, BYTE threshold, CxImage* iDst)
{
	...
	if (!Dst.IsValid()){
		strcpy(m_stImgInfo.szLastError,Dst.GetLastError());
		return false;
	}
	...
	long xmin,xmax,ymin,ymax;
	xmin = Tmp.m_stImgInfo.rSelectionBox.left;
	xmax = Tmp.m_stImgInfo.rSelectionBox.right;
	ymin = Tmp.m_stImgInfo.rSelectionBox.bottom;
	ymax = Tmp.m_stImgInfo.rSelectionBox.top;
...
bool CxImage::UnsharpMask(float radius /*= 5.0*/, float amount /*= 0.5*/, int threshold /*= 0*/)
{
	...
	{
		if (m_stImgInfo.nEscape) break;
		m_stImgInfo.nProgress = (long)((y-ymin)*dbScaler);
...
bool CxImage::Lut(BYTE* pLut)
{
	...
	if (m_bmpHeadInfo.biClrUsed==0){
		...
		for(long y=ymin; y<ymax; y++){
			m_stImgInfo.nProgress = (long)((y-ymin)*dbScaler); //<Anatoly Ivasyuk>
			for(long x=xmin; x<xmax; x++){
#if CXIMAGE_SUPPORT_SELECTION
				if (BlindSelectionIsInside(x,y))
#endif //CXIMAGE_SUPPORT_SELECTION
...
#if CXIMAGE_SUPPORT_SELECTION
	} else if (m_pSelection && (m_bmpHeadInfo.biBitCount==8) && IsGrayScale()){
		long xmin,xmax,ymin,ymax;
		xmin = m_stImgInfo.rSelectionBox.left; xmax = m_stImgInfo.rSelectionBox.right;
		ymin = m_stImgInfo.rSelectionBox.bottom; ymax = m_stImgInfo.rSelectionBox.top;
...
{
	if (!m_pDib || !pLutR || !pLutG || !pLutB) return false;
	RGBQUAD color;
...
bool CxImage::Saturate(const long saturation, const long colorspace)
{
	...
	case 2:
		...
		break;
	default:
		strcpy(m_stImgInfo.szLastError,"Saturate: wrong colorspace");
		return false;
	}
	return true;
}
...
bool CxImage::Solarize(BYTE level, bool bLinkedChannels)
{
	...
	if (m_pSelection==0 || (!IsGrayScale() && IsIndexed())){
		if (bLinkedChannels){
			if ((BYTE)RGB2GRAY(m_stImgInfo.nBkgndColor.rgbRed,m_stImgInfo.nBkgndColor.rgbGreen,m_stImgInfo.nBkgndColor.rgbBlue)>level){
			...
			if (m_stImgInfo.nBkgndColor.rgbBlue>level)	 m_stImgInfo.nBkgndColor.rgbBlue = (BYTE)(255-m_stImgInfo.nBkgndColor.rgbBlue);
			if (m_stImgInfo.nBkgndColor.rgbGreen>level) m_stImgInfo.nBkgndColor.rgbGreen = (BYTE)(255-m_stImgInfo.nBkgndColor.rgbGreen);
			if (m_stImgInfo.nBkgndColor.rgbRed>level)	 m_stImgInfo.nBkgndColor.rgbRed = (BYTE)(255-m_stImgInfo.nBkgndColor.rgbRed);
		}
	}
...
bool CxImage::ConvertColorSpace(const long dstColorSpace, const long srcColorSpace)
{
	...
	for (long y=0;y<h;y++){
		m_stImgInfo.nProgress = (long)(100*y/h);
		if (m_stImgInfo.nEscape) break;
		for (long x=0;x<w;x++){
			...
			case 4:
				...
				break;
			default:
				strcpy(m_stImgInfo.szLastError,"ConvertColorSpace: unknown source colorspace");
				return false;
			}
			...
			case 4:
				...
				break;
			default:
				strcpy(m_stImgInfo.szLastError,"ConvertColorSpace: unknown destination colorspace");
				return false;
			}
			BlindSetPixelColor(x,y,c);
		}
...
int  CxImage::OptimalThreshold(long method, RECT * pBox, CxImage* pContrastMask)
{
	...
	if (m_bmpHeadInfo.biBitCount!=8){
		strcpy(m_stImgInfo.szLastError,"OptimalThreshold works only on 8 bit images");
		return -1;
	}
	...
	if (pContrastMask){
		if (!pContrastMask->IsValid() ||
			...
			strcpy(m_stImgInfo.szLastError,"OptimalThreshold invalid ContrastMask");
			return -1;
		}
	}
	...
	if (pBox){
		...
		xmax = min(pBox->right,m_bmpHeadInfo.biWidth);
		ymin = max(pBox->bottom,0);
		ymax = min(pBox->top,m_bmpHeadInfo.biHeight);
	} else {
		xmin = ymin = 0;
		xmax = m_bmpHeadInfo.biWidth; ymax=m_bmpHeadInfo.biHeight;
	}
...
bool CxImage::AdaptiveThreshold(long method, long nBoxSize, CxImage* pContrastMask, long nBias, float fGlobalLocalBalance)
{
	...
	if (pContrastMask){
		if (!pContrastMask->IsValid() ||
			...
			pContrastMask->GetWidth() != GetWidth() ||
			pContrastMask->GetHeight() != GetHeight()){
			strcpy(m_stImgInfo.szLastError,"AdaptiveThreshold invalid ContrastMask");
			return false;
		}
	}
	...
	long mw = (m_bmpHeadInfo.biWidth + nBoxSize - 1)/nBoxSize;
	long mh = (m_bmpHeadInfo.biHeight + nBoxSize - 1)/nBoxSize;
	...
	for (long y=0; y<mh; y++){
		for (long x=0; x<mw; x++){
			m_stImgInfo.nProgress = (long)(100*(x+y*mw)/(mw*mh));
			if (m_stImgInfo.nEscape) break;
			RECT r;
			r.left = x*nBoxSize;
	...
	mask.Crop(0,m_bmpHeadInfo.biHeight,m_bmpHeadInfo.biWidth,0);
						...
						BYTE nOpacity, const bool bSelectFilledArea, const BYTE nSelectionLevel)
{
	...
	if ((nTolerance || nOpacity != 255) &&	!(m_bmpHeadInfo.biBitCount == 24 || IsGrayScale())){
		pPalette = new RGBQUAD[m_bmpHeadInfo.biClrUsed];
		memcpy(pPalette, GetPalette(),GetPaletteSize());
		if (!IncreaseBpp(24))
			return false;
	...
	BYTE* pFillMask = (BYTE*)calloc(m_bmpHeadInfo.biWidth * m_bmpHeadInfo.biHeight,1);
	if (!pFillMask)
		return false;
...
//------------------------------------- Begin of Flood Fill
			...
			for (int z=0; z<4; z++){
				...
				if(IsInside(x,y)){
...
#endif //CXIMAGE_SUPPORT_SELECTION
				  {
					...
					BYTE* pFill = pFillMask + x + y * m_bmpHeadInfo.biWidth;
	...
	if (pFillMask[xStart+yStart*m_bmpHeadInfo.biWidth] == 0 && nOpacity>0){
		if (nOpacity == 255)
	...
	pFillMask[xStart+yStart*m_bmpHeadInfo.biWidth] = 1;
//------------------------------------- End of Flood Fill
	...
	if (bSelectFilledArea){
		...
		for (long y = r.bottom; y < r.top; y++){
			BYTE* pFill = pFillMask + r.left + y * m_bmpHeadInfo.biWidth;
