...
bool CxImageTGA::Decode(CxFile *hFile)
{
  ...
  cx_try
  {
	...
	if (m_stImgInfo.nEscape == -1){
		m_bmpHeadInfo.biWidth = tgaHead.ImageWidth ;
		m_bmpHeadInfo.biHeight= tgaHead.ImageHeight;
		m_stImgInfo.dwType = CXIMAGE_FORMAT_TGA;
		return true;
	}
...
bool CxImageTGA::Encode(CxFile * hFile)
{
	...
	if (m_bmpHeadInfo.biBitCount<8){
		strcpy(m_stImgInfo.szLastError,"Bit depth must be 8 or 24");
		return false;
	}
    ...
    tgaHead.ImageType = (m_bmpHeadInfo.biBitCount == 8) ? (BYTE)TGA_Map : (BYTE)TGA_RGB; // Image Type

    tgaHead.CmapIndex=0;				// First Entry Index
    tgaHead.CmapLength=(m_bmpHeadInfo.biBitCount == 8) ? 256 : 0;   // Color Map Length
    tgaHead.CmapEntrySize=(m_bmpHeadInfo.biBitCount == 8) ? (BYTE)24 : (BYTE)0; // Color Map Entry Size

    tgaHead.X_Origin=0;					// X-origin of Image
    tgaHead.Y_Origin=0;					// Y-origin of Image
    tgaHead.ImageWidth=(WORD)m_bmpHeadInfo.biWidth;		// Image Width
    tgaHead.ImageHeight=(WORD)m_bmpHeadInfo.biHeight;	// Image Height
    tgaHead.PixelDepth=(BYTE)m_bmpHeadInfo.biBitCount;	// Pixel Depth
    tgaHead.ImagDesc=0;					// Image Descriptor

	if (m_pAlpha && m_bmpHeadInfo.biBitCount==24) tgaHead.PixelDepth=32; 
	...
	CImageIterator iter(this);
	BYTE* pDest;
	if (m_pAlpha==0 || m_bmpHeadInfo.biBitCount==8){
		for (int y=0; y < tgaHead.ImageHeight; y++){
			pDest = iter.GetRow(y);
			hFile->Write(pDest,tgaHead.ImageWidth * (m_bmpHeadInfo.biBitCount >> 3),1);
		}
	} else {
...
BYTE CxImageTGA::ExpandCompressedLine(BYTE* pDest,TGAHEADER* ptgaHead,CxFile *hFile,int width, int y, BYTE rleLeftover)
{
	...
	for (int x=0; x<width; ){
		...
		if (m_bmpHeadInfo.biBitCount == 24)	pDest += rle*3;	else pDest += rle;
		x += rle;
	}
	return rleLeftover;
}
