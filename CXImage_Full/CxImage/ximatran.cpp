...
bool CxImage::GrayScale()
{
	...
	if (m_bmpHeadInfo.biBitCount<=8){
		RGBQUAD* ppal=GetPalette();
		int gray;
		//converts the colors to gray, use the blue channel only
		for(DWORD i=0;i<m_bmpHeadInfo.biClrUsed;i++){
			gray=(int)RGB2GRAY(ppal[i].rgbRed,ppal[i].rgbGreen,ppal[i].rgbBlue);
			ppal[i].rgbBlue = (BYTE)gray;
		}
		// preserve transparency
		if (m_stImgInfo.nBkgndIndex >= 0) m_stImgInfo.nBkgndIndex = ppal[m_stImgInfo.nBkgndIndex].rgbBlue;
		//create a "real" 8 bit gray scale image
		if (m_bmpHeadInfo.biBitCount==8){
			BYTE *img=m_stImgInfo.pImage;
			for(DWORD i=0;i<m_bmpHeadInfo.biSizeImage;i++) img[i]=ppal[img[i]].rgbBlue;
			SetGrayPalette();
		}
		//transform to 8 bit gray scale
		if (m_bmpHeadInfo.biBitCount==4 || m_bmpHeadInfo.biBitCount==1){
			CxImage ima;
			ima.CopyInfo(*this);
			if (!ima.Create(m_bmpHeadInfo.biWidth,m_bmpHeadInfo.biHeight,8,m_stImgInfo.dwType)) return false;
			ima.SetGrayPalette();
...
bool CxImage::Flip(bool bFlipSelection, bool bFlipAlpha)
{
	...
	BYTE *buff = (BYTE*)malloc(m_stImgInfo.dwEffWidth);
	if (!buff) return false;

	BYTE *iSrc,*iDst;
	iSrc = GetBits(m_bmpHeadInfo.biHeight-1);
	iDst = GetBits(0);
	for (long i=0; i<(m_bmpHeadInfo.biHeight/2); ++i)
	{
		memcpy(buff, iSrc, m_stImgInfo.dwEffWidth);
		memcpy(iSrc, iDst, m_stImgInfo.dwEffWidth);
		memcpy(iDst, buff, m_stImgInfo.dwEffWidth);
		iSrc-=m_stImgInfo.dwEffWidth;
		iDst+=m_stImgInfo.dwEffWidth;
	}
...
bool CxImage::Mirror(bool bMirrorSelection, bool bMirrorAlpha)
{
	...
	BYTE *iSrc,*iDst;
	long wdt=(m_bmpHeadInfo.biWidth-1) * (m_bmpHeadInfo.biBitCount==24 ? 3:1);
	iSrc=m_stImgInfo.pImage + wdt;
	iDst=imatmp->m_stImgInfo.pImage;
	long x,y;
	switch (m_bmpHeadInfo.biBitCount){
	case 24:
...
bool CxImage::RotateLeft(CxImage* iDst)
{
	...
	if (m_bmpHeadInfo.biBitCount == 1) {
	
		BYTE *sbits, *dbits, *dbitsmax, bitpos, *nrow,*srcdisp;
		ldiv_t div_r;

		BYTE *bsrc = GetBits(), *bdest = imgDest.GetBits();
		dbitsmax = bdest + imgDest.m_bmpHeadInfo.biSizeImage - 1;
		dlineup = 8 * imgDest.m_stImgInfo.dwEffWidth - imgDest.m_bmpHeadInfo.biWidth;
		...
		for (y = 0; y < m_bmpHeadInfo.biHeight; y++) {
			...
			srcdisp = bsrc + y * m_stImgInfo.dwEffWidth;
			for (x = 0; x < (long)m_stImgInfo.dwEffWidth; x++) {
				// Get Source Bits
				sbits = srcdisp + x;
				// Get destination column
				nrow = bdest + (x * 8) * imgDest.m_stImgInfo.dwEffWidth + imgDest.m_stImgInfo.dwEffWidth - 1 - div_r.quot;
				for (long z = 0; z < 8; z++) {
				   // Get Destination Byte
					dbits = nrow + z * imgDest.m_stImgInfo.dwEffWidth;
...
#if CXIMAGE_SUPPORT_SELECTION
		if (SelectionIsValid()) {
			imgDest.m_stImgInfo.rSelectionBox.left = newWidth-m_stImgInfo.rSelectionBox.top;
			imgDest.m_stImgInfo.rSelectionBox.right = newWidth-m_stImgInfo.rSelectionBox.bottom;
			imgDest.m_stImgInfo.rSelectionBox.bottom = m_stImgInfo.rSelectionBox.left;
			imgDest.m_stImgInfo.rSelectionBox.top = m_stImgInfo.rSelectionBox.right;
			for (x = 0; x < newWidth; x++){
				x2=newWidth-x-1;
				for (y = 0; y < newHeight; y++){
			...
			for (ys = 0; ys < newHeight; ys+=RBLOCK) {
				if (m_bmpHeadInfo.biBitCount==24) {
					...
					for (x = xs; x < min(newWidth, xs+RBLOCK); x++){    //do rotation
						m_stImgInfo.nProgress = (long)(100*x/newWidth);
						x2=newWidth-x-1;
						...
						for (y = ys; y < min(newHeight, ys+RBLOCK); y++){
							...
							*(dstPtr+2) = *(srcPtr+2);
							srcPtr += 3;
							dstPtr += imgDest.m_stImgInfo.dwEffWidth;
						}//for y
					}//for x
				} else {
					//anything else than 24bpp (and 1bpp): palette
					for (x = xs; x < min(newWidth, xs+RBLOCK); x++){
						m_stImgInfo.nProgress = (long)(100*x/newWidth); //<Anatoly Ivasyuk>
						x2=newWidth-x-1;
						for (y = ys; y < min(newHeight, ys+RBLOCK); y++){
	...
	if (m_bmpHeadInfo.biBitCount == 1) {
		...
		for (y = 0; y < m_bmpHeadInfo.biHeight; y++) {
			...
			for (x = 0; x < (long)m_stImgInfo.dwEffWidth; x++) {
				...
				sbits = srcdisp + x;
				// Get destination column
				nrow = bdest + (imgDest.m_bmpHeadInfo.biHeight-1-(x*8)) * imgDest.m_stImgInfo.dwEffWidth + div_r.quot;
				for (long z = 0; z < 8; z++) {
				   // Get Destination Byte
					dbits = nrow - z * imgDest.m_stImgInfo.dwEffWidth;
					if ((dbits < bdest) || (dbits > dbitsmax)) break;
...
#if CXIMAGE_SUPPORT_SELECTION
		if (SelectionIsValid()){
			imgDest.m_stImgInfo.rSelectionBox.left = m_stImgInfo.rSelectionBox.bottom;
			imgDest.m_stImgInfo.rSelectionBox.right = m_stImgInfo.rSelectionBox.top;
			imgDest.m_stImgInfo.rSelectionBox.bottom = newHeight-m_stImgInfo.rSelectionBox.right;
			imgDest.m_stImgInfo.rSelectionBox.top = newHeight-m_stImgInfo.rSelectionBox.left;
			for (y = 0; y < newHeight; y++){
				y2=newHeight-y-1;
				for (x = 0; x < newWidth; x++){
		...
		for (xs = 0; xs < newWidth; xs+=RBLOCK) {
			for (ys = 0; ys < newHeight; ys+=RBLOCK) {
				if (m_bmpHeadInfo.biBitCount==24) {
					//RGB24 optimized pixel access:
					for (y = ys; y < min(newHeight, ys+RBLOCK); y++){
						m_stImgInfo.nProgress = (long)(100*y/newHeight); //<Anatoly Ivasyuk>
						y2=newHeight-y-1;
...
bool CxImage::Negative()
{
	...
	if (m_bmpHeadInfo.biBitCount<=8){
		if (IsGrayScale()){ //GRAYSCALE, selection
			if (m_pSelection){
				for(long y=m_stImgInfo.rSelectionBox.bottom; y<m_stImgInfo.rSelectionBox.top; y++){
					for(long x=m_stImgInfo.rSelectionBox.left; x<m_stImgInfo.rSelectionBox.right; x++){
#if CXIMAGE_SUPPORT_SELECTION
				...
				BYTE *iSrc=m_stImgInfo.pImage;
				for(unsigned long i=0; i < m_bmpHeadInfo.biSizeImage; i++){
					*iSrc=(BYTE)~(*(iSrc));
					iSrc++;
				}
			}
		...
		if (m_pSelection==NULL){ //RGB, full image
			BYTE *iSrc=m_stImgInfo.pImage;
			for(unsigned long i=0; i < m_bmpHeadInfo.biSizeImage; i++){
				*iSrc=(BYTE)~(*(iSrc));
				iSrc++;
		...
		m_stImgInfo.nBkgndColor.rgbBlue = (BYTE)(255-m_stImgInfo.nBkgndColor.rgbBlue);
		m_stImgInfo.nBkgndColor.rgbGreen = (BYTE)(255-m_stImgInfo.nBkgndColor.rgbGreen);
		m_stImgInfo.nBkgndColor.rgbRed = (BYTE)(255-m_stImgInfo.nBkgndColor.rgbRed);
	}
	return true;
}
...
bool CxImage::Rotate(float angle, CxImage* iDst)
{
	...
	if (m_bmpHeadInfo.biClrUsed==0){ //RGB
		for (y = (int)leftTop.y, newY = 0; y<=(int)leftBottom.y; y++,newY++){
			m_stImgInfo.nProgress = (long)(100*newY/newHeight);
			if (m_stImgInfo.nEscape) break;
...
{
	if (!m_pDib) return false;					//no dib no go
	...
	if (!IsIndexed()){ //RGB24
		...
		for (desty=0; desty<newHeight; desty++) {
			m_stImgInfo.nProgress = (long)(100*desty/newHeight);
			if (m_stImgInfo.nEscape) break;
			//initialize x
			x=ssx;
...
bool CxImage::Rotate180(CxImage* iDst)
{
	...
	long x,y,y2;
	for (y = 0; y < ht; y++){
		m_stImgInfo.nProgress = (long)(100*y/ht); //<Anatoly Ivasyuk>
		y2=ht-y-1;
		for (x = 0; x < wid; x++){
			if(m_bmpHeadInfo.biClrUsed==0)//RGB
...
bool CxImage::Resample(long newx, long newy, int mode, CxImage* iDst)
{
	...
	if (m_bmpHeadInfo.biWidth==newx && m_bmpHeadInfo.biHeight==newy){
		if (iDst) iDst->Copy(*this);
		return true;
	}

	float xScale, yScale, fX, fY;
	xScale = (float)m_bmpHeadInfo.biWidth  / (float)newx;
	yScale = (float)m_bmpHeadInfo.biHeight / (float)newy;

	CxImage newImage;
	newImage.CopyInfo(*this);
	newImage.Create(newx,newy,m_bmpHeadInfo.biBitCount,GetType());
	newImage.SetPalette(GetPalette());
	if (!newImage.IsValid()){
		strcpy(m_stImgInfo.szLastError,newImage.GetLastError());
		return false;
	}
	...
	{ 
		for(long y=0; y<newy; y++){
			m_stImgInfo.nProgress = (long)(100*y/newy);
			if (m_stImgInfo.nEscape) break;
			fY = y * yScale;
		...
		for(long y=0; y<newy; y++){
			...
			for(long x=0; x<newx; x++){
				...
				for(int m=-1; m<3; m++) {
					...
					yy = i_y+m;
					if (yy<0) yy=0;
					if (yy>=m_bmpHeadInfo.biHeight) yy = m_bmpHeadInfo.biHeight-1;
					for(int n=-1; n<3; n++) {
						r2 = r1 * KernelBSpline(b - (float)n);
						xx = i_x+n;
						if (xx<0) xx=0;
						if (xx>=m_bmpHeadInfo.biWidth) xx=m_bmpHeadInfo.biWidth-1;

						if (m_bmpHeadInfo.biClrUsed){
							rgb = GetPixelColor(xx,yy);
						} else {
							iDst  = m_stImgInfo.pImage + yy*m_stImgInfo.dwEffWidth + xx*3;
							rgb.rgbBlue = *iDst++;
							rgb.rgbGreen= *iDst++;
							rgb.rgbRed  = *iDst;
						}
				...
				if (m_bmpHeadInfo.biClrUsed)
					...
					iDst = newImage.m_stImgInfo.pImage + y*newImage.m_stImgInfo.dwEffWidth + x*3;
					*iDst++ = (BYTE)bb;
					*iDst++ = (BYTE)gg;
					*iDst   = (BYTE)rr;
				}
	...
	default: // bilinear interpolation
		if (!(m_bmpHeadInfo.biWidth>newx && m_bmpHeadInfo.biHeight>newy && m_bmpHeadInfo.biBitCount==24)) {
			...
			xmax = m_bmpHeadInfo.biWidth-1;
			ymax = m_bmpHeadInfo.biHeight-1;
			for(long y=0; y<newy; y++){
				m_stImgInfo.nProgress = (long)(100*y/newy);
				if (m_stImgInfo.nEscape) break;
				fY = y * yScale;
				...
				for(long x=0; x<newx; x++){
					...
					if (m_bmpHeadInfo.biClrUsed){
						...
						BYTE* iDst;
						iDst = m_stImgInfo.pImage + ifY*m_stImgInfo.dwEffWidth + ifX*3;
						rgb1.rgbBlue = *iDst++;	rgb1.rgbGreen= *iDst++;	rgb1.rgbRed =*iDst;
						iDst = m_stImgInfo.pImage + ifY*m_stImgInfo.dwEffWidth + ifX1*3;
						rgb2.rgbBlue = *iDst++;	rgb2.rgbGreen= *iDst++;	rgb2.rgbRed =*iDst;
						iDst = m_stImgInfo.pImage + ifY1*m_stImgInfo.dwEffWidth + ifX*3;
						rgb3.rgbBlue = *iDst++;	rgb3.rgbGreen= *iDst++;	rgb3.rgbRed =*iDst;
						iDst = m_stImgInfo.pImage + ifY1*m_stImgInfo.dwEffWidth + ifX1*3;
			...
			BYTE* pSource;
			BYTE* pDest = newImage.m_stImgInfo.pImage;
			...
			for (y = 0; y < m_bmpHeadInfo.biHeight; y++){
				m_stImgInfo.nProgress = (long)(100*y/m_bmpHeadInfo.biHeight); //<Anatoly Ivasyuk>
				if (m_stImgInfo.nEscape) break;
				pSource = m_stImgInfo.pImage + y * m_stImgInfo.dwEffWidth;
				u = i = 0;
				fEndX = xScale - 1.0f;
				if ((float)y < fEndY) {       // complete source row goes into dest row
					...
					pDest = newImage.m_stImgInfo.pImage + (++v * newImage.m_stImgInfo.dwEffWidth);
					fEndY += yScale;
				}
			}
...
{
	if (newx<=0 || newy<=0 || !m_pDib) return false;
	...
	if ((xScale<=1 && yScale<=1) || disableAveraging) {
		...
		if (!IsIndexed()) {
			...
			for(dY=0; dY<newy; dY++){
				m_stImgInfo.nProgress = (long)(100*dY/newy);
				if (m_stImgInfo.nEscape) break;
		...
		for(dY=0; dY<newy; dY++){
			m_stImgInfo.nProgress = (long)(100*dY/newy); if (m_stImgInfo.nEscape) break;
			sY = (dY+0.5f) * yScale - 0.5f;
			for(dX=0; dX<newx; dX++){
				sX = (dX+0.5f) * xScale - 0.5f;
...
{
	if (!m_pDib) return false;
	if (m_bmpHeadInfo.biBitCount <  nbit){
		strcpy(m_stImgInfo.szLastError,"DecreaseBpp: target BPP greater than source BPP");
		return false;
	}
	if (m_bmpHeadInfo.biBitCount == nbit){
		if (clrimportant==0) return true;
		if (m_bmpHeadInfo.biClrImportant && (m_bmpHeadInfo.biClrImportant<clrimportant)) return true;
	}
	...
	CxImage tmp;
	tmp.CopyInfo(*this);
	tmp.Create(m_bmpHeadInfo.biWidth,m_bmpHeadInfo.biHeight,(WORD)nbit,m_stImgInfo.dwType);
	...
	if (ppal) {
		if (clrimportant) {
			...
			tmp.SetPalette(ppal,1<<tmp.m_bmpHeadInfo.biBitCount);
		}
	} else {
		tmp.SetStdPalette();
	}
	...
	for (long y=0;y<m_bmpHeadInfo.biHeight;y++){
		if (m_stImgInfo.nEscape) break;
		m_stImgInfo.nProgress = (long)(100*y/m_bmpHeadInfo.biHeight);
		for (long x=0;x<m_bmpHeadInfo.biWidth;x++){
			if (!errordiffusion){
...
bool CxImage::IncreaseBpp(DWORD nbit)
{
	...
	case 4:
		{
			if (m_bmpHeadInfo.biBitCount==4) return true;
			if (m_bmpHeadInfo.biBitCount>4) return false;

			CxImage tmp;
			tmp.CopyInfo(*this);
			tmp.Create(m_bmpHeadInfo.biWidth,m_bmpHeadInfo.biHeight,4,m_stImgInfo.dwType);
	...
	case 8:
		{
			if (m_bmpHeadInfo.biBitCount==8) return true;
			if (m_bmpHeadInfo.biBitCount>8) return false;

			CxImage tmp;
			tmp.CopyInfo(*this);
			tmp.Create(m_bmpHeadInfo.biWidth,m_bmpHeadInfo.biHeight,8,m_stImgInfo.dwType);
			tmp.SetPalette(GetPalette(),GetNumColors());
			if (!tmp.IsValid()){
				strcpy(m_stImgInfo.szLastError,tmp.GetLastError());
				return false;
	...
	case 24:
		{
			if (m_bmpHeadInfo.biBitCount==24) return true;
			if (m_bmpHeadInfo.biBitCount>24) return false;

			CxImage tmp;
			tmp.CopyInfo(*this);
			tmp.Create(m_bmpHeadInfo.biWidth,m_bmpHeadInfo.biHeight,24,m_stImgInfo.dwType);
			if (!tmp.IsValid()){
				strcpy(m_stImgInfo.szLastError,tmp.GetLastError());
				return false;
			}

			if (m_stImgInfo.nBkgndIndex>=0) //translate transparency
				tmp.m_stImgInfo.nBkgndColor=GetPaletteColor((BYTE)m_stImgInfo.nBkgndIndex);
...
{
	if (!m_pDib) return false;
	if (m_bmpHeadInfo.biBitCount == 1) return true;
	
	GrayScale();

	CxImage tmp;
	tmp.CopyInfo(*this);
	tmp.Create(m_bmpHeadInfo.biWidth, m_bmpHeadInfo.biHeight, 1, m_stImgInfo.dwType);
	if (!tmp.IsValid()){
		strcpy(m_stImgInfo.szLastError,tmp.GetLastError());
		return false;
	}
...
bool CxImage::CropRotatedRectangle( long topx, long topy, long width, long height, float angle, CxImage* iDst)
{
    ...
    endx   = (tmp.m_bmpHeadInfo.biWidth+width)/2;
	startx = (tmp.m_bmpHeadInfo.biWidth-width)/2;
	starty = (tmp.m_bmpHeadInfo.biHeight+height)/2;
    endy   = (tmp.m_bmpHeadInfo.biHeight-height)/2;
...
bool CxImage::Crop(long left, long top, long right, long bottom, CxImage* iDst)
{
	...
	long startx = max(0L,min(left,m_bmpHeadInfo.biWidth));
	long endx = max(0L,min(right,m_bmpHeadInfo.biWidth));
	long starty = m_bmpHeadInfo.biHeight - max(0L,min(top,m_bmpHeadInfo.biHeight));
	long endy = m_bmpHeadInfo.biHeight - max(0L,min(bottom,m_bmpHeadInfo.biHeight));
	...
	CxImage tmp(endx-startx,endy-starty,m_bmpHeadInfo.biBitCount,m_stImgInfo.dwType);
	if (!tmp.IsValid()){
		strcpy(m_stImgInfo.szLastError,tmp.GetLastError());
		return false;
	}

	tmp.SetPalette(GetPalette(),m_bmpHeadInfo.biClrUsed);
	tmp.m_stImgInfo.nBkgndIndex = m_stImgInfo.nBkgndIndex;
	tmp.m_stImgInfo.nBkgndColor = m_stImgInfo.nBkgndColor;
	...
	case 4:
	{
		for(long y=starty, yd=0; y<endy; y++, yd++){
			m_stImgInfo.nProgress = (long)(100*(y-starty)/(endy-starty)); //<Anatoly Ivasyuk>
	...
	case 24:
	{
		int linelen = tmp.m_bmpHeadInfo.biWidth * tmp.m_bmpHeadInfo.biBitCount >> 3;
		BYTE* pDest = tmp.m_stImgInfo.pImage;
		BYTE* pSrc = m_stImgInfo.pImage + starty * m_stImgInfo.dwEffWidth + (startx*m_bmpHeadInfo.biBitCount >> 3);
		for(long y=starty; y<endy; y++){
			m_stImgInfo.nProgress = (long)(100*(y-starty)/(endy-starty)); //<Anatoly Ivasyuk>
			memcpy(pDest,pSrc,linelen);
			pDest+=tmp.m_stImgInfo.dwEffWidth;
			pSrc+=m_stImgInfo.dwEffWidth;
		}
    }
	}
	...
	if (AlphaIsValid()){ //<oboolo>
		...
		BYTE* pDest = tmp.m_pAlpha;
		BYTE* pSrc = m_pAlpha + startx + starty*m_bmpHeadInfo.biWidth;
		for (long y=starty; y<endy; y++){
			memcpy(pDest,pSrc,endx-startx);
			pDest+=tmp.m_bmpHeadInfo.biWidth;
			pSrc+=m_bmpHeadInfo.biWidth;
		}
	}
#endif //CXIMAGE_SUPPORT_ALPHA
...
bool CxImage::Skew(float xgain, float ygain, long xpivot, long ypivot, bool bEnableInterpolation)
{
	...
	if (m_pSelection){
		xmin = m_stImgInfo.rSelectionBox.left; xmax = m_stImgInfo.rSelectionBox.right;
		ymin = m_stImgInfo.rSelectionBox.bottom; ymax = m_stImgInfo.rSelectionBox.top;
	} else {
		xmin = ymin = 0;
		xmax = m_bmpHeadInfo.biWidth; ymax=m_bmpHeadInfo.biHeight;
	}
	for(long y=ymin; y<ymax; y++){
		m_stImgInfo.nProgress = (long)(100*(y-ymin)/(ymax-ymin));
		if (m_stImgInfo.nEscape) break;
...
bool CxImage::Expand(long left, long top, long right, long bottom, RGBQUAD canvascolor, CxImage* iDst)
{
    ...
    long newWidth = m_bmpHeadInfo.biWidth + left + right;
    long newHeight = m_bmpHeadInfo.biHeight + top + bottom;

    right = left + m_bmpHeadInfo.biWidth - 1;
    top = bottom + m_bmpHeadInfo.biHeight - 1;
    
    CxImage tmp;
	tmp.CopyInfo(*this);
	if (!tmp.Create(newWidth, newHeight, m_bmpHeadInfo.biBitCount, m_stImgInfo.dwType)){
		strcpy(m_stImgInfo.szLastError,tmp.GetLastError());
		return false;
	}
    ...
    case 4:
		{
			...
			for(long y=0; y < newHeight; y++){
				m_stImgInfo.nProgress = (long)(100*y/newHeight);
    ...
    case 24:
		{
			if (m_bmpHeadInfo.biBitCount == 8) {
				BYTE pixel = tmp.GetNearestIndex( canvascolor);
				memset(tmp.m_stImgInfo.pImage, pixel,  + (tmp.m_stImgInfo.dwEffWidth * newHeight));
			} else {
				for (long y = 0; y < newHeight; ++y) {
					BYTE *pDest = tmp.m_stImgInfo.pImage + (y * tmp.m_stImgInfo.dwEffWidth);
					for (long x = 0; x < newWidth; ++x) {
			...
			BYTE* pDest = tmp.m_stImgInfo.pImage + (tmp.m_stImgInfo.dwEffWidth * bottom) + (left*(m_bmpHeadInfo.biBitCount >> 3));
			BYTE* pSrc = m_stImgInfo.pImage;
			for(long y=bottom; y <= top; y++){
				m_stImgInfo.nProgress = (long)(100*y/(1 + top - bottom));
				memcpy(pDest,pSrc,(m_bmpHeadInfo.biBitCount >> 3) * (right - left + 1));
				pDest+=tmp.m_stImgInfo.dwEffWidth;
				pSrc+=m_stImgInfo.dwEffWidth;
			}
		}
	...
	if (SelectionIsValid()){
		...
		for(long y=bottom; y <= top; y++){
			...
			pSrc+=m_bmpHeadInfo.biWidth;
			pDst+=tmp.m_bmpHeadInfo.biWidth;
		}
		tmp.m_stImgInfo.rSelectionBox.left = m_stImgInfo.rSelectionBox.left + left;
		tmp.m_stImgInfo.rSelectionBox.right = m_stImgInfo.rSelectionBox.right + left;
		tmp.m_stImgInfo.rSelectionBox.top = m_stImgInfo.rSelectionBox.top + bottom;
		tmp.m_stImgInfo.rSelectionBox.bottom = m_stImgInfo.rSelectionBox.bottom + bottom;
	}
#endif //CXIMAGE_SUPPORT_SELECTION
...
bool CxImage::Expand(long newx, long newy, RGBQUAD canvascolor, CxImage* iDst)
{
    ...
    if ((newx < m_bmpHeadInfo.biWidth) || (newy < m_bmpHeadInfo.biHeight)) return false;

    int nAddLeft = (newx - m_bmpHeadInfo.biWidth) / 2;
    int nAddTop = (newy - m_bmpHeadInfo.biHeight) / 2;

    return Expand(nAddLeft, nAddTop, newx - (m_bmpHeadInfo.biWidth + nAddLeft), newy - (m_bmpHeadInfo.biHeight + nAddTop), canvascolor, iDst);
}
////////////////////////////////////////////////////////////////////////////////
/**
...
bool CxImage::Thumbnail(long newx, long newy, RGBQUAD canvascolor, CxImage* iDst)
{
    ...
    if ((m_bmpHeadInfo.biWidth > newx) || (m_bmpHeadInfo.biHeight > newy)) {
        ...
        if (fAspect * m_bmpHeadInfo.biHeight > m_bmpHeadInfo.biWidth) {
            fScale = (float) newy / m_bmpHeadInfo.biHeight;
        } else {
            fScale = (float) newx / m_bmpHeadInfo.biWidth;
        }
        tmp.Resample((long) (fScale * m_bmpHeadInfo.biWidth), (long) (fScale * m_bmpHeadInfo.biHeight), 0);
    }
...
bool CxImage::QIShrink(long newx, long newy, CxImage* const iDst, bool bChangeBpp)
{
	...
	if (newx>m_bmpHeadInfo.biWidth || newy>m_bmpHeadInfo.biHeight) { 
		//let me repeat... this method can't enlarge image
		strcpy(m_stImgInfo.szLastError,"QIShrink can't enlarge image");
		return false;
	}
	...
	CxImage newImage;
	newImage.CopyInfo(*this);
	newImage.Create(newx,newy,(bChangeBpp)?24:m_bmpHeadInfo.biBitCount,GetType());
	newImage.SetPalette(GetPalette());
	if (!newImage.IsValid()){
		strcpy(m_stImgInfo.szLastError,newImage.GetLastError());
		return false;
    ...
    const int oldx = m_bmpHeadInfo.biWidth;
    const int oldy = m_bmpHeadInfo.biHeight;
    ...
    if (!IsIndexed()) {
		...
		for(int y=0; y<oldy; y++){                                    //for all source rows
			m_stImgInfo.nProgress = (long)(100*y/oldy); if (m_stImgInfo.nEscape) break;
			ey += newy;                                                   
			...
			if (ey>=oldy) {                                                 //now when this happens
				...
				memset(accu, 0, newx * accuCellSize * sizeof(unsigned int));                   //clear accu
				destPtrS += newImage.m_stImgInfo.dwEffWidth;
			}//if (ey overflow)

			srcPtrS += m_stImgInfo.dwEffWidth;                                     //next round we start from new source row
		}//for y
    } else {
		...
		for(int y=0; y<oldy; y++){                                    //for all source rows
			...
			for(int x=0; x<oldx; x++){                                    //for all source columns
				...
				*(accuPtr+2) += rgb.rgbGreen;
				(*(accuPtr+3)) ++;
#if CXIMAGE_SUPPORT_ALPHA
				if (m_pAlpha) *(accuPtr+4) += rgb.rgbReserved;
#endif
				if (ex>oldx) {                                                //when we reach oldx, it's time to move to new slot
					accuPtr += accuCellSize;
					ex -= oldx;                                                   //(substract oldx from ex and resume from there on)
...
#if CXIMAGE_SUPPORT_ALPHA
					if (m_pAlpha) rgb.rgbReserved = (BYTE)(*(accuPtr+4) / *(accuPtr+3));
#endif
					newImage.SetPixelColor(dx, dy, rgb, m_pAlpha!=0);
					accuPtr += accuCellSize;
				}//for dx
				memset(accu, 0, newx * accuCellSize * sizeof(unsigned int));                   //clear accu
				dy++;
