...
bool CxImageWBMP::Decode(CxFile *hFile)
{
  ...
  cx_try
  {
	...
	m_bmpHeadInfo.biWidth = wbmpHead.ImageWidth;
	m_bmpHeadInfo.biHeight= wbmpHead.ImageHeight;

	if (m_bmpHeadInfo.biWidth<=0 || m_bmpHeadInfo.biHeight<=0)
		cx_throw("Corrupted WBMP");

	if (m_stImgInfo.nEscape == -1){
		m_stImgInfo.dwType = CXIMAGE_FORMAT_WBMP;
		return true;
	}

	Create(m_bmpHeadInfo.biWidth, m_bmpHeadInfo.biHeight, 1, CXIMAGE_FORMAT_WBMP);
	if (!IsValid()) cx_throw("WBMP Create failed");
	SetGrayPalette();

	int linewidth=(m_bmpHeadInfo.biWidth+7)/8;
...
bool CxImageWBMP::Encode(CxFile * hFile)
{
	...
	if (m_bmpHeadInfo.biBitCount!=1){
		strcpy(m_stImgInfo.szLastError,"Can't save this image as WBMP");
		return false;
	}
	...
	wbmpHead.ImageWidth=m_bmpHeadInfo.biWidth;
	wbmpHead.ImageHeight=m_bmpHeadInfo.biHeight;
