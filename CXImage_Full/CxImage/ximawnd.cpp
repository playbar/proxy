...
{
	if((m_pDib==0)||(pDC==0)||(!m_stImgInfo.bEnabled)) return 0;

	HBRUSH brImage = CreateDIBPatternBrushPt(m_pDib, DIB_RGB_COLORS);
	POINT pt;
	SetBrushOrgEx(pDC,x,y,&pt); //<RT>
	HBRUSH brOld = (HBRUSH) SelectObject(pDC, brImage);
	PatBlt(pDC, x, y, m_bmpHeadInfo.biWidth, m_bmpHeadInfo.biHeight, PATCOPY);
...
bool CxImage::CreateFromHANDLE(HANDLE hMem)
{
	...
	if (lpVoid){
		...
		memcpy(&m_bmpHeadInfo,pHead,sizeof(BITMAPINFOHEADER));
		//check if it's a top-down bitmap
		bool bTopDownDib = m_bmpHeadInfo.biHeight<0;
		if (bTopDownDib) m_bmpHeadInfo.biHeight=-m_bmpHeadInfo.biHeight;
		//create the image
		if(!Create(m_bmpHeadInfo.biWidth,m_bmpHeadInfo.biHeight,m_bmpHeadInfo.biBitCount)){
			GlobalUnlock(lpVoid);
			return false;
		}
		//preserve DPI
		SetXDPI((long)floor(m_bmpHeadInfo.biXPelsPerMeter * 254.0 / 10000.0 + 0.5));
		SetYDPI((long)floor(m_bmpHeadInfo.biYPelsPerMeter * 254.0 / 10000.0 + 0.5));
		...
		// <Michael Gandyra>
		// fill in color map
		bool bIsOldBmp = (m_bmpHeadInfo.biSize == sizeof(BITMAPCOREHEADER));
...
case 16 :
	{
		...
		// copy the pixels
		memcpy(m_stImgInfo.pImage, lpDIBBits + offset, m_bmpHeadInfo.biHeight*((m_bmpHeadInfo.biWidth+1)/2)*4);
		// transform into RGB
		Bitfield2RGB(m_stImgInfo.pImage, bf[0], bf[1], bf[2], 16);
	}
	break;
case 8 :
case 4 :
...
default :
	...
	for (int i = 0; i < status_byte; i++) {
		if ((BYTE*)(sline+bits) < (BYTE*)(m_stImgInfo.pImage+m_bmpHeadInfo.biSizeImage)){
			if (low_nibble) {
				if (i&1)
...
default :
	...
	break;
				...
				second_byte = *(lpDIBBits++);
				for (unsigned i = 0; i < status_byte; i++) {
					if ((DWORD)bits<m_stImgInfo.dwEffWidth){
...
HBITMAP CxImage::MakeBitmap(HDC hdc)
{
	...
	if (!hdc){
		...
		LPVOID pBit32;
		HBITMAP bmp = CreateDIBSection(hMemDC,(LPBITMAPINFO)m_pDib,DIB_RGB_COLORS, &pBit32, NULL, 0);
		if (pBit32) memcpy(pBit32, GetBits(), m_bmpHeadInfo.biSizeImage);
		DeleteDC(hMemDC);
		return bmp;
	}

	// this single line seems to work very well
	//HBITMAP bmp = CreateDIBitmap(hdc, (LPBITMAPINFOHEADER)m_pDib, CBM_INIT,
	//	GetBits(), (LPBITMAPINFO)pDib, DIB_RGB_COLORS);
	// this alternative works also with _WIN32_WCE
	LPVOID pBit32;
	HBITMAP bmp = CreateDIBSection(hdc, (LPBITMAPINFO)m_pDib, DIB_RGB_COLORS, &pBit32, NULL, 0);
	if (pBit32) memcpy(pBit32, GetBits(), m_bmpHeadInfo.biSizeImage);
...
bool CxImage::CreateFromHBITMAP(HBITMAP hbmp, HPALETTE hpal)
{
	...
	if (hbmp) { 
		...
		// copy the pixels
		if (GetDIBits(dc, hbmp, 0, m_bmpHeadInfo.biHeight, m_stImgInfo.pImage,
			(LPBITMAPINFO)m_pDib, DIB_RGB_COLORS) == 0){ //replace &head with m_pDib <Wil Stark>
				strcpy(m_stImgInfo.szLastError,"GetDIBits failed");
				::ReleaseDC(NULL, dc);
				return false;
		}
		::ReleaseDC(NULL, dc);
...
{
	if((m_pDib==0)||(hdc==0)||(cx==0)||(cy==0)||(!m_stImgInfo.bEnabled)) 
		return 0;

	if (cx < 0) 
		cx = m_bmpHeadInfo.biWidth;
	if (cy < 0)
		cy = m_bmpHeadInfo.biHeight;
	bool bTransparent = m_stImgInfo.nBkgndIndex >= 0;
	bool bAlpha = m_pAlpha != 0;
	...
	if (!(bTransparent || bAlpha || m_stImgInfo.bAlphaPaletteEnabled))
	{
		if (cx==m_bmpHeadInfo.biWidth && cy==m_bmpHeadInfo.biHeight)
		{ 
			//NORMAL
#if !defined (_WIN32_WCE)
			...
			SetDIBitsToDevice(hdc, x, y, cx, cy, 0, 0, 0, cy,
				m_stImgInfo.pImage,(BITMAPINFO*)m_pDib,DIB_RGB_COLORS);
		} 
		else 
		{ //STRETCH
			//pixel informations
			...
			if (pbase)
			{
				...
				float fx=(float)m_bmpHeadInfo.biWidth/(float)cx;
				float fy=(float)m_bmpHeadInfo.biHeight/(float)cy;

				for(yy=0;yy<desth;yy++)
				{
					dy = m_bmpHeadInfo.biHeight-(ymax-yy-y)*fy;
					sy = max(0L,(long)floor(dy));
					psrc = m_stImgInfo.pImage+sy*m_stImgInfo.dwEffWidth;
					pdst = pbase+yy*ew;
		...
		if (pbase)
		{
			...
			if (cx!=m_bmpHeadInfo.biWidth || cy!=m_bmpHeadInfo.biHeight){
				...
				for(yy=0;yy<desth;yy++){
					...
					alphaoffset = sy*m_bmpHeadInfo.biWidth;
					pdst = pbase + yy*ew;
					psrc = m_stImgInfo.pImage + sy*m_stImgInfo.dwEffWidth;
					...
					for(xx=0;xx<destw;xx++){
						...
						if (bAlpha) a=m_pAlpha[alphaoffset+sx]; else a=255;
						a =(BYTE)((a*(1+m_stImgInfo.nAlphaMax))>>8);
						...
						if (m_bmpHeadInfo.biClrUsed)
						{
							...
							if (m_stImgInfo.bAlphaPaletteEnabled){
						...
						if ((m_bmpHeadInfo.biClrUsed && ci!=cit) || (!m_bmpHeadInfo.biClrUsed && *pc!=*pct) || !bTransparent){
			...
			{
				//NORMAL
				iy=m_bmpHeadInfo.biHeight-ymax+y;
				for(yy=0;yy<desth;yy++,iy++)
				{
					alphaoffset=iy*m_bmpHeadInfo.biWidth;
					ix=xmin-x;
					pdst=pbase+yy*ew;
					ppix=m_stImgInfo.pImage+iy*m_stImgInfo.dwEffWidth+ix*3;
					for(xx=0;xx<destw;xx++,ix++)
					{

						if (bAlpha) a=m_pAlpha[alphaoffset+ix]; else a=255;
						a = (BYTE)((a*(1+m_stImgInfo.nAlphaMax))>>8);
...
{
	if((m_pDib==0)||(hdc==0)||(cx==0)||(cy==0)||(!m_stImgInfo.bEnabled)) return 0;
	if (cx < 0) cx = m_bmpHeadInfo.biWidth;
	if (cy < 0) cy = m_bmpHeadInfo.biHeight;
	bool bTransparent = (m_stImgInfo.nBkgndIndex >= 0);
	...
	if (!bTransparent){
		...
		SetStretchBltMode(hdc,COLORONCOLOR);	
#endif
		StretchDIBits(hdc, x, y, cx, cy, 0, 0, m_bmpHeadInfo.biWidth, m_bmpHeadInfo.biHeight,
			m_stImgInfo.pImage,(BITMAPINFO*)m_pDib, DIB_RGB_COLORS,SRCCOPY);
	} else {
		...
		if (RectVisible(hdc, &rcDst)){
			...
			int nWidth = m_bmpHeadInfo.biWidth;
			int nHeight = m_bmpHeadInfo.biHeight;
			...
			RGBQUAD rgbBG;
			if (m_bmpHeadInfo.biBitCount<24) rgbBG = GetPaletteColor((BYTE)m_stImgInfo.nBkgndIndex);
			else rgbBG = m_stImgInfo.nBkgndColor;
...
long CxImage::Stretch(HDC hdc, long xoffset, long yoffset, long xsize, long ysize, DWORD dwRop)
{
	if((m_pDib)&&(hdc)) {
		...
		StretchDIBits(hdc, xoffset, yoffset,
			xsize, ysize, 0, 0, m_bmpHeadInfo.biWidth, m_bmpHeadInfo.biHeight,
			m_stImgInfo.pImage,(BITMAPINFO*)m_pDib,DIB_RGB_COLORS,dwRop);
		return 1;
	}
	return 0;
}
...
long CxImage::Tile(HDC hdc, RECT *rc)
{
	if((m_pDib)&&(hdc)&&(rc)) {
		...
		int x,y,z;
		int bx=m_bmpHeadInfo.biWidth;
		int by=m_bmpHeadInfo.biHeight;
		for (y = 0 ; y < h ; y += by){
			if ((y+by)>h) by=h-y;
			z=bx;
			for (x = 0 ; x < w ; x += z){
...
long CxImage::LayerDrawAll(HDC hdc, long x, long y, long cx, long cy, RECT* pClipRect, bool bSmooth)
{
	...
	while(pLayer=GetLayer(n++)){
		if (pLayer->Draw(hdc,x+pLayer->m_stImgInfo.xOffset,y+pLayer->m_stImgInfo.yOffset,cx,cy,pClipRect,bSmooth)==0)
			return 0;
		if (pLayer->LayerDrawAll(hdc,x+pLayer->m_stImgInfo.xOffset,y+pLayer->m_stImgInfo.yOffset,cx,cy,pClipRect,bSmooth)==0)
			return 0;
	}
	return 1;
}
