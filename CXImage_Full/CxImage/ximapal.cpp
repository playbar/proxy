...
{
	return (m_bmpHeadInfo.biClrUsed * sizeof(RGBQUAD));
}
////////////////////////////////////////////////////////////////////////////////
void CxImage::SetPaletteColor(BYTE idx, BYTE r, BYTE g, BYTE b, BYTE alpha)
{
	if ((m_pDib)&&(m_bmpHeadInfo.biClrUsed)){
		BYTE* iDst = (BYTE*)(m_pDib) + sizeof(BITMAPINFOHEADER);
		if (idx<m_bmpHeadInfo.biClrUsed){
			long ldx=idx*sizeof(RGBQUAD);
			iDst[ldx++] = (BYTE) b;
			iDst[ldx++] = (BYTE) g;
			iDst[ldx++] = (BYTE) r;
			iDst[ldx] = (BYTE) alpha;
			m_stImgInfo.last_c_isvalid = false;
		}
	}
}
////////////////////////////////////////////////////////////////////////////////
...
{
	if ((m_pDib==NULL)||(m_bmpHeadInfo.biClrUsed==0)) return 0;

	if ((x<0)||(y<0)||(x>=m_bmpHeadInfo.biWidth)||(y>=m_bmpHeadInfo.biHeight)) {
		if (m_stImgInfo.nBkgndIndex >= 0)	return (BYTE)m_stImgInfo.nBkgndIndex;
		else return *m_stImgInfo.pImage;
	}
	if (m_bmpHeadInfo.biBitCount==8){
		return m_stImgInfo.pImage[y*m_stImgInfo.dwEffWidth + x];
	} else {
		BYTE pos;
		BYTE iDst= m_stImgInfo.pImage[y*m_stImgInfo.dwEffWidth + (x*m_bmpHeadInfo.biBitCount >> 3)];
		if (m_bmpHeadInfo.biBitCount==4){
			pos = (BYTE)(4*(1-x%2));
			iDst &= (0x0F<<pos);
			return (BYTE)(iDst >> pos);
		} else if (m_bmpHeadInfo.biBitCount==1){
			pos = (BYTE)(7-x%8);
			iDst &= (0x01<<pos);
			return (BYTE)(iDst >> pos);
		}
...
{
#ifdef _DEBUG
	if ((m_pDib==NULL) || (m_bmpHeadInfo.biClrUsed==0) || !IsInside(x,y))
...
{
//	RGBQUAD rgb={0,0,0,0};
	RGBQUAD rgb=m_stImgInfo.nBkgndColor; //<mpwolski>
	if ((m_pDib==NULL)||(x<0)||(y<0)||
		(x>=m_bmpHeadInfo.biWidth)||(y>=m_bmpHeadInfo.biHeight)){
		if (m_stImgInfo.nBkgndIndex >= 0){
			if (m_bmpHeadInfo.biBitCount<24) return GetPaletteColor((BYTE)m_stImgInfo.nBkgndIndex);
			else return m_stImgInfo.nBkgndColor;
		} else if (m_pDib) return GetPixelColor(0,0);
		return rgb;
	}
	...
	if (m_bmpHeadInfo.biClrUsed){
		...
		BYTE* iDst  = m_stImgInfo.pImage + y*m_stImgInfo.dwEffWidth + x*3;
		rgb.rgbBlue = *iDst++;
		rgb.rgbGreen= *iDst++;
		rgb.rgbRed  = *iDst;
	}
...
{
#ifdef _DEBUG
	if ((m_pDib==NULL)||(m_bmpHeadInfo.biClrUsed==0)||
		(x<0)||(y<0)||(x>=m_bmpHeadInfo.biWidth)||(y>=m_bmpHeadInfo.biHeight))
  #if CXIMAGE_SUPPORT_EXCEPTION_HANDLING
		throw 0;
  #else
		return;
	...
	if (m_bmpHeadInfo.biBitCount==8){
		m_stImgInfo.pImage[y*m_stImgInfo.dwEffWidth + x]=i;
		return;
	} else {
		BYTE pos;
		BYTE* iDst= m_stImgInfo.pImage + y*m_stImgInfo.dwEffWidth + (x*m_bmpHeadInfo.biBitCount >> 3);
...
{
	if ((m_pDib==NULL)||(m_bmpHeadInfo.biClrUsed==0)||
		(x<0)||(y<0)||(x>=m_bmpHeadInfo.biWidth)||(y>=m_bmpHeadInfo.biHeight)) return ;
...
{
	if ((m_pDib==NULL)||(x<0)||(y<0)||
		(x>=m_bmpHeadInfo.biWidth)||(y>=m_bmpHeadInfo.biHeight)) return;
	if (m_bmpHeadInfo.biClrUsed)
		BlindSetPixelIndex(x,y,GetNearestIndex(c));
	else {
		BYTE* iDst = m_stImgInfo.pImage + y*m_stImgInfo.dwEffWidth + x*3;
...
BYTE CxImage::GetNearestIndex(RGBQUAD c)
{
	...
	if (m_stImgInfo.last_c_isvalid && (*(long*)&m_stImgInfo.last_c == *(long*)&c)) return m_stImgInfo.last_c_index;
	m_stImgInfo.last_c = c;
	m_stImgInfo.last_c_isvalid = true;

	BYTE* iDst = (BYTE*)(m_pDib) + sizeof(BITMAPINFOHEADER);
	long distance=200000;
	int i,j = 0;
	long k,l;
	int m = (int)(m_bmpHeadInfo.biClrImportant==0 ? m_bmpHeadInfo.biClrUsed : m_bmpHeadInfo.biClrImportant);
...
//		k = abs(iDst[l]-c.rgbBlue)+abs(iDst[l+1]-c.rgbGreen)+abs(iDst[l+2]-c.rgbRed);
	...
	m_stImgInfo.last_c_index = (BYTE)j;
	return (BYTE)j;
}
////////////////////////////////////////////////////////////////////////////////
/**
...
{
	if (buffer && (m_bmpHeadInfo.biClrUsed==0)){
		BYTE temp;
		length = min(length,(int)m_stImgInfo.dwEffWidth);
		length = min(length,(int)(3*m_bmpHeadInfo.biWidth));
...
{
	if ((!r)||(m_pDib==NULL)||(m_bmpHeadInfo.biClrUsed==0)) return;
	if (!g) g = r;
	if (!b) b = g;
	RGBQUAD* ppal=GetPalette();
	DWORD m=min(n,m_bmpHeadInfo.biClrUsed);
	for (DWORD i=0; i<m;i++){
		ppal[i].rgbRed=r[i];
		ppal[i].rgbGreen=g[i];
		ppal[i].rgbBlue=b[i];
...
{
	if ((!rgb)||(m_pDib==NULL)||(m_bmpHeadInfo.biClrUsed==0)) return;
	RGBQUAD* ppal=GetPalette();
	DWORD m=min(nColors,m_bmpHeadInfo.biClrUsed);
	for (DWORD i=0; i<m;i++){
...
{
	if ((pPal==NULL)||(m_pDib==NULL)||(m_bmpHeadInfo.biClrUsed==0)) return;
...
{
	if ((m_pDib==NULL)||(m_bmpHeadInfo.biClrUsed==0)) return;
	RGBQUAD* pal=GetPalette();
	for (DWORD ni=0;ni<m_bmpHeadInfo.biClrUsed;ni++)
		pal[ni].rgbBlue=pal[ni].rgbGreen = pal[ni].rgbRed = (BYTE)(ni*(255/(m_bmpHeadInfo.biClrUsed-1)));
}
////////////////////////////////////////////////////////////////////////////////
/**
...
void CxImage::BlendPalette(COLORREF cr,long perc)
{
	...
	for(i=0;i<m_bmpHeadInfo.biClrUsed;i++){
...
{
	RGBQUAD* ppal=GetPalette();
	if(!(m_pDib && ppal && m_bmpHeadInfo.biClrUsed)) return false;
...
void CxImage::SwapRGB2BGR()
{
	...
	if (m_bmpHeadInfo.biClrUsed){
		...
		BYTE b;
		if(!ppal) return;
		for(WORD a=0;a<m_bmpHeadInfo.biClrUsed;a++){
			b=ppal[a].rgbBlue; ppal[a].rgbBlue=ppal[a].rgbRed; ppal[a].rgbRed=b;
		}
	} else {
		for(long y=0;y<m_bmpHeadInfo.biHeight;y++){
			RGBtoBGR(GetBits(y),3*m_bmpHeadInfo.biWidth);
		}
	}
}
////////////////////////////////////////////////////////////////////////////////
bool CxImage::IsTransparent(long x, long y)
{
	...
	if (m_stImgInfo.nBkgndIndex>=0){
		if (m_bmpHeadInfo.biClrUsed){
			if (GetPixelIndex(x,y) == m_stImgInfo.nBkgndIndex) return true;
		} else {
			RGBQUAD ct = m_stImgInfo.nBkgndColor;
...
#if CXIMAGE_SUPPORT_ALPHA
	if (m_pAlpha) return AlphaGet(x,y)==0;
#endif
...
bool CxImage::GetTransparentMask(CxImage* iDst)
{
	...
	CxImage tmp;
	tmp.Create(m_bmpHeadInfo.biWidth, m_bmpHeadInfo.biHeight, 1, GetType());
	tmp.SetStdPalette();
	tmp.Clear(0);
...
{
	if (m_bmpHeadInfo.biClrUsed != img.m_bmpHeadInfo.biClrUsed)
		return false;
	if (m_bmpHeadInfo.biClrUsed == 0)
		return false;

	RGBQUAD c1,c2;
	for (DWORD n=0; n<m_bmpHeadInfo.biClrUsed; n++){
...
{
	return m_bmpHeadInfo.biClrImportant;
}
////////////////////////////////////////////////////////////////////////////////
/**
...
void CxImage::SetClrImportant(DWORD ncolors)
{
	if (ncolors==0 || ncolors>256) {
		m_bmpHeadInfo.biClrImportant = 0;
		return;
	}

	switch(m_bmpHeadInfo.biBitCount){
	case 1:
		m_bmpHeadInfo.biClrImportant = min(ncolors,2);
		break;
	case 4:
		m_bmpHeadInfo.biClrImportant = min(ncolors,16);
		break;
	case 8:
		m_bmpHeadInfo.biClrImportant = ncolors;
		break;
	}
	return;
}
...
#endif
  if (!IsIndexed())
    return m_stImgInfo.pImage + y*m_stImgInfo.dwEffWidth + x*3;
  else
    return 0;
}
////////////////////////////////////////////////////////////////////////////////
