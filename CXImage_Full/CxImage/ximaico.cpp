...
bool CxImageICO::Decode(CxFile *hFile)
{
	...
	int	page=m_stImgInfo.nFrame;	//internal icon structure indexes
	...
	if ((icon_header.idReserved == 0) && ((icon_header.idType == 1)||(icon_header.idType == 2))) {
		...
		m_stImgInfo.nNumFrames = icon_header.idCount;
		...
		if ((page>=0)&&(page<icon_header.idCount)){
			...
			if (m_stImgInfo.nEscape == -1) {
				...
				m_bmpHeadInfo.biWidth = icon_list[page].bWidth;
				m_bmpHeadInfo.biHeight = icon_list[page].bHeight;
#if CXIMAGE_SUPPORT_PNG
				if (m_bmpHeadInfo.biWidth==0 && m_bmpHeadInfo.biHeight==0)
				{	// Vista icon support
				...
				free(icon_list);
				m_stImgInfo.dwType = CXIMAGE_FORMAT_ICO;
				return true;
			}
			...
			{	// standard icon
				...
				if (bih.biClrUsed)
					...
					hFile->Read(pal,m_bmpHeadInfo.biClrUsed*sizeof(RGBQUAD), 1);

				SetPalette(pal,m_bmpHeadInfo.biClrUsed);	//palette assign

				//read the icon
				if (c<=24){
					hFile->Read(m_stImgInfo.pImage, m_bmpHeadInfo.biSizeImage, 1);
				} else { // 32 bit icon
					BYTE* buf=(BYTE*)malloc(4*m_bmpHeadInfo.biHeight*m_bmpHeadInfo.biWidth);
					BYTE* src = buf;
					hFile->Read(buf, 4*m_bmpHeadInfo.biHeight*m_bmpHeadInfo.biWidth, 1);
				...
				int maskwdt = ((m_bmpHeadInfo.biWidth+31) / 32) * 4;	//line width of AND mask (always 1 Bpp)
				int masksize = m_bmpHeadInfo.biHeight * maskwdt;				//size of mask
				...
				if (hFile->Read(mask, masksize, 1)){
					...
					if (bGoodMask){
						...
						if (c <= 8){ // only for icons with less than 256 colors (XP icons need alpha).
							...
							int iTransIdx = -1;
							for (x = (int)(m_bmpHeadInfo.biClrUsed-1); x>=0 ; x--){
								if (colorsUsed[x] == 0){
...
bool CxImageICO::Encode(CxFile * hFile, CxImage ** pImages, int nPageCount)
{
  cx_try
  {
	...
	for (i=0; i<nPageCount; i++){	//write headers
		...
		ghost.m_stImgInfo.nNumFrames = nPageCount;
		if (i==0) {
...
bool CxImageICO::Encode(CxFile * hFile, bool bAppend, int nPageCount)
{
	...
	//check format limits
	if ((m_bmpHeadInfo.biWidth>255)||(m_bmpHeadInfo.biHeight>255)){
		strcpy(m_stImgInfo.szLastError,"Can't save this image as icon");
		return false;
	}
#endif
	...
	if (m_bmpHeadInfo.biBitCount<=8 && pal==NULL) return false;

	int maskwdt=((m_bmpHeadInfo.biWidth+31)/32)*4; //mask line width
	int masksize=m_bmpHeadInfo.biHeight * maskwdt; //size of mask
	int bitcount=m_bmpHeadInfo.biBitCount;
	int imagesize=m_bmpHeadInfo.biSizeImage;
#if CXIMAGE_SUPPORT_ALPHA
	if (AlphaIsValid() && m_bmpHeadInfo.biClrUsed==0){
		bitcount=32;
		imagesize=4*m_bmpHeadInfo.biHeight*m_bmpHeadInfo.biWidth;
	}
#endif
	...
	DWORD dwBytesInRes = sizeof(BITMAPINFOHEADER)+m_bmpHeadInfo.biClrUsed*sizeof(RGBQUAD)+imagesize+masksize;

	ICONDIRENTRY icon_list={
		(BYTE)m_bmpHeadInfo.biWidth,
		(BYTE)m_bmpHeadInfo.biHeight,
		(BYTE)m_bmpHeadInfo.biClrUsed,
		0, 0,
		(WORD)bitcount,
		dwBytesInRes,
		m_dwImageOffset
	...
	BITMAPINFOHEADER bi={
		...
		m_bmpHeadInfo.biWidth,
		2*m_bmpHeadInfo.biHeight,
		1,
		(WORD)bitcount,
		0, imagesize,
		0, 0, 0, 0
	...
	if ((bAppend && nPageCount==m_stImgInfo.nNumFrames) || (!bAppend && nPageCount==0)){
	...
	if ((bAppend && nPageCount<m_stImgInfo.nNumFrames) || (!bAppend && nPageCount==0))
	{
		...
		{	// standard icon
			...
			if (pal){
				if (bTransparent) SetPaletteColor((BYTE)m_stImgInfo.nBkgndIndex,0,0,0,0);
			 	hFile->Write(pal,m_bmpHeadInfo.biClrUsed*sizeof(RGBQUAD),1); //write palette
				if (bTransparent) SetPaletteColor((BYTE)m_stImgInfo.nBkgndIndex,ct);
			}
			...
			if (AlphaIsValid() && m_bmpHeadInfo.biClrUsed==0){
				...
				hFile->Write(m_stImgInfo.pImage,imagesize,1);	//write image
			}
#else
			hFile->Write(m_stImgInfo.pImage,imagesize,1);	//write image
#endif
			...
			for (int y = 0; y < m_bmpHeadInfo.biHeight; y++) {
				for (int x = 0; x < m_bmpHeadInfo.biWidth; x++) {
					i=0;
#if CXIMAGE_SUPPORT_ALPHA
