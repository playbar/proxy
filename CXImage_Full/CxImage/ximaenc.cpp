...
bool CxImage::EncodeSafeCheck(CxFile *hFile)
{
	if (hFile==NULL)
	{
		strcpy(m_stImgInfo.szLastError,CXIMAGE_ERR_NOFILE);
		return true;
	}

	if (m_pDib==NULL)
	{
		strcpy(m_stImgInfo.szLastError,CXIMAGE_ERR_NOIMAGE);
		return true;
	}
	return false;
}
...
{
	if (buffer!=NULL){
		strcpy(m_stImgInfo.szLastError,"the buffer must be empty");
		return false;
	}
	CxMemFile file;
	file.Open();
...
bool CxImage::Encode(CxFile *hFile, DWORD imagetype)
{
	...
	if (imagetype==CXIMAGE_FORMAT_BMP)
	{
		...
		{
			strcpy(m_stImgInfo.szLastError,newima.GetLastError());
			return false;
		}
	}
#endif
	...
	strcpy(m_stImgInfo.szLastError,"Encode: Unknown format");
	return false;
}
////////////////////////////////////////////////////////////////////////////////
/**
...
#endif
	strcpy(m_stImgInfo.szLastError,"Multipage Encode, Unsupported operation for this format");
	return false;
}
...
bool CxImage::Encode2RGBA(CxFile *hFile, bool bFlipY)
{
	...
	for (long y1 = 0; y1 < m_bmpHeadInfo.biHeight; y1++) {
		long y = bFlipY ? m_bmpHeadInfo.biHeight - 1 - y1 : y1;
...
//bool CxImage::Load(const char * filename, DWORD imagetype)
{
	...
	char szError[256];
	strcpy(szError,m_stImgInfo.szLastError); //save the first error
	...
	if (!bOK && imagetype > 0) strcpy(m_stImgInfo.szLastError,szError); //restore the first error
...
bool CxImage::LoadResource(HRSRC hRes, DWORD imagetype, HMODULE hModule)
{
	...
	} else strcpy(m_stImgInfo.szLastError,"Unable to load resource!");
	return false;
}
#endif //WIN32
////////////////////////////////////////////////////////////////////////////////
...
bool CxImage::Decode(CxFile *hFile, DWORD imagetype)
{
	...
	if (imagetype==CXIMAGE_FORMAT_ICO){
		...
		if (newima.Decode(hFile)){
			...
			m_stImgInfo.nNumFrames = newima.m_stImgInfo.nNumFrames;
			strcpy(m_stImgInfo.szLastError,newima.GetLastError());
			return false;
		}
	}
	...
	strcpy(m_stImgInfo.szLastError,"Decode: Unknown or wrong format");
	return false;
}
////////////////////////////////////////////////////////////////////////////////
/**
...
bool CxImage::CheckFormat(BYTE * buffer, DWORD size, DWORD imagetype)
{
	if (buffer==NULL || size==NULL){
		strcpy(m_stImgInfo.szLastError,"invalid or empty buffer");
		return false;
	}
	CxMemFile file(buffer,size);
