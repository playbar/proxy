...
bool CxImage::HistogramStretch(long method, double threshold)
{
	...
	double dbScaler = 50.0/m_bmpHeadInfo.biHeight;
	long x,y;
  ...
  if ((m_bmpHeadInfo.biBitCount==8) && IsGrayScale()){
	...
	for (y=0; y<m_bmpHeadInfo.biHeight; y++)	{
		if (m_stImgInfo.nEscape) break;
		m_stImgInfo.nProgress = (long)(50.0+y*dbScaler);
		for (x=0; x<m_bmpHeadInfo.biWidth; x++)
		{
...
bool CxImage::HistogramEqualize()
{
	...
	memset( &equalize_map, 0, sizeof(int) * 256 );
     ...
     // form histogram
	for(y=0; y < m_bmpHeadInfo.biHeight; y++){
		m_stImgInfo.nProgress = (long)(50*y/m_bmpHeadInfo.biHeight);
		if (m_stImgInfo.nEscape) break;
		for(x=0; x < m_bmpHeadInfo.biWidth; x++){
			color = BlindGetPixelColor( x, y );
	...
	if(m_bmpHeadInfo.biClrUsed == 0){ // No Palette
		for( y = 0; y < m_bmpHeadInfo.biHeight; y++ ){
			m_stImgInfo.nProgress = (long)(50+50*y/m_bmpHeadInfo.biHeight);
			if (m_stImgInfo.nEscape) break;
			for( x = 0; x < m_bmpHeadInfo.biWidth; x++ ){
		...
		for( i = 0; i < (int)m_bmpHeadInfo.biClrUsed; i++ ){
...
bool CxImage::HistogramNormalize()
{
	...
	threshold_intensity = ( m_bmpHeadInfo.biWidth * m_bmpHeadInfo.biHeight) / 100;
