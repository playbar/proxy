...
bool CxImagePCX::Decode(CxFile *hFile)
{
  ...
  cx_try
  {
	...
	m_stImgInfo.xDPI = pcxHeader.Hres;
	m_stImgInfo.yDPI = pcxHeader.Vres;

	if (m_stImgInfo.nEscape == -1){
		m_bmpHeadInfo.biWidth = Width;
		m_bmpHeadInfo.biHeight= Height;
		m_stImgInfo.dwType = CXIMAGE_FORMAT_PCX;
		return true;
	}
	...
	for (DWORD idx=0; idx<m_bmpHeadInfo.biClrUsed; idx++) SetPaletteColor((BYTE)idx,ColorMap[idx][0],ColorMap[idx][1],ColorMap[idx][2]);
...
bool CxImagePCX::Encode(CxFile * hFile)
{
  ...
  cx_try
  {
	...
	pcxHeader.Xmin = 0;
	pcxHeader.Ymin = 0;
	pcxHeader.Xmax = (WORD)m_bmpHeadInfo.biWidth-1;
	pcxHeader.Ymax = (WORD)m_bmpHeadInfo.biHeight-1;
	pcxHeader.Hres = (WORD)m_stImgInfo.xDPI;
	pcxHeader.Vres = (WORD)m_stImgInfo.yDPI;
	pcxHeader.Reserved = 0;
	pcxHeader.PaletteType = m_bmpHeadInfo.biClrUsed==0;
	...
	case 8:
		{
			...
			pcxHeader.ColorPlanes = m_bmpHeadInfo.biClrUsed==0 ? 3 : 1;
#if CXIMAGE_SUPPORT_ALPHA
			if (AlphaIsValid() && m_bmpHeadInfo.biClrUsed==0) pcxHeader.ColorPlanes =4;
#endif //CXIMAGE_SUPPORT_ALPHA
			pcxHeader.BytesPerLine = (WORD)m_bmpHeadInfo.biWidth;
			break;
		}
	default: //(4 1)
		pcxHeader.BitsPerPixel = 1;
		pcxHeader.ColorPlanes = m_bmpHeadInfo.biClrUsed==16 ? 4 : 1;
		pcxHeader.BytesPerLine = (WORD)((m_bmpHeadInfo.biWidth * pcxHeader.BitsPerPixel + 7)>>3);
	}
	...
	if (m_bmpHeadInfo.biClrUsed==0){
		for (y = m_bmpHeadInfo.biHeight-1; y >=0 ; y--){
			for (int p=0; p<pcxHeader.ColorPlanes; p++){
				...
				PCX_PackPixels(-1-(m_bmpHeadInfo.biWidth&0x1),c,n,buffer);
			}
		}

		hFile->Write(buffer.GetBuffer(false),buffer.Tell(),1);

	} else if (m_bmpHeadInfo.biBitCount==8) {
		...
		bool binvert = false;
		if (CompareColors(&rgb[0],&rgb[1])>0) binvert=(m_bmpHeadInfo.biBitCount==1);
		
		BYTE* plane = (BYTE*)malloc(pcxHeader.BytesPerLine);
		BYTE* raw = (BYTE*)malloc(m_bmpHeadInfo.biWidth);

		for(y = m_bmpHeadInfo.biHeight-1; y >=0 ; y--) {

			for( x = 0; x < m_bmpHeadInfo.biWidth; x++)	raw[x] = (BYTE)GetPixelIndex(x,y);

			if (binvert) for( x = 0; x < m_bmpHeadInfo.biWidth; x++)	raw[x] = 1-raw[x];

			for( x = 0; x < pcxHeader.ColorPlanes; x++ ) {
				PCX_PixelsToPlanes(raw, m_bmpHeadInfo.biWidth, plane, x);
