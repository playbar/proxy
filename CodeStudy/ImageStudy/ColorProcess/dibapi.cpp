 ...
 * 
 ************************************************************************/ 
void CorrectGamma(int nGammaArray[], BYTE *r, BYTE *g, BYTE *b)
{
	*r = nGammaArray[*r];
	*g = nGammaArray[*g];
	*b = nGammaArray[*b];
 ...
 * 
 ************************************************************************/ 
BOOL CorrectDIBGamma(HDIB hDib, float fGamma)
{
	if (hDib == NULL)
		return FALSE;
	BITMAPINFO *bmi = (BITMAPINFO *)GlobalLock(hDib);
	...
	// create Gamma Array
	int nGammaArray[256]; 		// nGammaArray为Gamma数组
	fGamma = (float)max(0.1, min(5.0, fGamma));
	double g = 1 / fGamma; 		// fGamma为给定的Gamma值
	for (int i = 0; i < 256; i++)
	{
		nGammaArray [i] = (int)min(255, (int)(pow(i/255.0, g) * 255 + 0.5));
	}
	...
	else		// No palette
	{
		...
		for (int ny=0; ny<lpbi->biHeight; ny++)
		{
			for (int nx=0; nx<lpbi->biWidth; nx++)
			{
				...
				g = (BYTE)*(lpBits+1);
				r = (BYTE)*(lpBits+2);
				CorrectGamma(nGammaArray, &r, &g, &b);
				*lpBits++ = b;
				*lpBits++ = g;
				*lpBits++ = r;
			}
 ...
 *		Exposure pixel
 ************************************************************************/ 
void Exposure(BYTE threshold, BYTE *r, BYTE *g, BYTE *b)
{
	if (*r < threshold)
		*r = 255 - *r;
	if (*g < threshold)
 ...
 *		Exposure DIB
 ************************************************************************/ 
BOOL ExposureDIB(HDIB hDib, BYTE threshold)
{
	if (hDib == NULL)
		return FALSE;
	BITMAPINFO *bmi = (BITMAPINFO *)GlobalLock(hDib);
