...
BOOL CDib::CorrectGamma(float fGamma)
{
	if (! CorrectDIBGamma(m_hDib, fGamma))
		return FALSE;
...
BOOL CDib::Exposure(BYTE threshold)
{
	if (! ExposureDIB(m_hDib, threshold))
		return FALSE;
...
BOOL CDib::Embossment(int nBackground)
{
	if (IsEmpty())
		return FALSE;

	if (! EmbossmentDIB(m_hDib, nBackground))
		return FALSE;
...
BOOL CDib::Spread(int nBlocksize)
{
	...
	if (! SpreadDIB(m_hDib, nBlocksize))
		return FALSE;
...
BOOL CDib::EdegeDetect(int nAlgorithm)
{
	if (IsEmpty())
		return FALSE;

	if (! EdgeDetectDIB(m_hDib, nAlgorithm))
		return FALSE;
...
BOOL CDib::HomogenityEdegeDetect()
{
	if (IsEmpty())
		return FALSE;

	if (! HomogenityEdegeDetectDIB(m_hDib))
		return FALSE;
...
BOOL CDib::DifferenceEdegeDetect()
{
	if (IsEmpty())
		return FALSE;

	if (! DifferenceEdegeDetectDIB(m_hDib))
		return FALSE;
...
BOOL CDib::CannyEdegeDetect()
{
	if (IsEmpty())
		return FALSE;

	if (! CannyEdegeDetectDIB(m_hDib))
		return FALSE;
