...
int Game_Init(void *parms)
{
	...
	//obj_flat_cube.texture->buffer = bitmap.buffer;
	//obj_flat_cube.texture->height = bitmap.bitmapinfoheader.biHeight;
	//obj_flat_cube.texture->width  = bitmap.bitmapinfoheader.biWidth;
...
int Game_Main(void *parms)
{
	...
	DDraw_Fill_Surface(lpddsback, RGB16Bit(0,0,0));
	...
	obj_flat_cube.texture->buffer = g_pFlashData;
	...
	USHORT *lpBack_buffer = (USHORT *) back_buffer;*/

	USHORT *lpBack_buffer = (USHORT *) back_buffer;
	//USHORT * temp = (USHORT *)back_buffer;

	//UCHAR * lpChar = back_buffer ;
	//*
	for (int y = 0; y < nHeight; y++) 
	{
		//temp = temp + (back_lpitch >> 1);
		//lpChar = lpChar + back_lpitch;
		//memcpy(video_buffer+ (y*lpitch<<1) , lpBuffer, nWidth<<1 );
		for (int x = 0; x < nWidth; x ++)
		{
			UCHAR blue  = g_pFlashData[ y*nWidth*4 + x*4 + 0 ] >>3;
			UCHAR green = g_pFlashData[ y*nWidth*4 + x*4 + 1 ] >>2;
			UCHAR red   = g_pFlashData[ y*nWidth*4 + x*4 + 2 ] >>3;
			USHORT pixel = _RGB16BIT565(red, green, blue );
			lpBack_buffer[x + (y * back_lpitch >> 1)] = pixel;
			//lpChar[2 * x] = pixel ;
			//lpChar[2 * x + 1] = pixel >> 8; 
			//*(USHORT *)lpChar = pixel;
			//lpChar[x ] = pixel; 
			//lpBuffer += nWidth << 2;
		...
		//lpChar += back_lpitch;
		//CopyMemory(video_buffer + y*lpitch16, pdwFlashData + y*g_flashSprite.m_nWidth, g_flashSprite.m_nWidth);
	}
	//*/
