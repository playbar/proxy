...
HRESULT CDirectXVideoTexture::CreateFromAVIFile( TCHAR *pFileName)
{
	char sAppPath[MAX_PATH];
	if (strlen(pFileName) >=2 && pFileName[1] == ':')
	{
		strcpy(sAppPath, pFileName);
	}
	else
	{
	...
	MultiByteToWideChar(CP_ACP, 0, sAppPath, -1, wFileName, MAX_PATH);
