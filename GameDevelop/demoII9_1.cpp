...
int Game_Init(void *parms)
{
	...
	face.texture = new BITMAP_IMAGE_TYP();
...
int Game_Main(void *parms)
{
	...
	face.tvlist[0].x = 1;
	face.tvlist[0].y = 1;
	face.lit_color[0] = RGB16Bit(255,0,0);

	face.tvlist[1].x = 400;
	face.tvlist[1].y = 1;
	face.lit_color[1] = RGB16Bit(0,255,0);

	face.tvlist[2].x = 400;
	face.tvlist[2].y = 400;
	face.lit_color[2] = RGB16Bit(0,0,255);

	face.texture->buffer = bitmap.buffer;
	face.texture->height = bitmap.bitmapinfoheader.biHeight;
	face.texture->width  = bitmap.bitmapinfoheader.biWidth;

 
	// draw the gouraud shaded triangle
	Draw_Gouraud_Triangle16(&face, back_buffer, back_lpitch);

	//Draw_Textured_Triangle16( &face, back_buffer, back_lpitch);
