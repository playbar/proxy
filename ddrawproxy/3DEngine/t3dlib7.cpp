	...
	for (int poly=0; poly < rend_list->num_polys; poly++)
	{
		...
		if (rend_list->poly_ptrs[poly]->attr & POLY4DV2_ATTR_SHADE_MODE_TEXTURE)
		{
			...
			else
			{
				...
				face.lit_color[0] = 0xffff;
				Draw_Textured_TriangleFS16_My(&face, video_buffer, lpitch);
			} // end else
...
void Draw_Textured_TriangleFS16_My(POLYF4DV2_PTR face,   // ptr to face
								UCHAR *_dest_buffer,   // pointer to video buffer
								int mem_pitch)        // bytes per line, 320, 640 etc.
{
	// this function draws a textured triangle in 16-bit mode with flat shading
	...
	UCHAR *textmap     = NULL;
	textmap = (UCHAR *) face->texture->buffer;
				...
				(x2 < min_clip_x) || (x2 > max_clip_x))
			{
				...
				for (yi = ystart; yi<=yend; yi++)
				{
					...
					for (xi=xstart; xi<=xend; xi++)
					{
						...
						// get textel first
						//textel = textmap[(ui >> FIXP16_SHIFT) + ((vi >> FIXP16_SHIFT) << texture_shift2)];
						int uii = ui >> FIXP16_SHIFT;
						int vii = vi >> FIXP16_SHIFT;
						UCHAR blue  = textmap[(128 - vii)*128*4 + uii*4  + 0 ] >>3;
						UCHAR green = textmap[(128 - vii)*128*4 + uii*4  + 1 ] >>2;
						UCHAR red   = textmap[(128 - vii)*128*4 + uii*4  + 2 ] >>3;

						USHORT pixel = _RGB16BIT565(red, green, blue );

						// extract rgb components
						r_textel  = ((pixel >> 11)       ); 
						g_textel  = ((pixel >> 5)  & 0x3f); 
						b_textel =   (pixel        & 0x1f);
			...
			else
			{
				...
				for (yi = ystart; yi<=yend; yi++)
				{
					...
					for (xi=xstart; xi<=xend; xi++)
					{
						...
						//int ii = (ui >> FIXP16_SHIFT) + ((vi >> FIXP16_SHIFT) << (texture_shift2 + 2));
						int uii = ui >> FIXP16_SHIFT;
						int vii = vi >> FIXP16_SHIFT;
						UCHAR blue  = textmap[(128 - vii)*128*4 + uii*4  + 0 ] >>3;
						UCHAR green = textmap[(128 - vii)*128*4 + uii*4  + 1 ] >>2;
			...
			else
			{
				...
				for (yi = ystart; yi<=yend; yi++)
				{
					...
					for (xi=xstart; xi<=xend; xi++)
					{
						...
						screen_ptr[xi] = ((ui >> (FIXP16_SHIFT+3)) << 11) + ((vi >> (FIXP16_SHIFT+2)) << 5) + (wi >> (FIXP16_SHIFT+3));   
						//
						/*int r = (ui >> (FIXP16_SHIFT+3)) << 11;
						int g = (vi >> (FIXP16_SHIFT+2)) << 5 ;
						int b = (wi >> (FIXP16_SHIFT+3));
						USHORT temp = r + g + b;
						screen_ptr[xi] = temp;*/
