...
void CMeshLibrary::Render( DWORD id, D3DXMATRIX* pmatWorld, LPDIRECT3DTEXTURE8 pAdvTex,  char* url )
{
	//先判断是不是没有的资源
	...
	m_pMesh->Update(GetIGA3DModule()->m_ElapseTime * 5, true );
	...
	//添加的拾取代码
		...
		if(sStatus < 0 )
		{
			if( !bIsPick )
			{
				...
				{
					//得到屏幕的大小
					IDirect3DSurface8 *pkD3DSurf;
					HRESULT hRes = m_pd3dDevice->GetBackBuffer(0, D3DBACKBUFFER_TYPE_MONO, &pkD3DSurf);
...
{
	//加载这个资源
	FxSkeletalMesh* pSkinMesh = new FxSkeletalMesh(m_pd3dDevice);
	...
	bool bIsLoad = pSkinMesh->LoadFromXFile(acXFileFullName, acXFilePath, 0, D3DXMESH_SYSTEMMEM);
	if( !bIsLoad )
	{
	...
	//OutputDebugString("CSkinMeshLibrary:加载文件成功！");
