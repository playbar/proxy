...
protected:
    struct STATE
    {
        IDirect3D8*          m_D3D;                     // the main D3D object

        IDirect3DDevice8*    m_D3DDevice;               // the D3D rendering device
        ...
        D3DCAPS8             m_Caps;                    // D3D caps for current device
...
public:
    ...
    GET_SET_ACCESSOR( IDirect3D8*, D3D );

    GET_SET_ACCESSOR( IDirect3DDevice8*, D3DDevice );
    ...
    GETP_SETP_ACCESSOR( D3DCAPS8, Caps );
...
{
	GetDXUTState().SetInsideMainloop( bState );
}
...
HRESULT DXUTChangeDevice( DXUTDeviceSettings* pNewDeviceSettings, IDirect3DDevice8* pd3dDeviceFromApp, bool bForceRecreate, bool bClipWindowToSingleAdapter );
void    DXUTUpdateDeviceSettingsWithOverrides( DXUTDeviceSettings* pNewDeviceSettings );
HRESULT DXUTCreate3DEnvironment( IDirect3DDevice8* pd3dDeviceFromApp );
...
void    DXUTUpdateDeviceStats( D3DDEVTYPE DeviceType, DWORD BehaviorFlags, D3DADAPTER_IDENTIFIER8* pAdapterIdentifier );
...
HRESULT DXUTSetDeviceCursor( IDirect3DDevice8* pd3dDevice, HCURSOR hCursor, bool bAddWatermark );
...
HRESULT DXUTInit( bool bParseCommandLine, bool bHandleDefaultHotkeys, bool bShowMsgBoxOnError, bool bHandleAltEnter )
{
    ...
    //////IDirect3D8* pD3D = DXUTGetD3DObject();
    //////if( pD3D == NULL )
    //////{
    ...
    //////    pD3D = DXUT_Dynamic_Direct3DCreate8( D3D_SDK_VERSION );
    ...
    //////    return DXUT_ERR( L"Direct3DCreate8", DXUTERR_NODIRECT3D );
    //////}
...
HRESULT DXUTSetWindow( HWND hWndFocus, HWND hWndDeviceFullScreen, HWND hWndDeviceWindowed, bool bHandleMessages )
{
    ...
    if( bHandleMessages )
    {
		//g_hWndHook	= hWndFocus;
		g_HookerMsg.SetHookWnd( hWndFocus );
		if( g_HookerMsg.GetHook() == NULL)
		{
			//	g_hHook	= ::SetWindowsHookEx(WH_CALLWNDPROC, HookWindowProc, NULL, ::GetCurrentThreadId());
...
/*                                                                      */
/************************************************************************/
HRESULT DXUTSetD3D8(IDirect3D8 *pD3D8)
{
	if(pD3D8 != NULL)
	{
		GetDXUTState().SetD3D(pD3D8);
	}
	return	S_OK;
}
//--------------------------------------------------------------------------------------
...
//--------------------------------------------------------------------------------------
HRESULT DXUTSetDevice( IDirect3DDevice8* pd3dDevice )
{
    HRESULT hr;
    ...
    if( SUCCEEDED(hr) )
    {
        ...
        hr = pBackBuffer->GetContainer( IID_IDirect3DSwapChain8, (void**) &pSwapChain );
        if( SUCCEEDED(hr) )
        {
...
HRESULT DXUTCreate3DEnvironment( IDirect3DDevice8* pd3dDeviceFromApp )
{
    ...
    D3DCAPS8* pd3dCaps = GetDXUTState().GetCaps();
...
HRESULT DXUTReset3DEnvironment()
{
    ...
    IDirect3DDevice8* pd3dDevice = DXUTGetD3DDevice();
...
void DXUTCleanup3DEnvironment( bool bReleaseSettings )
{
   ...
   //     }
        ...
        D3DCAPS8* pd3dCaps = GetDXUTState().GetCaps();
        ZeroMemory( pd3dCaps, sizeof(D3DCAPS8) );
...
//--------------------------------------------------------------------------------------
IDirect3D8* DXUTGetD3DObject()                      { return GetDXUTState().GetD3D(); }        
IDirect3DDevice8* DXUTGetD3DDevice()                { return GetDXUTState().GetD3DDevice(); }  
const D3DSURFACE_DESC* DXUTGetBackBufferSurfaceDesc() { return GetDXUTState().GetBackBufferSurfaceDesc(); }
const D3DCAPS8* DXUTGetDeviceCaps()                 { return GetDXUTState().GetCaps(); }
...
HRESULT DXUTSetDeviceCursor( IDirect3DDevice8* pd3dDevice, HCURSOR hCursor, bool bAddWatermark )
{
    ...
    ICONINFO iconinfo;
    bool bBWCursor;
    LPDIRECT3DSURFACE8 pCursorSurface = NULL;
    HDC hdcColor = NULL;
    HDC hdcMask = NULL;
    HDC hdcScreen = NULL;
    BITMAP bm;
