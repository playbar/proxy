...
void DXUTDisplaySwitchingToREFWarning()
{
    if( DXUTGetShowMsgBoxOnError() )
    {
        ...
        HKEY hKey;
        LONG lResult = RegOpenKeyEx( HKEY_CURRENT_USER, L"Software\\Microsoft\\DirectX 8.0 SDK", 0, KEY_READ, &hKey );
        ...
        if( dwSkipWarning == 0 )
        {
            ...
            {
                lResult = RegOpenKeyEx( HKEY_CURRENT_USER, L"Software\\Microsoft\\DirectX 8.0 SDK", 0, KEY_WRITE, &hKey );
                if( ERROR_SUCCESS == lResult ) 
                {
                    dwSkipWarning = 1;
...
HRESULT CDXUTResourceCache::CreateTextureFromFile( LPDIRECT3DDEVICE8 pDevice, LPCTSTR pSrcFile, LPDIRECT3DTEXTURE8 *ppTexture )
{
...
//--------------------------------------------------------------------------------------
HRESULT CDXUTResourceCache::CreateTextureFromFileEx( LPDIRECT3DDEVICE8 pDevice, LPCTSTR pSrcFile, UINT Width, UINT Height, UINT MipLevels, DWORD Usage, D3DFORMAT Format, D3DPOOL Pool, DWORD Filter, DWORD MipFilter, D3DCOLOR ColorKey, D3DXIMAGE_INFO *pSrcInfo, PALETTEENTRY *pPalette, LPDIRECT3DTEXTURE8 *ppTexture )
{
    ...
    for( int i = 0; i < m_TextureCache.GetSize(); ++i )
    {
            ...
            Entry.Type == D3DRTYPE_TEXTURE )
        {
            ...
            return Entry.pTexture->QueryInterface( IID_IDirect3DTexture8, (LPVOID*)ppTexture );
        }
    }
    ...
    (*ppTexture)->QueryInterface( IID_IDirect3DBaseTexture8, (LPVOID*)&NewEntry.pTexture );
...
//--------------------------------------------------------------------------------------
HRESULT CDXUTResourceCache::CreateTextureFromResource( LPDIRECT3DDEVICE8 pDevice, HMODULE hSrcModule, LPCTSTR pSrcResource, LPDIRECT3DTEXTURE8 *ppTexture )
{
...
//--------------------------------------------------------------------------------------
HRESULT CDXUTResourceCache::CreateTextureFromResourceEx( LPDIRECT3DDEVICE8 pDevice, HMODULE hSrcModule, LPCTSTR pSrcResource, UINT Width, UINT Height, UINT MipLevels, DWORD Usage, D3DFORMAT Format, D3DPOOL Pool, DWORD Filter, DWORD MipFilter, D3DCOLOR ColorKey, D3DXIMAGE_INFO *pSrcInfo, PALETTEENTRY *pPalette, LPDIRECT3DTEXTURE8 *ppTexture )
{
    // Search the cache for a matching entry.
    for( int i = 0; i < m_TextureCache.GetSize(); ++i )
    {
        ...
        if( Entry.Location == DXUTCACHE_LOCATION_RESOURCE &&
            ...
            Entry.Type == D3DRTYPE_TEXTURE )
        {
            // A match is found. Obtain the IDirect3DTexture8 interface and return that.
            return Entry.pTexture->QueryInterface( IID_IDirect3DTexture8, (LPVOID*)ppTexture );
        }
    }
...
//--------------------------------------------------------------------------------------
HRESULT CDXUTResourceCache::CreateCubeTextureFromFile( LPDIRECT3DDEVICE8 pDevice, LPCTSTR pSrcFile, LPDIRECT3DCUBETEXTURE8 *ppCubeTexture )
{
...
//--------------------------------------------------------------------------------------
HRESULT CDXUTResourceCache::CreateCubeTextureFromFileEx( LPDIRECT3DDEVICE8 pDevice, LPCTSTR pSrcFile, UINT Size, UINT MipLevels, DWORD Usage, D3DFORMAT Format, D3DPOOL Pool, DWORD Filter, DWORD MipFilter, D3DCOLOR ColorKey, D3DXIMAGE_INFO *pSrcInfo, PALETTEENTRY *pPalette, LPDIRECT3DCUBETEXTURE8 *ppCubeTexture )
{
    // Search the cache for a matching entry.
    for( int i = 0; i < m_TextureCache.GetSize(); ++i )
    {
        ...
        {
            // A match is found. Obtain the IDirect3DCubeTexture8 interface and return that.
            return Entry.pTexture->QueryInterface( IID_IDirect3DCubeTexture8, (LPVOID*)ppCubeTexture );
        }
    }
    ...
    (*ppCubeTexture)->QueryInterface( IID_IDirect3DBaseTexture8, (LPVOID*)&NewEntry.pTexture );
...
//--------------------------------------------------------------------------------------
HRESULT CDXUTResourceCache::CreateCubeTextureFromResource( LPDIRECT3DDEVICE8 pDevice, HMODULE hSrcModule, LPCTSTR pSrcResource, LPDIRECT3DCUBETEXTURE8 *ppCubeTexture )
{
...
//--------------------------------------------------------------------------------------
HRESULT CDXUTResourceCache::CreateCubeTextureFromResourceEx( LPDIRECT3DDEVICE8 pDevice, HMODULE hSrcModule, LPCTSTR pSrcResource, UINT Size, UINT MipLevels, DWORD Usage, D3DFORMAT Format, D3DPOOL Pool, DWORD Filter, DWORD MipFilter, D3DCOLOR ColorKey, D3DXIMAGE_INFO *pSrcInfo, PALETTEENTRY *pPalette, LPDIRECT3DCUBETEXTURE8 *ppCubeTexture )
{
    // Search the cache for a matching entry.
    for( int i = 0; i < m_TextureCache.GetSize(); ++i )
    {
...
//--------------------------------------------------------------------------------------
HRESULT CDXUTResourceCache::CreateVolumeTextureFromFile( LPDIRECT3DDEVICE8 pDevice, LPCTSTR pSrcFile, LPDIRECT3DVOLUMETEXTURE8 *ppVolumeTexture )
{
...
//--------------------------------------------------------------------------------------
HRESULT CDXUTResourceCache::CreateVolumeTextureFromFileEx( LPDIRECT3DDEVICE8 pDevice, LPCTSTR pSrcFile, UINT Width, UINT Height, UINT Depth, UINT MipLevels, DWORD Usage, D3DFORMAT Format, D3DPOOL Pool, DWORD Filter, DWORD MipFilter, D3DCOLOR ColorKey, D3DXIMAGE_INFO *pSrcInfo, PALETTEENTRY *pPalette, LPDIRECT3DVOLUMETEXTURE8 *ppTexture )
{
    // Search the cache for a matching entry.
    for( int i = 0; i < m_TextureCache.GetSize(); ++i )
    {
        ...
        {
            // A match is found. Obtain the IDirect3DVolumeTexture8 interface and return that.
            return Entry.pTexture->QueryInterface( IID_IDirect3DVolumeTexture8, (LPVOID*)ppTexture );
        }
    }
...
//--------------------------------------------------------------------------------------
HRESULT CDXUTResourceCache::CreateVolumeTextureFromResource( LPDIRECT3DDEVICE8 pDevice, HMODULE hSrcModule, LPCTSTR pSrcResource, LPDIRECT3DVOLUMETEXTURE8 *ppVolumeTexture )
{
...
HRESULT CDXUTResourceCache::CreateVolumeTextureFromResourceEx( LPDIRECT3DDEVICE8 pDevice, HMODULE hSrcModule, LPCTSTR pSrcResource, UINT Width, UINT Height, UINT Depth, UINT MipLevels, DWORD Usage, D3DFORMAT Format, D3DPOOL Pool, DWORD Filter, DWORD MipFilter, D3DCOLOR ColorKey, D3DXIMAGE_INFO *pSrcInfo, PALETTEENTRY *pPalette, LPDIRECT3DVOLUMETEXTURE8 *ppVolumeTexture )
{
    ...
    for( int i = 0; i < m_TextureCache.GetSize(); ++i )
    {
        ...
        {
            // A match is found. Obtain the IDirect3DVolumeTexture8 interface and return that.
            return Entry.pTexture->QueryInterface( IID_IDirect3DVolumeTexture8, (LPVOID*)ppVolumeTexture );
        }
    }
    ...
    NewEntry.Pool = Pool;
    NewEntry.Type = D3DRTYPE_VOLUMETEXTURE;
    (*ppVolumeTexture)->QueryInterface( IID_IDirect3DBaseTexture8, (LPVOID*)&NewEntry.pTexture );
...
//--------------------------------------------------------------------------------------
HRESULT CDXUTResourceCache::CreateFont( LPDIRECT3DDEVICE8 pDevice, UINT Height, UINT Width, UINT Weight, UINT MipLevels, BOOL Italic, DWORD CharSet, DWORD OutputPrecision, DWORD Quality, DWORD PitchAndFamily, LPCTSTR pFacename, LPD3DXFONT *ppFont )
{
    D3DXFONT_DESCW Desc;
...
//--------------------------------------------------------------------------------------
HRESULT CDXUTResourceCache::CreateFontIndirect( LPDIRECT3DDEVICE8 pDevice, CONST D3DXFONT_DESC *pDesc, LPD3DXFONT *ppFont )
{
    // Search the cache for a matching entry.
...
//--------------------------------------------------------------------------------------
HRESULT CDXUTResourceCache::CreateEffectFromFile( LPDIRECT3DDEVICE8 pDevice, LPCTSTR pSrcFile, const D3DXMACRO *pDefines, LPD3DXINCLUDE pInclude, DWORD Flags, LPD3DXEFFECTPOOL pPool, LPD3DXEFFECT *ppEffect, LPD3DXBUFFER *ppCompilationErrors )
{
    // Search the cache for a matching entry.
...
//--------------------------------------------------------------------------------------
HRESULT CDXUTResourceCache::CreateEffectFromResource( LPDIRECT3DDEVICE8 pDevice, HMODULE hSrcModule, LPCTSTR pSrcResource, const D3DXMACRO *pDefines, LPD3DXINCLUDE pInclude, DWORD Flags, LPD3DXEFFECTPOOL pPool, LPD3DXEFFECT *ppEffect, LPD3DXBUFFER *ppCompilationErrors )
{
    // Search the cache for a matching entry.
    for( int i = 0; i < m_EffectCache.GetSize(); ++i )
    {
...
//--------------------------------------------------------------------------------------
HRESULT CDXUTResourceCache::OnCreateDevice( IDirect3DDevice8 *pd3dDevice )
{
    return S_OK;
}
...
//--------------------------------------------------------------------------------------
HRESULT CDXUTResourceCache::OnResetDevice( IDirect3DDevice8 *pd3dDevice )
{
...
//--------------------------------------------------------------------------------------
HRESULT CDXUTLineManager::OnCreatedDevice( IDirect3DDevice8* pd3dDevice )
{
...
//--------------------------------------------------------------------------------------
IDirect3DDevice8* CDXUTDirectionWidget::s_pd3dDevice = NULL;
...
//--------------------------------------------------------------------------------------
// Direct3D8 dynamic linking support -- calls top-level D3D8 APIs with graceful
...
static HMODULE s_hModD3D8 = NULL;
...
static bool DXUT_EnsureD3DAPIs( void )
{
    ...
    // that this doesn't guarantee that all D3D8 procaddresses were found.
    if( s_hModD3D8 != NULL )
        return true;

    // This may fail if DirectX 8 isn't installed
    WCHAR wszPath[MAX_PATH+1];
    if( !::GetSystemDirectory( wszPath, MAX_PATH+1 ) )
        return false;
    StringCchCat( wszPath, MAX_PATH, L"\\d3d8.dll" );
    s_hModD3D8 = LoadLibrary( wszPath );
    if( s_hModD3D8 == NULL ) 
        return false;
    s_DynamicDirect3DCreate8 = (LPDIRECT3DCREATE8)GetProcAddress( s_hModD3D8, "Direct3DCreate8" );
    s_DynamicD3DPERF_BeginEvent = (LPD3DPERF_BEGINEVENT)GetProcAddress( s_hModD3D8, "D3DPERF_BeginEvent" );
    s_DynamicD3DPERF_EndEvent = (LPD3DPERF_ENDEVENT)GetProcAddress( s_hModD3D8, "D3DPERF_EndEvent" );
    s_DynamicD3DPERF_SetMarker = (LPD3DPERF_SETMARKER)GetProcAddress( s_hModD3D8, "D3DPERF_SetMarker" );
    s_DynamicD3DPERF_SetRegion = (LPD3DPERF_SETREGION)GetProcAddress( s_hModD3D8, "D3DPERF_SetRegion" );
    s_DynamicD3DPERF_QueryRepeatFrame = (LPD3DPERF_QUERYREPEATFRAME)GetProcAddress( s_hModD3D8, "D3DPERF_QueryRepeatFrame" );
    s_DynamicD3DPERF_SetOptions = (LPD3DPERF_SETOPTIONS)GetProcAddress( s_hModD3D8, "D3DPERF_SetOptions" );
    s_DynamicD3DPERF_GetStatus = (LPD3DPERF_GETSTATUS)GetProcAddress( s_hModD3D8, "D3DPERF_GetStatus" );
    return true;
}

IDirect3D8 * WINAPI DXUT_Dynamic_Direct3DCreate8(UINT SDKVersion) 
{
    if( DXUT_EnsureD3DAPIs() && s_DynamicDirect3DCreate8 != NULL )
        return s_DynamicDirect3DCreate8( SDKVersion );
    else
        return NULL;
}
...
//--------------------------------------------------------------------------------------
void DXUTTraceDecl( D3DVERTEXELEMENT8 decl[MAX_FVF_DECL_SIZE] )
{
    int iDecl=0;
...
//--------------------------------------------------------------------------------------
IDirect3DDevice8* DXUTCreateRefDevice( HWND hWnd, bool bNullRef )
{
    HRESULT hr;
    IDirect3D8* pD3D = DXUT_Dynamic_Direct3DCreate8( D3D_SDK_VERSION );
    if( NULL == pD3D )
        return NULL;
