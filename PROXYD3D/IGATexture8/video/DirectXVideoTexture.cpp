...
HRESULT CDirectXVideoTexture::SetMediaType(const CMediaType *pMediaType)
{
	...
	{
		OutputDebugString("CDirectXVideoTexture: m_pTexture创建失败");
        return E_FAIL;
	}

	// Create the texture that maps to this media type
    if(FAILED(D3DXCreateTexture(m_pDevice, m_VideoWidth, m_VideoHeight, 1, 0, 
                                D3DFMT_A8R8G8B8, D3DPOOL_MANAGED, &m_pTexture1)))
	{
		OutputDebugString("CDirectXVideoTexture: m_pTexture1创建失败");
        return E_FAIL;
	}
...
LPDIRECT3DTEXTURE8 CDirectXVideoTexture::GetTexture()
{
	if( m_bChangeSurface )
	{
		return m_pTexture1;
...
{	
    ...
    BYTE *pVideoRowStart = NULL;
    BYTE *pTextureRowStart = pTextureBuffer + TexturePitch * (m_VideoHeight - 1); 
	pTextureBuffer = pTextureRowStart;

	for(long y = 0; y < m_VideoHeight; y++) 
	{
        pVideoRowStart = pVideoBuffer;
        pTextureRowStart = pTextureBuffer;  
	...
	m_bChangeSurface = !m_bChangeSurface;

    return S_OK;
}

HRESULT CDirectXVideoTexture::CreateFromAVIFile(LPDIRECT3DDEVICE8 pDevice, char *pFileName)
{
...
void CDirectXVideoTexture::CheckForLoop()
{
	...
	////--------------------------------------------------------
	//HRESULT hr = m_pDevice->GetBackBuffer( 0, D3DBACKBUFFER_TYPE_MONO, &m_pBackBuf );
	//if( SUCCEEDED(hr) )
	//{
	...
	//	rc.left = m_X;
	//	rc.top  = m_Y;
	//	rc.right = m_X + m_VideoWidth;
	//	rc.bottom = m_Y + m_VideoHeight;
