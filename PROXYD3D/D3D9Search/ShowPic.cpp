...
extern CDXUTDialog			g_SampleUI;				//对话框
extern D3D9SEARCHSETTINGS	g_D3D9SearchSetting;	//自定义变量，存放加载变量和通过接口获取变量
...
HRESULT CShowPic::InitPic()
{
	...
	*++p	=0;
	...
	V_RETURN( m_spD3DDevice->CreateVertexBuffer( 4*sizeof(CUSTOMVERTEX),
	...
	V_RETURN (D3DXCreateTextureFromFile( m_spD3DDevice, szwPicFileName, &m_spTextureSearch ));

	//m_D3D9ManageTexture.D3D9TextureInit( m_spD3DDevice,m_szVideoFileName, m_iWidth, m_iHeight );
...
HRESULT CShowPic::InitTranslationMatrix()
{
	...
	VOID* pVertices;
	V_RETURN( m_spVBSearchPic->Lock( 0, sizeof(customerVertices), (void**)&pVertices, 0 ) );
...
HRESULT CShowPic::RenderPic()
{
	...
	//else
	//{
	//	m_spD3DDevice->SetTexture( 0, m_D3D9ManageTexture.D3D9GetTexture() );
	//}
