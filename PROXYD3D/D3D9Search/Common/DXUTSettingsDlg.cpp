...
CD3DSettingsDlg g_CD3DSettingsDlg;
...
HRESULT CD3DSettingsDlg::Refresh()
{
    ...
    CD3DEnumeration* pD3DEnum = &g_CD3DEnumeration;
    m_DeviceSettings = DXUTGetDeviceSettings();

    // Fill the UI with the current settings
    AddDeviceType( m_DeviceSettings.DeviceType );
    SetWindowed( FALSE != m_DeviceSettings.pp.Windowed );
    SetDeviceClip( 0 != (m_DeviceSettings.pp.Flags & D3DPRESENTFLAG_DEVICECLIP) );
    AddAdapterFormat( m_DeviceSettings.AdapterFormat );
    AddResolution( m_DeviceSettings.pp.BackBufferWidth, m_DeviceSettings.pp.BackBufferHeight );
    AddRefreshRate( m_DeviceSettings.pp.FullScreen_RefreshRateInHz );
    AddBackBufferFormat( m_DeviceSettings.pp.BackBufferFormat );
    AddDepthStencilBufferFormat( m_DeviceSettings.pp.AutoDepthStencilFormat );
    AddMultisampleType( m_DeviceSettings.pp.MultiSampleType );
    AddMultisampleQuality( m_DeviceSettings.pp.MultiSampleQuality );
    
    if( m_DeviceSettings.BehaviorFlags & D3DCREATE_PUREDEVICE )
        AddVertexProcessingType( D3DCREATE_PUREDEVICE );
    else if( m_DeviceSettings.BehaviorFlags & D3DCREATE_HARDWARE_VERTEXPROCESSING )
        AddVertexProcessingType( D3DCREATE_HARDWARE_VERTEXPROCESSING );
    else if( m_DeviceSettings.BehaviorFlags & D3DCREATE_SOFTWARE_VERTEXPROCESSING )
        AddVertexProcessingType( D3DCREATE_SOFTWARE_VERTEXPROCESSING );
    else if( m_DeviceSettings.BehaviorFlags & D3DCREATE_MIXED_VERTEXPROCESSING )
        AddVertexProcessingType( D3DCREATE_MIXED_VERTEXPROCESSING );

    CD3DEnumDeviceSettingsCombo* pBestDeviceSettingsCombo = pD3DEnum->GetDeviceSettingsCombo( m_DeviceSettings.AdapterOrdinal, m_DeviceSettings.DeviceType, m_DeviceSettings.AdapterFormat, m_DeviceSettings.pp.BackBufferFormat, (m_DeviceSettings.pp.Windowed != 0) );
    ...
    pAdapterCombo->SetSelectedByData( ULongToPtr( m_DeviceSettings.AdapterOrdinal ) );
                               ...
                               CDXUTControl* pControl )
{
    switch( nControlID )
    {
        ...
        {
            if( m_DeviceSettings.pp.Windowed )
            {
                m_DeviceSettings.pp.FullScreen_RefreshRateInHz = 0;
                ...
                m_DeviceSettings.pp.BackBufferWidth = dwWindowWidth;
                m_DeviceSettings.pp.BackBufferHeight = dwWindowHeight;
            }

            if( m_DeviceSettings.pp.MultiSampleType != D3DMULTISAMPLE_NONE )
            {
                m_DeviceSettings.pp.Flags &= ~D3DPRESENTFLAG_LOCKABLE_BACKBUFFER;
            }

            DXUTCreateDeviceFromSettings( &m_DeviceSettings );
...
HRESULT CD3DSettingsDlg::SetDeviceSettingsFromUI()
{
    ...
    m_DeviceSettings.DeviceType = (D3DDEVTYPE) PtrToUlong( pComboBox->GetSelectedData() );
    
    // DXUTSETTINGSDLG_WINDOWED
    pRadioButton = m_Dialog.GetRadioButton( DXUTSETTINGSDLG_WINDOWED );
    m_DeviceSettings.pp.Windowed = pRadioButton->GetChecked();

    // DXUTSETTINGSDLG_ADAPTER_FORMAT
    pComboBox = m_Dialog.GetComboBox( DXUTSETTINGSDLG_ADAPTER_FORMAT );
    m_DeviceSettings.AdapterFormat = (D3DFORMAT) PtrToUlong( pComboBox->GetSelectedData() );
    ...
    else
    {
        ...
        m_DeviceSettings.pp.BackBufferFormat = (D3DFORMAT) PtrToUlong( pComboBox->GetSelectedData() );
    
        // DXUTSETTINGSDLG_RESOLUTION
        pComboBox = m_Dialog.GetComboBox( DXUTSETTINGSDLG_RESOLUTION );
        DWORD dwResolution = PtrToUlong( pComboBox->GetSelectedData() );
        m_DeviceSettings.pp.BackBufferWidth = HIWORD( dwResolution );
        m_DeviceSettings.pp.BackBufferHeight = LOWORD( dwResolution );
        
        // DXUTSETTINGSDLG_REFRESH_RATE
        pComboBox = m_Dialog.GetComboBox( DXUTSETTINGSDLG_REFRESH_RATE );
        m_DeviceSettings.pp.FullScreen_RefreshRateInHz = PtrToUlong( pComboBox->GetSelectedData() );
    }

    // DXUTSETTINGSDLG_DEPTH_STENCIL
    pComboBox = m_Dialog.GetComboBox( DXUTSETTINGSDLG_DEPTH_STENCIL );
    m_DeviceSettings.pp.AutoDepthStencilFormat = (D3DFORMAT) PtrToUlong( pComboBox->GetSelectedData() );
...
CD3DEnumAdapterInfo* CD3DSettingsDlg::GetCurrentAdapterInfo()
{
   //CD3DEnumeration* pD3DEnum = DXUTGetEnumeration();
    return g_CD3DEnumeration.GetAdapterInfo( m_DeviceSettings.AdapterOrdinal );
}
...
{
    //CD3DEnumeration* pD3DEnum = DXUTGetEnumeration();
    return g_CD3DEnumeration.GetDeviceInfo( m_DeviceSettings.AdapterOrdinal,
...
{
    //CD3DEnumeration* pD3DEnum = DXUTGetEnumeration();
    return g_CD3DEnumeration.GetDeviceSettingsCombo( m_DeviceSettings.AdapterOrdinal,
                                             m_DeviceSettings.DeviceType,
                                             m_DeviceSettings.AdapterFormat,
                                             m_DeviceSettings.pp.BackBufferFormat,
...
HRESULT CD3DSettingsDlg::OnAdapterChanged()
{
    ...
    m_DeviceSettings.AdapterOrdinal = GetSelectedAdapter();
    ...
    pDeviceTypeComboBox->SetSelectedByData( ULongToPtr(m_DeviceSettings.DeviceType) );
...
HRESULT CD3DSettingsDlg::OnDeviceTypeChanged()
{
    ...
    m_DeviceSettings.DeviceType = GetSelectedDeviceType();
    ...
    SetWindowed( m_DeviceSettings.pp.Windowed && bHasWindowedDeviceCombo );
...
HRESULT CD3DSettingsDlg::OnWindowedFullScreenChanged()
{
    ...
    m_DeviceSettings.pp.Windowed = bWindowed;
    ...
    bool bDeviceClip = ( 0x0 != (m_DeviceSettings.pp.Flags & D3DPRESENTFLAG_DEVICECLIP) );
    ...
    if( m_DeviceSettings.pp.Windowed )
    {
        ...
        D3DDISPLAYMODE mode;
        hr = pD3D->GetAdapterDisplayMode( m_DeviceSettings.AdapterOrdinal, &mode );
        if( FAILED(hr) )
        ...
        if( rc.right == 0 || rc.bottom == 0 )
        {
            ...
            m_DeviceSettings.pp.BackBufferWidth = mode.Width;
            m_DeviceSettings.pp.BackBufferHeight = mode.Height;
        }
        else
        {
            m_DeviceSettings.pp.BackBufferWidth = rc.right;
            m_DeviceSettings.pp.BackBufferHeight = rc.bottom;
        }

        m_DeviceSettings.AdapterFormat = mode.Format;
        m_DeviceSettings.pp.FullScreen_RefreshRateInHz = mode.RefreshRate;
    }

    const D3DFORMAT adapterFormat = m_DeviceSettings.AdapterFormat;
    const DWORD dwWidth = m_DeviceSettings.pp.BackBufferWidth;
    const DWORD dwHeight = m_DeviceSettings.pp.BackBufferHeight;
    const DWORD dwRefreshRate = m_DeviceSettings.pp.FullScreen_RefreshRateInHz;
...
{ 
    ...
    m_DeviceSettings.AdapterFormat = adapterFormat;
    ...
    DXUTGetDesktopResolution( m_DeviceSettings.AdapterOrdinal, &dmDesktop.Width, &dmDesktop.Height );
    ...
    const DWORD dwCurResolution = MAKELONG( m_DeviceSettings.pp.BackBufferWidth, 
                                            m_DeviceSettings.pp.BackBufferHeight );
    ...
    for( int idc = 0; idc < pDeviceInfo->deviceSettingsComboList.GetSize(); idc++ )
    {
        ...
        if( pDeviceCombo->Windowed == bWindowed &&
            pDeviceCombo->AdapterFormat == m_DeviceSettings.AdapterFormat )
        {
    ...
    pBackBufferFormatComboBox->SetSelectedByData( ULongToPtr(m_DeviceSettings.pp.BackBufferFormat) );
...
HRESULT CD3DSettingsDlg::OnResolutionChanged()
{
    ...
    m_DeviceSettings.pp.BackBufferWidth = dwWidth;
    m_DeviceSettings.pp.BackBufferHeight = dwHeight;

    DWORD dwRefreshRate = m_DeviceSettings.pp.FullScreen_RefreshRateInHz;

    // Update the refresh rate list
    CDXUTComboBox* pRefreshRateComboBox = m_Dialog.GetComboBox( DXUTSETTINGSDLG_REFRESH_RATE );
    pRefreshRateComboBox->RemoveAllItems();

    D3DFORMAT adapterFormat = m_DeviceSettings.AdapterFormat;
    for( int idm = 0; idm < pAdapterInfo->displayModeList.GetSize(); idm++ )
    {
        D3DDISPLAYMODE displayMode = pAdapterInfo->displayModeList.GetAt( idm );
...
{
    // Set refresh rate
    m_DeviceSettings.pp.FullScreen_RefreshRateInHz = GetSelectedRefreshRate();
...
HRESULT CD3DSettingsDlg::OnBackBufferFormatChanged()
{
    ...
    m_DeviceSettings.pp.BackBufferFormat = GetSelectedBackBufferFormat();
    
    D3DFORMAT adapterFormat = m_DeviceSettings.AdapterFormat;
    D3DFORMAT backBufferFormat = m_DeviceSettings.pp.BackBufferFormat;
    ...
    g_CD3DEnumeration.GetPossibleVertexProcessingList( &bAllowSoftwareVP, &bAllowHardwareVP, 
    ...
    for( int idc=0; idc < pDeviceInfo->deviceSettingsComboList.GetSize(); idc++ )
    {
        ...
        if( pDeviceCombo->Windowed == (m_DeviceSettings.pp.Windowed == TRUE) &&
            ...
            pDeviceCombo->BackBufferFormat == backBufferFormat )
        {
            ...
            pDepthStencilComboBox->SetEnabled( (m_DeviceSettings.pp.EnableAutoDepthStencil == TRUE) ); 

            if( m_DeviceSettings.pp.EnableAutoDepthStencil )
            {
                ...
                pDepthStencilComboBox->SetSelectedByData( ULongToPtr(m_DeviceSettings.pp.AutoDepthStencilFormat) );
            }
            else
            {
            ...
            pPresentIntervalComboBox->SetSelectedByData( ULongToPtr( m_DeviceSettings.pp.PresentationInterval ) );
...
HRESULT CD3DSettingsDlg::OnDepthStencilBufferFormatChanged()
{
    ...
    if( m_DeviceSettings.pp.EnableAutoDepthStencil )
        m_DeviceSettings.pp.AutoDepthStencilFormat = depthStencilBufferFormat;
    ...
    pMultisampleQualityCombo->SetSelectedByData( ULongToPtr(m_DeviceSettings.pp.MultiSampleType) );
...
HRESULT CD3DSettingsDlg::OnMultisampleTypeChanged()
{
    ...
    m_DeviceSettings.pp.MultiSampleType = multisampleType;
    ...
    pMultisampleQualityCombo->SetSelectedByData( ULongToPtr(m_DeviceSettings.pp.MultiSampleQuality) );
...
{
    m_DeviceSettings.pp.MultiSampleQuality = GetSelectedMultisampleQuality();
...
{
    DWORD dwBehavior = m_DeviceSettings.BehaviorFlags;
    ...
    // Make changes
    m_DeviceSettings.BehaviorFlags = dwBehavior | dwNewFlags;
...
{
    m_DeviceSettings.pp.PresentationInterval = GetSelectedPresentInterval();
...
{
    if( IsDeviceClip() )
        m_DeviceSettings.pp.Flags |= D3DPRESENTFLAG_DEVICECLIP;
    else
        m_DeviceSettings.pp.Flags &= ~D3DPRESENTFLAG_DEVICECLIP;
