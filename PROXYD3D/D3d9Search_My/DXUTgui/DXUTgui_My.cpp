...
//--------------------------------------------------------------------------------------
bool CDXUTDialog::MsgProc( HWND hWnd, UINT uMsg, WPARAM wParamKey, LPARAM lParam )
{
	bool bHandled = false;
		...
		s_pControlFocus->GetEnabled() )
	{
		...
		if( s_pControlFocus->MsgProc( uMsg, wParamKey, lParam ) )
			return true;
	}
	...
	case WM_SYSKEYUP:
		{
			...
			if( s_pControlFocus && 
				...
				s_pControlFocus->GetEnabled() )
			{
				if( s_pControlFocus->HandleKeyboard( uMsg, wParamKey, lParam ) )
					return true;
			}
				...
				&& s_pControlFocus->GetType() != DXUT_CONTROL_IMEEDITBOX ) ) )
			{
				...
				{
					CDXUTControl* pControl = m_Controls.GetAt( i );
					if( pControl->GetHotkey() == wParamKey )
					{
						pControl->OnHotkey();
						return true;
					}
	...
	case WM_MOUSEWHEEL:
		{
			...
			if( s_pControlFocus && 
				...
				s_pControlFocus->GetEnabled() )
			{
				if( s_pControlFocus->HandleMouse( uMsg, mousePoint, wParamKey, lParam ) )
					return true;
			}
			...
			{
				bHandled = pControl->HandleMouse( uMsg, mousePoint, wParamKey, lParam );
				if( bHandled )
					return true;
			}
			else
...
void CDXUTEditBox::PasteFromClipboard()
{
	...
	if( OpenClipboard( NULL ) )
	{
		...
		if( handle )
		{
			...
			WCHAR * wchTemp = wcsstr(pwszText, TEXT("\n"));
			if ( wchTemp != NULL )
			{
				*wchTemp = '\0';
			}
...
CDXUTIMEEditBox::CDXUTIMEEditBox( CDXUTDialog *pDialog )
{
	...
	//ms_hImcDef = _ImmGetContext( g_hGameWnd );
	ms_hImcDef = ImmCreateContext();
	_ImmReleaseContext( g_hGameWnd, ms_hImcDef );
...
#define LCID_INVARIANT MAKELCID(MAKELANGID(LANG_ENGLISH, SUBLANG_ENGLISH_US), SORT_DEFAULT)
	...
	if ( !_ImmGetIMEFileNameA( ms_hklCurrent, szTmp, ( sizeof(szTmp) / sizeof(szTmp[0]) ) - 1 ) )
	{
		dwID[0] = dwID[1] = 0;
		return dwID[uIndex];
	}
...
void CDXUTIMEEditBox::CheckToggleState()
{
	...
	CheckInputLocale();
	bool bIme = _ImmIsIME( ms_hklCurrent ) != 0;
	s_bChineseIME = ( GetPrimaryLanguage() == LANG_CHINESE ) && bIme;

	HIMC hImc;
	if( NULL != ( hImc = _ImmGetContext( g_hGameWnd ) ) )
	{
		if( s_bChineseIME )
		{
		...
		_ImmReleaseContext( g_hGameWnd, hImc );
	}
	else
	{
...
void CDXUTIMEEditBox::SetupImeApi()
{
	...
	if( _ImmGetIMEFileNameA( ms_hklCurrent, szImeFile, sizeof(szImeFile)/sizeof(szImeFile[0]) - 1 ) == 0 )
		return;
...
void CDXUTIMEEditBox::GetPrivateReadingString()
{
	...
	HIMC hImc;
	hImc = _ImmGetContext( g_hGameWnd );
	if( !hImc )
	{
		s_bShowReadingWindow = false;
		return;
...
void CDXUTIMEEditBox::OnFocusIn()
{
	...
	{
		_ImmAssociateContext( g_hGameWnd, ms_hImcDef );
		CheckToggleState();
	} 
	else
	{
		_ImmAssociateContext( g_hGameWnd, NULL );
	}
	...
	case WM_LBUTTONDBLCLK:
		{
				...
				s_ptCompString.y + pFont->nHeight > pt.y )
			{
				...
				HIMC hImc = _ImmGetContext( g_hGameWnd );
				if( !hImc )
					return true;
