...
extern 	CDXUTIMEEditBox *g_pIMEEdit;
...
{   
	...
	//{
	//	m_fTimeLastRefresh = g_DXUTState.GetTime();
	//	Refresh();
	//}
...
CDXUTDialogResourceManager::~CDXUTDialogResourceManager()
{
	...
	//OutputDebugStringA("CDXUTDialogResourceManager::~CDXUTDialogResourceManager....");
}
...
HRESULT CDXUTDialogResourceManager::OnCreateDevice( LPDIRECT3DDEVICE9 pd3dDevice )
{
	...
	V_RETURN( m_pd3dDevice->CreateStateBlock( D3DSBT_ALL, &m_pStateBlock ) );
...
HRESULT CDXUTDialogResourceManager::OnResetDevice()
{
	...
	//IDirect3DDevice9* pd3dDevice = g_DXUTState.GetD3DDevice();
	...
	case WM_LBUTTONDBLCLK:
		{
			...
			if( PtInRect( &m_rcUpButton, pt ) )
			{
				...
				UpdateThumbRect();
				m_Arrow = CLICKED_UP;
				m_dArrowTS = g_DXUTState.GetTime();
				return true;
			}
...
void CDXUTScrollBar::Render( IDirect3DDevice9* pd3dDevice, float fElapsedTime )
{
	...
	if( m_Arrow != CLEAR )
	{
		double dCurrTime = g_DXUTState.GetTime();
...
CDXUTEditBox::CDXUTEditBox( CDXUTDialog *pDialog )
{
	...
	m_dfLastBlink = g_CDXUTTimer.GetAbsoluteTime();
	s_bHideCaret = false;
	m_nFirstVisible = 0;
			...
			default:
				{
					...
					{
						//判断重复输入，主要问题是接管键盘消息造成的，这种解决办法现在不是好办法
						/*	if ( g_bIm )
						{
						if (g_istrCurLen < g_istrlen)
						{
						g_istrCurLen++;
						if (g_istrCurLen == g_istrlen )
						{
...
void CDXUTEditBox::Render( IDirect3DDevice9* pd3dDevice, float fElapsedTime )
{
	...
	// Blink the caret
	//
	if( g_CDXUTTimer.GetAbsoluteTime() - m_dfLastBlink >= m_dfBlink )
	{
...
void CDXUTEditBox::SetTextFloatArray( const float *pNumbers, int nCount )
{
	...
	{
		StringCchPrintf( wszTmp, 64, TEXT("%.4f "), pNumbers[i] );
...
HINSTANCE CDXUTIMEEditBox::ms_hDllImm32;      // IMM32 DLL handle
HINSTANCE CDXUTIMEEditBox::ms_hDllVer;        // Version DLL handle
HKL       CDXUTIMEEditBox::ms_hklCurrent;     // Current keyboard layout of the process
...
HINSTANCE CDXUTIMEEditBox::ms_hDllIme;           // Instance handle of the current IME module
HIMC      CDXUTIMEEditBox::ms_hImcDef;           // Default input context
...
CUniBuffer CDXUTIMEEditBox::ms_CompString = CUniBuffer( 0 );
...
{
	//SHOWOUTPUTDEBUG(CDXUTIMEEditBox::CDXUTIMEEditBox);

	CDXUTIMEEditBox::Initialize(); // ensure static vars are properly init'ed first
	
	ms_hImcDef = ImmGetContext( g_hGameWnd );
	ImmReleaseContext( g_hGameWnd, ms_hImcDef );

	ImmDisableTextFrameService( (DWORD)-1 );  // Disable TSF for the current process
...
{
	//SHOWOUTPUTDEBUG( CDXUTIMEEditBox::Initialize );
	//if( ms_hDllImm32 ) // Only need to do once
	//	return;
	//
	FARPROC Temp;

	//ms_CompString.SetBufferSize( MAX_COMPSTRING_SIZE );

	WCHAR wszPath[MAX_PATH+1];
	if( !::GetSystemDirectory( wszPath, MAX_PATH+1 ) )
		return;
	//StringCchCat( wszPath, MAX_PATH, IMM32_DLLNAME );
	//ms_hDllImm32 = LoadLibrary( wszPath );
	//if( ms_hDllImm32 )
	//{
	//	GETPROCADDRESS( ms_hDllImm32, ImmLockIMC, Temp );
	//	GETPROCADDRESS( ms_hDllImm32, ImmUnlockIMC, Temp );
	//	GETPROCADDRESS( ms_hDllImm32, ImmLockIMCC, Temp );
	//	GETPROCADDRESS( ms_hDllImm32, ImmUnlockIMCC, Temp );
	//	GETPROCADDRESS( ms_hDllImm32, ImmDisableTextFrameService, Temp );
	//	GETPROCADDRESS( ms_hDllImm32, ImmGetCompositionStringW, Temp );
	//	GETPROCADDRESS( ms_hDllImm32, ImmGetCandidateListW, Temp );
	//	GETPROCADDRESS( ms_hDllImm32, ImmGetContext, Temp );
	//	GETPROCADDRESS( ms_hDllImm32, ImmReleaseContext, Temp );
	//	GETPROCADDRESS( ms_hDllImm32, ImmAssociateContext, Temp );
	//	GETPROCADDRESS( ms_hDllImm32, ImmGetOpenStatus, Temp );
	//	GETPROCADDRESS( ms_hDllImm32, ImmSetOpenStatus, Temp );
	//	GETPROCADDRESS( ms_hDllImm32, ImmGetConversionStatus, Temp );
	//	GETPROCADDRESS( ms_hDllImm32, ImmGetDefaultIMEWnd, Temp );
	//	GETPROCADDRESS( ms_hDllImm32, ImmGetIMEFileNameA, Temp );
	//	GETPROCADDRESS( ms_hDllImm32, ImmGetVirtualKey, Temp );
	//	GETPROCADDRESS( ms_hDllImm32, ImmNotifyIME, Temp );
	//	GETPROCADDRESS( ms_hDllImm32, ImmSetConversionStatus, Temp );
	//	GETPROCADDRESS( ms_hDllImm32, ImmSimulateHotKey, Temp );
	//	GETPROCADDRESS( ms_hDllImm32, ImmIsIME, Temp );
	//}

	if( !::GetSystemDirectory( wszPath, MAX_PATH+1 ) )
		return;
	StringCchCat( wszPath, MAX_PATH, VER_DLLNAME );
	ms_hDllVer = LoadLibrary( wszPath );
	if( ms_hDllVer )
	{
		GETPROCADDRESS( ms_hDllVer, VerQueryValueA, Temp );
		GETPROCADDRESS( ms_hDllVer, GetFileVersionInfoA, Temp );
		GETPROCADDRESS( ms_hDllVer, GetFileVersionInfoSizeA, Temp );
	}
}
...
{
	////SHOWOUTPUTDEBUG( CDXUTIMEEditBox::Uninitialize );
	//if( ms_hDllImm32 )
	//{
...
{
	//SHOWOUTPUTDEBUG(CDXUTIMEEditBox::SendKey);
...
{
	//SHOWOUTPUTDEBUG( CDXUTIMEEditBox::StaticOnCreateDevice );
...
{
	//SHOWOUTPUTDEBUG( CDXUTIMEEditBox::UpdateRects );
...
{
	//SHOWOUTPUTDEBUG( CDXUTIMEEditBox::GetImeId);
...
#define LCID_INVARIANT MAKELCID(MAKELANGID(LANG_ENGLISH, SUBLANG_ENGLISH_US), SORT_DEFAULT)
	...
	if( hklPrev == ms_hklCurrent )
		return dwID[uIndex];

	hklPrev = ms_hklCurrent;  // Save for the next invocation

	// Check if we are using an older Chinese IME
	if( !( ( ms_hklCurrent == _CHT_HKL ) || ( ms_hklCurrent == _CHT_HKL2 ) || ( ms_hklCurrent == _CHS_HKL ) ) )
	{
		dwID[0] = dwID[1] = 0;
		return dwID[uIndex];
	}
...
{
	//SHOWOUTPUTDEBUG( CDXUTIMEEditBox::CheckInputLocale );
	static HKL s_hklPrev = 0;
	...
	{
		WCHAR wszLang[5];
		GetLocaleInfoW( MAKELCID( LOWORD( ms_hklCurrent ), SORT_DEFAULT ), LOCALE_SABBREVLANGNAME, wszLang, 5 );
...
{
	SHOWOUTPUTDEBUG( CDXUTIMEEditBox::CheckToggleState );  
	CheckInputLocale();
	bool bIme = ImmIsIME( ms_hklCurrent ) != 0;
	s_bChineseIME = ( GetPrimaryLanguage() == LANG_CHINESE ) && bIme;

	HIMC hImc;
	if( NULL != ( hImc = ImmGetContext( g_hGameWnd ) ) )
	{
		if( s_bChineseIME )
		{
		...
		ImmReleaseContext( g_hGameWnd, hImc );
	}
	else
	{
...
{
	//SHOWOUTPUTDEBUG( CDXUTIMEEditBox::EnableImeSystem );
...
{
	//SHOWOUTPUTDEBUG( CDXUTIMEEditBox::SetupImeApi );
	char szImeFile[MAX_PATH + 1];

	_GetReadingString = NULL;
	_ShowReadingWindow = NULL;
	if( ImmGetIMEFileNameA( ms_hklCurrent, szImeFile, sizeof(szImeFile)/sizeof(szImeFile[0]) - 1 ) == 0 )
		return;

	if( ms_hDllIme ) 
		FreeLibrary( ms_hDllIme );
	ms_hDllIme = LoadLibraryA( szImeFile );
	if ( !ms_hDllIme )
		return;
	_GetReadingString = (UINT (WINAPI*)(HIMC, UINT, LPWSTR, PINT, BOOL*, PUINT))
		( GetProcAddress( ms_hDllIme, "GetReadingString" ) );
	_ShowReadingWindow =(BOOL (WINAPI*)(HIMC, BOOL))
		( GetProcAddress( ms_hDllIme, "ShowReadingWindow" ) );
}
...
{
	//SHOWOUTPUTDEBUG( CDXUTIMEEditBox::ResetCompositionString );
	s_nCompCaret = 0;
	ms_CompString.SetText( L"" );
...
{
	//SHOWOUTPUTDEBUG( CDXUTIMEEditBox::TruncateCompString );
	if( !s_bInsertOnType )
		return;

	int cc = (int) wcslen( ms_CompString.GetBuffer() );
	...
	for (int i = 0; i < cc - s_nCompCaret; ++i )
		SendMessage( g_hGameWnd, WM_KEYDOWN, VK_RIGHT, 0 );
	SendMessage( g_hGameWnd, WM_KEYUP, VK_RIGHT, 0 );
	...
	if( iNewStrLen < cc )
	{
		...
		{
			SendMessage( g_hGameWnd, WM_KEYDOWN, VK_BACK, 0 );  // Backspace character
			SendMessageW( g_hGameWnd, WM_CHAR, VK_BACK, 0 );
		}
		SendMessage( g_hGameWnd, WM_KEYUP, VK_BACK, 0 );
	}
	else
		iNewStrLen = cc;
	...
	for (int i = 0; i < iNewStrLen; ++i )
		SendMessage( g_hGameWnd, WM_KEYDOWN, VK_LEFT, 0 );
	SendMessage( g_hGameWnd, WM_KEYUP, VK_LEFT, 0 );
}
...
{
	//SHOWOUTPUTDEBUG( CDXUTIMEEditBox::SendCompString );
	for( int i = 0; i < lstrlenW( ms_CompString.GetBuffer() ); ++i )
	{
		MsgProc( WM_CHAR, (WPARAM)ms_CompString[i], 0 );
	}
}
...
{
	//SHOWOUTPUTDEBUG( CDXUTIMEEditBox::FinalizeString );
	HIMC hImc;
	if( NULL == ( hImc = ImmGetContext( g_hGameWnd ) ) )
		return;
	...
	if( !s_bInsertOnType && bSend )
	{
		...
		LONG lLength = lstrlen( ms_CompString.GetBuffer() );
		// In case of CHT IME, don't send the trailing double byte space, if it exists.
		if( GetLanguage() == LANG_CHT
			&& ms_CompString[lLength - 1] == 0x3000 )
		{
			ms_CompString[lLength - 1] = 0;
		}
		SendCompString();
	}
...
{
	//SHOWOUTPUTDEBUG( CDXUTIMEEditBox::GetReadingWindowOrientation );
	s_bHorizontalReading = ( ms_hklCurrent == _CHS_HKL ) || ( ms_hklCurrent == _CHT_HKL2 ) || ( dwId == 0 );
...
{
	//SHOWOUTPUTDEBUG( CDXUTIMEEditBox::GetPrivateReadingString );
	DWORD dwId = GetImeId();
	if( !dwId )
	{
	...
	//		p = *(LPBYTE *)((LPBYTE)ImmLockIMCC( lpIC->hPrivate ) + 24 );
	//		if( !p ) break;
	...
	//		p = *(LPBYTE *)( (LPBYTE)ImmLockIMCC( lpIC->hPrivate ) + 3 * 4 );
	//		if( !p ) break;
	...
	//		p = *(LPBYTE *)((LPBYTE)ImmLockIMCC( lpIC->hPrivate ) + 4);
	//		if( !p ) break;
	...
	//			p = *(LPBYTE *)((LPBYTE)ImmLockIMCC( lpIC->hPrivate ) + nOffset * 4);
	//			if( !p ) break;
	...
	//			p = *(LPBYTE *)((LPBYTE)ImmLockIMCC( lpIC->hPrivate ) + 1*4 + 1*4 + 6*4);
	//			if( !p ) break;
...
{
	//SHOWOUTPUTDEBUG( CDXUTIMEEditBox::PumpMessage );
	MSG msg;
...
{
	//SHOWOUTPUTDEBUG( CDXUTIMEEditBox::OnFocusIn );
	CDXUTEditBox::OnFocusIn();
	if( s_bEnableImeSystem )
	{
		ImmAssociateContext( g_hGameWnd, ms_hImcDef );
		CheckToggleState();
	} 
	else
	{
		ImmAssociateContext( g_hGameWnd, NULL );
	}
...
{
	//SHOWOUTPUTDEBUG( CDXUTIMEEditBox::OnFocusOut );
		...
		if( wParam )
		{
			...
			if( phKL )
			{
				...
				for( UINT i = 0; i < cKL; ++i )
				{
					...
					{
						if( LOWORD( s_Locale.GetAt( e ).m_hKL ) == LOWORD( phKL[i] ) )
						{
							bBreak = true;
							break;
						}
...
{
	//SHOWOUTPUTDEBUG( CDXUTIMEEditBox::MsgProc );
	if( !m_bEnabled || !m_bVisible )
		return false;
	...
	case WM_IME_COMPOSITION:
		DXUTTRACE( L"WM_IME_COMPOSITION\n" );
		{
			...
			*trapped = true;
			...
			{
				s_nCompCaret = ImmGetCompositionStringW( hImc, GCS_CURSORPOS, NULL, 0 );
				if( s_nCompCaret < 0 )
			...
			if ( lParam & GCS_RESULTSTR )
			{
				...
				lRet = ImmGetCompositionStringW( hImc, GCS_RESULTSTR, wszCompStr, sizeof( wszCompStr ) );
				if( lRet > 0 )
				{
					lRet /= sizeof(WCHAR);
					wszCompStr[lRet] = 0;  // Force terminate
					TruncateCompString( false, (int)wcslen( wszCompStr ) );
					ms_CompString.SetText( wszCompStr );
					SendCompString();
					ResetCompositionString();
				}
			}
			...
			if ( lParam & GCS_COMPSTR )
			{
				...
				lRet = ImmGetCompositionStringW( hImc, GCS_COMPSTR, wszCompStr, sizeof( wszCompStr ) );
				if( lRet > 0 )
				{
					...
					{
						if( lstrlen( ms_CompString.GetBuffer() ) )
						{
							...
							for( int i = 3; i >= 0; --i )
							{
								if( i > lstrlen( ms_CompString.GetBuffer() ) - 1 )
									s_CandList.awszCandidate[i][0] = 0;  // Doesn't exist
								else
								{
									s_CandList.awszCandidate[i][0] = ms_CompString[i];
									s_CandList.awszCandidate[i][1] = 0;
								}
							}
							s_CandList.dwPageSize = MAX_CANDLIST;
							// Clear comp string after we are done copying
							ZeroMemory( (LPVOID)ms_CompString.GetBuffer(), 4 * sizeof(WCHAR) );
							s_bShowReadingWindow = true;
					...
					if( s_bInsertOnType )
					{
						...
						int nCount = lstrlen( ms_CompString.GetBuffer() + s_nCompCaret );
			...
			{
				lRet = ImmGetCompositionStringW( hImc, GCS_COMPATTR, s_abCompStringAttr, sizeof( s_abCompStringAttr ) );
				if( lRet > 0 )
			...
			{
				lRet = ImmGetCompositionStringW(hImc, GCS_COMPCLAUSE, s_adwCompStringClause, sizeof( s_adwCompStringClause ) );
...
{
	//SHOWOUTPUTDEBUG( CDXUTIMEEditBox::HandleMouse );
	if( !m_bEnabled || !m_bVisible )
		return false;
	...
	case WM_LBUTTONDBLCLK:
		{
			...
			int nCompStrWidth;
			ms_CompString.CPtoX( ms_CompString.GetTextSize(), FALSE, &nCompStrWidth );
				...
				s_ptCompString.y + pFont->nHeight > pt.y )
			{
				...
				ms_CompString.XtoCP( pt.x - s_ptCompString.x, &nCharBodyHit, &nTrail );
				if( nTrail && nCharBodyHit < ms_CompString.GetTextSize() )
				...
				HIMC hImc = ImmGetContext( g_hGameWnd );
				if( !hImc )
					return true;
...
{
	//SHOWOUTPUTDEBUG( CDXUTIMEEditBox::RenderCandidateReadingWindow );
	RECT rc;
	...
	if ( GetLanguage() == LANG_CHT && !GetImeId() )
		...
		ms_CompString.CPtoX( s_nFirstTargetConv, FALSE, &nXComp );
	else
		ms_CompString.CPtoX( s_nCompCaret, FALSE, &nXComp );
	...
	//{
	//	int nXCompTrail;
	//	ms_CompString.CPtoX( s_nCompCaret, TRUE, &nXCompTrail );
...
{
	//SHOWOUTPUTDEBUG( CDXUTIMEEditBox::RenderComposition );
	...
	m_pDialog->CalcTextRect( ms_CompString.GetBuffer(), pElement, &rc );
	...
	const WCHAR *pwszComp = ms_CompString.GetBuffer();
	int nCharLeft = ms_CompString.GetTextSize();
	for( ; ; )
	{
		// Find the last character that can be drawn on the same line.
		int nLastInLine;
		int bTrail;
		ms_CompString.XtoCP( m_rcText.right - rc.left, &nLastInLine, &bTrail );
	...
	BYTE *pAttr;
	const WCHAR *pcComp;
	for( pcComp = ms_CompString.GetBuffer(), pAttr = s_abCompStringAttr;
		*pcComp != L'\0'; ++pcComp, ++pAttr )
	{
		...
		int nXLeft, nXRight;
		ms_CompString.CPtoX( int(pcComp - ms_CompString.GetBuffer()), FALSE, &nXLeft );
		ms_CompString.CPtoX( int(pcComp - ms_CompString.GetBuffer()), TRUE, &nXRight );
		...
		if( nXRight - nXFirst > m_rcText.right - rc.left )
		{
			...
			nCharFirst = int(pcComp - ms_CompString.GetBuffer());
			ms_CompString.CPtoX( nCharFirst, FALSE, &nXFirst );
		}

		// If the caret is on this character, save the coordinates
		// for drawing the caret later.
		if( s_nCompCaret == int(pcComp - ms_CompString.GetBuffer()) )
		{
			rcCaret = rc;
	...
	if( m_bCaretOn )
	{
		...
		if( s_nCompCaret == ms_CompString.GetTextSize() )
		{
			ms_CompString.CPtoX( s_nCompCaret, FALSE, &nX );
			rcCaret = rc;
...
{
	//SHOWOUTPUTDEBUG( CDXUTIMEEditBox::RenderIndicator );
...
{
	//SHOWOUTPUTDEBUG( CDXUTIMEEditBox::Render );
	if( m_bVisible == false )
		return;
	...
	if( pElement )
	{
		ms_CompString.SetFontNode( m_pDialog->GetFont( pElement->iFont ) );
	...
	if( m_bHasFocus )
	{
		// Render the input locale indicator, 屏蔽输入框指示按钮
...
{
	//SHOWOUTPUTDEBUG( CUniBuffer::CUniBuffer );
...
{
	//SHOWOUTPUTDEBUG( CUniBuffer::Initialize );
...
{
	//SHOWOUTPUTDEBUG( CUniBuffer::Uninitialize );
	if( s_hDll )
	{
...
{
	//SHOWOUTPUTDEBUG( CUniBuffer::SetBufferSize );
...
{
	//SHOWOUTPUTDEBUG( CUniBuffer::Analyse );
	if( m_Analysis )
		_ScriptStringFree( &m_Analysis );
...
{
	//SHOWOUTPUTDEBUG( CUniBuffer::Clear );
	*m_pwszBuffer = L'\0';
	m_bAnalyseRequired = true;
}
...
{
	//SHOWOUTPUTDEBUG( CUniBuffer::InsertChar );
	assert( nIndex >= 0 );
...
{
	//SHOWOUTPUTDEBUG( CUniBuffer::RemoveChar );
...
{
	//SHOWOUTPUTDEBUG( CUniBuffer::InsertString );
	assert( nIndex >= 0 );
...
{
	//SHOWOUTPUTDEBUG( CUniBuffer::SetText );
	assert( wszText != NULL );
...
{
	//SHOWOUTPUTDEBUG( CUniBuffer::CPtoX );
	assert( pX );
	*pX = 0;  // Default
...
{
	//SHOWOUTPUTDEBUG( CUniBuffer::XtoCP );
	assert( pCP && pnTrail );
...
{
	//SHOWOUTPUTDEBUG( CUniBuffer::GetPriorItemPos );
...
{
	//SHOWOUTPUTDEBUG( CUniBuffer::GetNextItemPos);
	*pPrior = nCP;  // Default is the char itself
