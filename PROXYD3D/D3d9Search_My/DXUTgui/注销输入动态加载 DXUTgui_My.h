...
extern HWND g_hGameWnd;		//游戏窗口句柄，通过导出接口获得
...
private:
	...
	void * m_Analysis;  // Analysis for the current string
...
protected:
	static WORD GetLanguage() { return LOWORD( ms_hklCurrent ); }
	static WORD GetPrimaryLanguage() { return PRIMARYLANGID( LOWORD( ms_hklCurrent ) ); }
	static WORD GetSubLanguage() { return SUBLANGID( LOWORD( ms_hklCurrent ) ); }
...
protected:
	...
	static HINSTANCE ms_hDllImm32;         // IMM32 DLL handle
	static HINSTANCE ms_hDllVer;           // Version DLL handle
	static HIMC      ms_hImcDef;           // Default input context

	static HKL     ms_hklCurrent;          // Current keyboard layout of the process
	...
	static HINSTANCE ms_hDllIme;           // Instance handle of the current IME module
	...
	static CUniBuffer ms_CompString;       // Buffer to hold the composition string (we fix its length)
