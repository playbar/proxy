...
{
	//if ( nMsg == WM_CHAR && wparam == 112  )
	//{
	//	SHOWOUTPUTDEBUG(WM_CHAR);
	//	//DefWindowProc(hwnd, nMsg, wparam, lparam);
	//}

	//if ( nMsg == WM_INPUTLANGCHANGE)
	//{
	//	SHOWOUTPUTDEBUG(D3D9Game_WindowProc);
	//}
	...
	{
		if( g_pGame->m_pUI->WndProc( nMsg, wparam, lparam ) )
			return NULL;
	}
	//主消息循环
	switch(nMsg)
...
int WINAPI WinMain(	HINSTANCE hinstance,HINSTANCE hprevinstance,LPSTR lpcmdline,int ncmdshow)
{
	...
	while(TRUE)
	{
		...
		{ 
			...
			//	sprintf(szOutputString, "%d", msg);
			//	OutputDebugStringA( szOutputString );
			//}
			//if ( msg.message == WM_CHAR && msg.wParam == 112  )
			//{
			//	SHOWOUTPUTDEBUG(WM_CHAR);
