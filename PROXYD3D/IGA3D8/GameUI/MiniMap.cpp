...
void CMiniMap::Init(DWORD dwId, float x, float y, float width, float height, bool bActivate, CBaseUI * pBaseUI )
{
	...
	pUIMiniMap->SetTexture(m_pBaseUI->m_pTextures[GAMEUI_TEX_MINIMAP_STATIC_MAP]);
	
	CUIStatic * pUIBack = AddStatic(UI_GAME_MINIMAP_CONTROL_STATIC_BACK,x,y,width,height,true);
	pUIBack->SetTexture(m_pBaseUI->m_pTextures[GAMEUI_TEX_MINIMAP_STATIC_BACK]);
	...
	pUIRole->SetTexture(m_pBaseUI->m_pTextures[GAMEUI_TEX_MINIMAP_STATIC_ROLE]);
...
bool CMiniMap::Event(UI_MSG * pMsg)
{
	return false;
}
