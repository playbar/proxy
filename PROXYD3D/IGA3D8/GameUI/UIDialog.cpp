...
void CUIDialog::SendMsg( UI_MSG * pMsg )
{
	//g_pUI->m_UIMsg.AddItem( pMsg );
}
...
void CUIEditBox::Render()
{
	...
	strcpy( tmp, m_szText );
	strcat( tmp, TEXT("a") );
	m_pVFont->m_pFont->DrawText( tmp, (INT)_tcslen( tmp ), &rect, DT_CALCRECT, m_dwTextColor );
	RECT rect2;
	rect2.left = 0;
	m_pVFont->m_pFont->DrawText( TEXT("a"), (INT)_tcslen( TEXT("a") ), &rect2, DT_CALCRECT, m_dwTextColor );
	...
	if( rect.right - rect.left > m_fWidth - 2 * m_fBWidth )
	{
		...
		if( m_bPassword )
			m_pVFont->m_pFont->DrawText( m_szPassword, (INT)_tcslen( m_szPassword ), &rect, DT_RIGHT, m_dwTextColor );
		else
			m_pVFont->m_pFont->DrawText( m_szText, (INT)_tcslen( m_szText ), &rect, DT_RIGHT, m_dwTextColor );
	}
	else
	{
		if( m_bPassword )
			m_pVFont->m_pFont->DrawText( m_szPassword, (INT)_tcslen( m_szPassword ), &rect, DT_LEFT, m_dwTextColor );
		else
			m_pVFont->m_pFont->DrawText( m_szText, (INT)_tcslen( m_szText ), &rect, DT_LEFT, m_dwTextColor );
	}
	...
	if( m_bCursor && m_bFocus )
	{
		...
		m_pVFont->m_pFont->DrawText( TEXT("|"), 1, &rect, DT_LEFT, m_dwTextColor );
	}
}
