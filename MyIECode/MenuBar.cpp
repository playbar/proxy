...
int nDelay = 10;
extern BOOL bOutMenu;
extern UINT nLastItemID;
extern UINT nLastFlag;
extern HMENU hLastMenu;
extern BOOL m_bRTab;
...
int CMenuBar::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
	if (CToolBar::OnCreate(lpCreateStruct)==-1)
		return -1;
	UpdateFont();
//	CWnd* pFrame = AfxGetApp()->m_pMainWnd; //GetOwner();
//	ASSERT_VALID(pFrame);
...
BOOL CMenuBar::OnMenuInput(MSG& m)
{
	...
	if (msg==WM_KEYDOWN) {
		...
		if((msg==WM_MOUSEMOVE || msg==WM_LBUTTONDOWN) && nDelay>=0)
			nDelay--;
		if (msg == WM_MOUSEMOVE && !noHook && nDelay<0) {
			if (pt != m_ptMouse) {
					int iButton = HitTest(pt);
		...
		} else if (msg == WM_LBUTTONDOWN && !noHook && nDelay<0) {
...
//			pt.x, pt.y, AfxGetApp()->m_pMainWnd->GetSafeHwnd(), &tpm);
