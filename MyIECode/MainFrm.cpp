...
int CMainFrame::BuildFavoritesMenu(LPCTSTR pszPath, int nStartPos, CMenu* pMenu,int nFixMenu, int FirstLevel, int& nFavs, int& nMostFavs)
{
	...
	try
	{
		...
		if(isLink && IsWindow(m_wndReBar.GetSafeHwnd()))
		{
		...
		if(FirstLevel && IsWindow(m_wndReBar.GetSafeHwnd()))
		{
			REBARBANDINFO rbbi;
...
void CMainFrame::AddMenuBreak(CMenu *pMenu, BOOL bFavMenu)
{
	...
	bool bInfo = SystemParametersInfo(SPI_GETNONCLIENTMETRICS,nm.cbSize,&nm,0);
...
void CMainFrame::BuildUtilMenu()
{
	...
	int bandid	= -1;
	if(IsWindow(m_wndReBar.GetSafeHwnd()))
	{
