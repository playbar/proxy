...
CComboBoxEx* CComboBar::GetAddressBox()
{
	return &m_wndAddress;
}
...
void CComboBar::Init() 
{
	...
	if (!m_wndAddress.Create(CBS_DROPDOWN | CBS_AUTOHSCROLL | WS_CHILD, CRect(0, 0, 200, 450), this, ID_TOOLBAR_SEARCH))
	{
...
void CComboBar::InitImage()
{
	...
	//
	hbmp = pmf->GetBitmap("SearchBar.bmp");
	img.Create(16, 16, ILC_COLORDDB|ILC_MASK, nIconCount ? 4+nIconCount+1 : 4, 1);
	ImageList_AddMasked(img.GetSafeHandle(), hbmp, RGB(255,0,255));
	DeleteObject(hbmp);
	//
	if (nIconCount)
