...
*/
CMyIECommandLineInfo::CMyIECommandLineInfo()
:m_pszURLToOpen(NULL)
{

}

void CMyIECommandLineInfo::ParseParam(const TCHAR* pszParam, BOOL bFlag, BOOL bLast)
{
	CCommandLineInfo::ParseParam(pszParam, bFlag, bLast);
	{
		if(__argc >= 3)
		{
			if (lstrcmpA(pszParam, _TEXT("u")) == 0)
			{
				m_pszURLToOpen	= __targv[2];
				ASSERT(m_pszURLToOpen != NULL);
...
BOOL CMyIEApp::InitInstance()
{
	...
	if (GetModuleFileName(hInstance, lpszModule, _MAX_PATH))
    {
		//{{smallfool
		HKEY	hKey	= NULL;
		if(::RegOpenKey(HKEY_LOCAL_MACHINE, _T("Software\\EMMOA\\IGXExplorer"), &hKey) != ERROR_SUCCESS
			||::RegCreateKey(HKEY_LOCAL_MACHINE, _T("Software\\EMMOA\\IGXExplorer"), &hKey) != ERROR_SUCCESS)
		{
			AfxMessageBox("App:Fail to UpdateRegistry!!!");
			delete [] lpszModule;
			free((void*)m_pszProfileName);
			return	FALSE;
		}
		::RegSetValueEx(hKey, _T(""), NULL, REG_SZ, (LPBYTE)lpszModule, lstrlen(lpszModule));
		::RegCloseKey(hKey);
		//smallfool}}
	...
	CMainFrame* pMainFrameTmp = new CMainFrame;
	if (!pMainFrameTmp->LoadFrame(IDR_MAINFRAME, WS_OVERLAPPEDWINDOW | FWS_ADDTOTITLE, pMainFrameTmp ))
		return FALSE;
	m_pMainWnd = pMainFrameTmp;
	
	//global main frame point
	::pMainFrame = pMainFrameTmp;
	...
	int nOpenType;
	CString strURL	= cmdInfo.m_pszURLToOpen;
	...
	*/
	...
	::SetMenu(pMainFrameTmp->m_hWnd, NULL);
	...
	pMainFrameTmp->ShowWindow(m_nCmdShow);
	...
	*/
	...
	if (cmdInfo.m_nShellCommand==CCommandLineInfo::FileOpen)
	{
		...
		pMainFrameTmp->NewChildWindow(1, nOpenType, strURL);
	}
	else
	{
		//continue last error session
		if(run)
		{	pMainFrameTmp->UpdateWindow();
		...
		{
			pMainFrameTmp->OpenAllFavs(((CMainFrame*)m_pMainWnd)->m_strStartFavFolder);
		}
		else if(bGoHome == 4)
		{
			pMainFrameTmp->OpenAllFavs(((CMainFrame*)m_pMainWnd)->m_strStartFavFolder, TRUE);
		}
		else if(bGoHome == 5)
		{
			//start group
			pMainFrameTmp->OpenGroup(pMainFrameTmp->m_strStartGroup);
		}
		else if(bGoHome == 6)
		{
			//do nothing			
			...
			pMainFrameTmp->PostMessage(WM_COMMAND, ID_FILE_NEW_S, 0);
		else
			//3.2	pMainFrame->NewChildWindow(1, nOpenType, strURL, NULL, TRUE);
			pMainFrameTmp->PostMessage(WM_COMMAND, ID_FILE_NEW_BLANK, 0);
	}
	...
	if(bFirst)
	{
		CString strURL = cmdInfo.m_pszURLToOpen ? cmdInfo.m_pszURLToOpen : _TEXT("http://changyou.mainpage.net/");
