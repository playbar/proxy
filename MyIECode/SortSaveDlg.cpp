...
BOOL CSortSaveDlg::OnInitDialog() 
{
	...
	static char tmp[5]; 
	//load bmp names
	CString strTemp = ((CMyIEApp*)AfxGetApp())->m_strDefaultDirSort;
	cur[0]=0;
...
void CSortSaveDlg::OnRename() 
{
	...
	if (dlg.DoModal()==IDOK)
	{
		...
		if (strName.GetLength())
		{
			...
			//		
			CString strOldPath = ((CMyIEApp*)AfxGetApp())->m_strDefaultDirSort + strNameOld;
			CString strNewPath = ((CMyIEApp*)AfxGetApp())->m_strDefaultDirSort + strName;
