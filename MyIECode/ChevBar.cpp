...
//						md->nFlags = iMenuStyle;
					...
					{
						FavSize=((CMainFrame*)pMainFrame)->m_nFavSize;
						iMenuStyle |= MF_POPUP;
						TBBUTTON tbb;
						if( pTBar->GetToolBarCtrl().GetButton( iCount,  &tbb ))
						{
					...
					//	POSITION pos = ((CMainFrame*)pMainFrame)->m_lstrUtilNames.FindIndex(iCount);
						SetMenuText(((CMainFrame*)pMainFrame)->m_ExternalUtilList.m_UtilList.GetAt(iCount)->m_strUtilName, md);
						pop.AppendMenu ( iMenuStyle|MF_OWNERDRAW, id , (char*)md );
					}
...
//							md->nFlags = iMenuStyle;
							...
							{
								pPopup = ((CMainFrame*)pMainFrame)->GetMenu()->GetSubMenu(5)->GetSubMenu(9);
							}
							else if(id== ID_TOOLS_SETBG)
							{
								pPopup = ((CMainFrame*)pMainFrame)->GetMenu()->GetSubMenu(6)->GetSubMenu(20);
							}
							else if(id== ID_GROUP)
							{
								pPopup = ((CMainFrame*)pMainFrame)->GetMenu()->GetSubMenu(4)->GetSubMenu(0);
							}
							else if(id== ID_OPTIONS_LOADIMAGE)
							{
								pPopup = ((CMainFrame*)pMainFrame)->GetMenu()->GetSubMenu(5)->GetSubMenu(0);
							}
							else if(id== ID_TOOLS_AUTOSCROLL_USEAUTOSCROLLING)
							{
								pPopup = ((CMainFrame*)pMainFrame)->GetMenu()->GetSubMenu(6)->GetSubMenu(19);
							}
							else if(id== ID_EDIT_UNDOCLOSE)
							{
								pPopup = ((CMainFrame*)pMainFrame)->GetMenu()->GetSubMenu(1)->GetSubMenu(0);
							}
							else if(id== ID_TOOLS_ALL)
							{
								pPopup = ((CMainFrame*)pMainFrame)->GetMenu()->GetSubMenu(6)->GetSubMenu(11);
							}
							else if(id == ID_OPTIONS_USE_PROXY)
							{
								...
								int i=0;
								//CString str, str1;
				/*				POSITION pos1=((CMainFrame*)pMainFrame)->m_aProxySpeed.GetHeadPosition(), pos = ((CMainFrame*)pMainFrame)->m_astrProxy.GetHeadPosition(), pos2 = ((CMainFrame*)pMainFrame)->m_astrProxyName.GetHeadPosition();
								while(pos!=NULL && pos1!=NULL && pos2!=NULL)
								{
									str = ((CMainFrame*)pMainFrame)->m_astrProxyName.GetAt(pos2);
									str = str.Left(50);
									str1.LoadString(IDS_SP_INVALID + ((CMainFrame*)pMainFrame)->m_aProxySpeed.GetAt(pos1));
									str1 = "(" + str1 + ")";
									if(str.Left(1)=="-")
									{
										menu.AppendMenu(MF_GRAYED|MF_STRING|MF_DISABLED, 0, str);
									}
									else
									...
									((CMainFrame*)pMainFrame)->m_astrProxyName.GetNext(pos2);
								...
								int pcates = 0;
								int l = 0;
								for( l = 0;l <= ((CMainFrame*)pMainFrame)->m_aProxyCategories.m_ProxyCategory.GetUpperBound();l++)
								{	
									...
									if(pl!=NULL && !pl->m_bIsWebProxy)
									{
										...
										for(i=0; i<=pl->m_Proxies.GetUpperBound(); i++)
										{
											...
											else
											{
												...
												if(pcates>1)
													submenu.AppendMenu(MF_STRING, 0x6320 + index, str+str1);
												else if(pcates==1)
													menu.AppendMenu(MF_STRING, 0x6320 + index, str+str1);
		
												pl->m_Proxies.ElementAt(i)->index = index;
												if(bUseProxy && ((CMainFrame*)pMainFrame)->m_strCurProxy == pl->m_Proxies.ElementAt(i)->m_strProxy)
												{
													if(pcates>1)
														submenu.CheckMenuItem(0x6320 + index, MF_CHECKED | MF_BYCOMMAND );
													else if(pcates==1)
														menu.CheckMenuItem(0x6320 + index, MF_CHECKED | MF_BYCOMMAND );
												}
												index++;
											}
										}
										if(pcates>1)
										{
											menu.AppendMenu(MF_POPUP, (UINT)submenu.m_hMenu, pl->m_strCategoryName);
											submenu.Detach();
										}
									}
								}
							...
							else if(id == ID_OPTIONS_USE_WEBP)
							{
								...
								// the font popup is stored in a resource
								menu.CreatePopupMenu();
							//	POSITION pos = ((CMainFrame*)pMainFrame)->m_astrWebProxy.GetHeadPosition(), pos1 = ((CMainFrame*)pMainFrame)->m_aWebProxySpeed.GetHeadPosition(), pos2 = ((CMainFrame*)pMainFrame)->m_astrWebProxyName.GetHeadPosition();
								/*3.1
								...
								((CMainFrame*)pMainFrame)->BuildWebProxyMenu(&menu, 2);
								pPopup = &menu;
							}
